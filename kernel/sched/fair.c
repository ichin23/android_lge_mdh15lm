...
void get_cur_cpu_load(void)
{
	...
	for (cpu = 0; cpu < NR_CPUS; ++cpu) {
		rq = cpu_rq(cpu);
		load_buffer[cpu] = rq->cpu_load[2] / 3;
	}
}
#endif
...
#ifdef CONFIG_SCHED_MIGRATION_COST_POLICY_GRADIENT
	if (this_rq->avg_idle <
		(sysctl_sched_migration_cost + policy_sched_migration_cost))
...
out:
	...
	if (need_decay) {
		...
		rq->max_idle_balance_cost =
			max((u64)(sysctl_sched_migration_cost +
				policy_sched_migration_cost), max_cost);
#else
		rq->max_idle_balance_cost =
