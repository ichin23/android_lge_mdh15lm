...
#ifdef CONFIG_SCHED_MIGRATION_COST_POLICY_GRADIENT
extern int policy_sched_migration_cost;

extern unsigned int load_buffer[NR_CPUS];
extern unsigned int idle_balance_try_count;
extern unsigned int idle_balance_reject_count;
extern unsigned int idle_balance_count;

extern void get_cur_cpu_load(void);
#endif

#ifdef CONFIG_CAPACITY_MARGIN_DVFS_POLICY_GRADIENT
extern int policy_capacity_margin_dvfs;

extern unsigned int util_buffer[NR_CPUS];

extern int init_capacity_margin_dvfs_pg(void);
extern int exit_capacity_margin_dvfs_pg(void);
#endif
