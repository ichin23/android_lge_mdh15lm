...
LGE_DTS_DIR := $(subst $\",,$(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES))/
LGE_DTS_FILES := $(notdir $(basename $(wildcard $(srctree)/arch/arm64/boot/dts/$(LGE_DTS_DIR)*.dts)))
PROJ_DT_NAMES := $(addprefix $(LGE_DTS_DIR), $(LGE_DTS_FILES))
else ifeq ($(findstring jdm,$(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES)),jdm)
JDM_DTS_DIR := $(subst $\",,$(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES))/
JDM_DTS_FILES := $(notdir $(basename $(wildcard $(srctree)/arch/arm64/boot/dts/$(JDM_DTS_DIR)*.dts)))
PROJ_DT_NAMES := $(addprefix $(JDM_DTS_DIR), $(JDM_DTS_FILES))
else
PROJ_DT_NAMES := $(subst $\",,$(CONFIG_BUILD_ARM64_DTB_OVERLAY_IMAGE_NAMES))
endif
else
ifeq ($(findstring lge,$(CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES)),lge)
LGE_DTS_DIR := $(subst $\",,$(CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES))/
LGE_DTS_FILES := $(notdir $(basename $(wildcard $(srctree)/arch/arm/boot/dts/$(LGE_DTS_DIR)*.dts)))
PROJ_DT_NAMES := $(addprefix $(LGE_DTS_DIR), $(LGE_DTS_FILES))
else ifeq ($(findstring jdm,$(CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES)),jdm)
JDM_DTS_DIR := $(subst $\",,$(CONFIG_BUILD_ARM_DTB_OVERLAY_IMAGE_NAMES))/
JDM_DTS_FILES := $(notdir $(basename $(wildcard $(srctree)/arch/arm/boot/dts/$(JDM_DTS_DIR)*.dts)))
PROJ_DT_NAMES := $(addprefix $(JDM_DTS_DIR), $(JDM_DTS_FILES))
else
...
ifneq ($(LGE_DTS_DIR),)
	for i in $(PROJ_DTS_FILES); do \
		prj_path=$(DRVGEN_OUT)/$(MTK_PROJECT) ;\
		dws_path=$(srctree)/$(DRVGEN_PATH)/$(MTK_PROJECT).dws ;\
		if [ -f $$dws_path ] ; then \
			mkdir -p $$prj_path ;\
			$(python) $(DRVGEN_TOOL) $$dws_path $$prj_path $$prj_path cust_dtsi;\
		fi \
	done
else ifneq ($(JDM_DTS_DIR),)
	for i in $(PROJ_DTS_FILES); do \
    		prj_path=$(DRVGEN_OUT)/$(MTK_PROJECT) ;\
		dws_path=$(srctree)/$(DRVGEN_PATH)/$(MTK_PROJECT).dws ;\
		if [ -f $$dws_path ] ; then \
...
echo $(abspath $(addsuffix .dtb,$(addprefix $(objtree)/arch/$(SRCARCH)/boot/dts/,$(1)))) >>$(2);\
echo " id=$(my_dtbo_id)" >>$(2);\
lge_rev=`grep -m 1 "lge,pcb_revision.*;" $(abspath $(addsuffix .dts,$(addprefix $(srctree)/arch/$(SRCARCH)/boot/dts/,$(1)))) | sed 's/\t//g' | sed 's/ //g' | sed 's/lge,pcb_revision=<//g' | sed 's/>;.*$$//g'`;\
echo " rev=$$lge_rev" >>$(2);\
...
$(objtree)/dtboimg.cfg: FORCE
	rm -f $@.tmp
	$(foreach f,$(PROJ_DT_NAMES),$(call mk_dtboimg_cfg,$(f),$@.tmp))
	if ! cmp -s $@.tmp $@; then \
		mv $@.tmp $@; \
	else \
		rm $@.tmp; \
