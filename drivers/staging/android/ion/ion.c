...
static int proc_ion_memory_usage_show(struct seq_file *m, void *v)
{
	ssize_t ret = 0;
...
static int ion_memory_usage_open(struct inode *inode, struct file *file)
{
	return single_open(file, proc_ion_memory_usage_show, PDE_DATA(inode));
}

static const struct file_operations ion_memory_usage_procfs = {
	.open    = ion_memory_usage_open,
	.read    = seq_read,
	.llseek  = seq_lseek,
	.release = single_release,
};
...
#ifdef CONFIG_PROC_FS
	if (heap->id == ION_HEAP_TYPE_MULTIMEDIA) {
		proc_file = proc_create_data("memory_usage", 0,
				dev->ion_pentry, &ion_memory_usage_procfs, heap);
		if (proc_file == NULL)
			pr_err("%s: failed to proc create heap usage\n",__func__);
	}
#endif
...
#ifdef CONFIG_PROC_FS
	idev->ion_pentry = proc_mkdir("ion", NULL);
	if (!idev->ion_pentry)
		pr_err("ion: failed to create procfs ion memory directory\n");
#endif
debugfs_done:
