...
#define TTF_START_MS_DELAY (6000) /* 6 sec */
...
static int additional_delay = TTF_START_MS;
...
static void chgctrl_time_to_full(struct work_struct *work)
{
	...
	int ibat;
	int i;
	const char *chgtype_usb = chgctrl_get_usb_type_name(chip);
	...
	if(!strcmp(chgtype_usb,"NON_STD")) {
		if (additional_delay == TTF_START_MS) {
			additional_delay = TTF_START_MS_DELAY;
			pr_info("[TTF] additional wait %dms by Non_std\n",
					additional_delay);
			schedule_delayed_work(to_delayed_work(work),
				msecs_to_jiffies(additional_delay));
			return;
		}
	}
				       ...
				       struct power_supply *psy)
{
	...
	if (chgctrl_get_floated_charger()) {
