...
int charger_dev_is_hz_enabled(struct charger_device *chg_dev, bool *en)
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->is_hz_enabled)
		return chg_dev->ops->is_hz_enabled(chg_dev, en);

	return -ENOTSUPP;
}
EXPORT_SYMBOL(charger_dev_is_hz_enabled);
...
{
	if (chg_dev != NULL && chg_dev->ops != NULL &&
	    chg_dev->ops->get_recharge_voltage)
