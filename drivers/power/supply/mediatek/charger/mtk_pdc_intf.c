...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
		chgctrl_get_max_watt(&info->pdc.pdc_max_watt);
#endif
	}
...
int mtk_pdc_setup(struct charger_manager *info, int idx)
{
	...
	if (pd->pd_idx != idx || force_update) {
		...
		if (info->lge_charging) {
			/*
			 * do nothing here.
			 * lge_charging will set input current
			 */
		} else
#endif
			...
			int *newidx)
{
...
#ifdef CONFIG_LGE_PM_CHARGER_CONTROLLER
	{
		...
		chgctrl_get_max_icl(&ma);
		if (ma > 0)
			ma /= 1000;	/* convert uA to mA */

		pd_max_watt = cap->max_mv[idx] * (min(cap->ma[idx], ma)
				/ 100 * (100 - info->data.ibus_err) - 100);
		now_max_watt = cap->max_mv[idx] * ibus + chg2_watt;
		pd_min_watt = cap->max_mv[pd->pd_buck_idx] * min(cap->ma[pd->pd_buck_idx], ma)
				/ 100 * (100 - info->data.ibus_err) - info->data.vsys_watt;
	}
#else /* MediaTek */
