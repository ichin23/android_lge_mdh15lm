...
#if defined(CONFIG_LGE_ATMF04_2CH)
#define ATMF04_CR_DUTY_LOW        320  // CR range MIN
#define ATMF04_CR_DUTY_HIGH       3008 // CR Range MAX
#define ATMF04_CS_DUTY_LOW        320  // CS Range MIN
		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	PINFO("irq gpio value [%u] safe_duty [%d]", gpio_get_value(data->platform_data->irq_gpio), get_bit(safe_duty, 7));

#if 1 // debugging calibration paused
	if (ret || (get_bit(safe_duty, 7) != 1)) {
		// There is i2c failure or safe duty bit is not 1
		PERR("calibration result is failed.....");
		cal_result = false;
	} else {
		PINFO("calibration result is successful!!!!!");
		cal_result = true;
	}
#endif
...
static ssize_t atmf04_show_check_far(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	{
		check_result += 1;
		PERR("fail CR duty range");
	}

	// CS Duty Rnage Check
	if ((cs1_duty_val < ATMF04_CS_DUTY_LOW) || (cs1_duty_val > ATMF04_CS_DUTY_HIGH)
			|| (cs2_duty_val < ATMF04_CS_DUTY_LOW) || (cs1_duty_val > ATMF04_CS_DUTY_HIGH))
	{
		check_result += 1;
		PERR("fail CS duty range");
	}
		...
		const struct i2c_device_id *id)
{
	...
	client->dev.init_name = "atmf04_dev";
