...
static void ili9881h_ddi_wr_pack(int packet)
{
	int retry = 100;
	u32 reg_data = 0;

	while (retry--) {
		if (ili9881h_ice_reg_read(touch_dev->dev, 0x73010, &reg_data, sizeof(u8)) < 0)
	...
	if (ili9881h_ice_reg_write(touch_dev->dev, 0x73000, packet, 4) < 0)
		TOUCH_E("Write %x at 0x73000\n", packet);
}

static u8 ili9881h_ddi_rd_pack(int packet)
{
	int retry = 100;
	u8 reg_data = 0;

	ili9881h_ddi_wr_pack(packet);
	while (retry--) {
		if (ili9881h_ice_reg_read(touch_dev->dev, 0x4800A, &reg_data, sizeof(u8)) < 0)
	...
	if (ili9881h_ice_reg_write(touch_dev->dev, 0x4800A, 0x02, 1) < 0)
		TOUCH_E("Write 0x2 at 0x4800A\n");

	if (ili9881h_ice_reg_read(touch_dev->dev, 0x73016, &reg_data, sizeof(u8)) < 0)
		TOUCH_E("Read 0x73016 error\n");

	return reg_data;
}

static void ili9881h_set_ddi_reg_onepage(u8 page, u8 reg, u8 data)
{
	...
	if (ili9881h_ice_mode_enable(touch_dev->dev, MCU_STOP) < 0)
		TOUCH_E("Enable ice mode failed before writing ddi reg\n");

	/*TDI_WR_KEY*/
	ili9881h_ddi_wr_pack(0x1FFF9527);
	/*Switch to Page*/
	ili9881h_ddi_wr_pack(setpage);
	/* Page*/
	ili9881h_ddi_wr_pack(setreg);
	/*TDI_WR_KEY OFF*/
	ili9881h_ddi_wr_pack(0x1FFF9500);

	if (ili9881h_ice_mode_disable(touch_dev->dev) < 0)
		TOUCH_E("Disable ice mode failed after writing ddi reg\n");
}

void ili9881h_get_ddi_reg_onepage(u8 page, u8 reg, u8 *data)
{
	u32 setpage = 0x1FFFFF00 | page;
	...
	ili9881h_ddi_wr_pack(setpage);
	/*TDI_RD_KEY*/
	ili9881h_ddi_wr_pack(0x1FFF9487);

	/*( *( __IO uint8 *)	(0x4800A) ) =0x2*/
	if (ili9881h_ice_reg_write(touch_dev->dev, 0x4800A, 0x02, 1) < 0)
		TOUCH_E("Write 0x2 at 0x4800A\n");

	*data = ili9881h_ddi_rd_pack(setreg);
	TOUCH_I("check page = 0x%X, reg = 0x%X, read 0x%X\n", page, reg, *data);

	/*TDI_RD_KEY OFF*/
	ili9881h_ddi_wr_pack(0x1FFF9400);
	/*TDI_WR_KEY OFF*/
	ili9881h_ddi_wr_pack(0x1FFF9500);
...
static ssize_t ilitek_proc_dump_flash_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	ili9881h_dump_flash_data(touch_dev->dev, temp[1], temp[2]);
	return size;
}
...
static ssize_t ilitek_proc_dump_iram_write(struct file *filp, const char *buff, size_t size, loff_t *pPos)
{
	...
	TOUCH_I("Start = 0x%x, End = 0x%x\n", temp[1], temp[2]);
	ili9881h_dump_iram_data(touch_dev->dev, temp[1], temp[2]);
	return size;
}
...
static ssize_t ilitek_proc_fw_upgrade_read(struct file *filp, char __user *buff, size_t size, loff_t *pPos)
{
	...
	ret = ili9881h_fw_upgrade(touch_dev->dev, fwpath);
	if (ret) {
...
static ssize_t ilitek_node_ioctl_write(struct file *filp, const char *buff, size_t size, loff_t *pos)
{
	...
	if (strncmp(cmd, "hwreset", strlen(cmd)) == 0) {
		...
		ili9881h_ic_info(touch_dev->dev);
		...
		ret = ili9881h_reg_write(touch_dev->dev, temp[0], &temp[1], w_len - 1);
		if (ret < 0) {
			TOUCH_E("Write error\n");
		}
		...
		ret = ili9881h_reg_read(touch_dev->dev, CMD_NONE, temp, r_len);
		if (ret < 0)
		...
		ili9881h_get_ddi_reg_onepage(data[1], data[2], &ddi_data);
		...
		ili9881h_set_ddi_reg_onepage(data[1], data[2], data[3]);
		...
		u8 power = 0;
		ili9881h_get_ddi_reg_onepage(0, 0xA, &power);
		TOUCH_I("Check DDI power status = %x\n", power);
	} else if (strncmp(cmd, "pclatch", strlen(cmd)) == 0) {
		ili9881h_get_pc_latch(touch_dev->dev);
	} else {
