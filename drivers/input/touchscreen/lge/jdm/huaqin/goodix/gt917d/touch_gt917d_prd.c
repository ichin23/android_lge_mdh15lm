...
static int SendCfgConfirm(struct device *dev)
{
	u8 i=0;
	int retry=0;
	...
	struct i2c_client *client = d->client;
		...
		for (i = 0; i < 3; i++) {
			...
			while (retry--) {
				...
				if (ret < 0 || (read_state[2] & 0x80) == 0x80)
					break;
				touch_msleep(5);
			}

			if (!ret && (read_state[2] & 0x80) == 0x80) {
				read_state[2] = 0x00;
				ret=gt917d_i2c_write(client, read_state, 3);
					if (ret < 0)
					return ret;
			} else if (ret < 0) {
				return ret;
			}
			TOUCH_D(JITTER, "Clear int status{0x814e}");
		}
		return 0;
}
...
static int jitter_test(struct device *dev, int *test_result)
{
	...
	ret=gt917d_i2c_write(client, test_config_jitter, ARRAY_SIZE(test_cfg_info_group) + GT917D_ADDR_LENGTH);
	if (ret < 0) {
		TOUCH_E("Failed to send test config:%d", ret);
		return ret;
	} else {
		SendCfgConfirm(dev);
	}
