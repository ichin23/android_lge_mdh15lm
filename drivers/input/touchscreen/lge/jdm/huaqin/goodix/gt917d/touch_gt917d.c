...
#define LPWG_FAILREASON_BUF_LEN 10
...
int gt917d_lpwg_failreason(struct device *dev)
{
	int ret = 0;
	int i = 0;
	u16 lpwg_fail_buffer[LPWG_FAILREASON_BUF_LEN] = {0, };
	u8 knockon_fail_reason = 0;
	u8 swipe_fail_reason = 0;

	TOUCH_TRACE();

	ret = gt917d_reg_read(dev, GT917D_REG_KNOCKON_ERR_REASON, &lpwg_fail_buffer, sizeof(lpwg_fail_buffer));
	if(ret < 0) {
		TOUCH_E("Failed read failreason_info\n");
		return ret;
	}

	for(i = 0 ; i < LPWG_FAILREASON_BUF_LEN ; i++) {
		if(lpwg_fail_buffer[i] == 0)
			return ret;
		if(lpwg_fail_buffer[i] & 0x01) {
			swipe_fail_reason = lpwg_fail_buffer[i] >> 8;
			if (swipe_fail_reason >= 0 && swipe_fail_reason <= 12)
				TOUCH_I("[FAIL_REASON_SWIPE] %s\n", swipe_debug_str[swipe_fail_reason]);
			else
				TOUCH_I("swipe_fail_reason = %d\n", swipe_fail_reason);
		}
		else {
			knockon_fail_reason = lpwg_fail_buffer[i] >> 8;
			if (knockon_fail_reason >= 0 && knockon_fail_reason <= 7)
				TOUCH_I("[FAIL_REASON_KNOCKON] %s\n", lpwg_failreason_tci_str[knockon_fail_reason]);
			else
...
static ssize_t store_reg_ctrl(struct device *dev, const char *buf, size_t count)
{
	...
	if (sscanf(buf, "%5s %hx %hhx", command, &addr, &value) <= 0)
		return count;

	mutex_lock(&ts->lock);

	if (!strcmp(command, "write")) {
		if (gt917d_reg_write(dev, addr, &value, sizeof(value)) < 0)
			TOUCH_E("reg addr 0x%04X write fail\n", addr);
		else
			TOUCH_I("reg[0x%04X] = 0x%02X\n", addr, value);
	} else if (!strcmp(command, "read")) {
		if (gt917d_reg_read(dev, addr, &value, sizeof(value)) < 0)
			TOUCH_E("reg addr 0x%04X read fail\n", addr);
		else
			TOUCH_I("reg[0x%04X] = 0x%02X\n", addr, value);
	} else {
		TOUCH_I("Usage\n");
