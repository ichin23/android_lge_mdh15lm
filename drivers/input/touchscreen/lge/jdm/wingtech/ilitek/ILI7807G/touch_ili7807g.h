...
#ifndef LGE_TOUCH_ILI7807G_H
#define LGE_TOUCH_ILI7807G_H

#define CHIP_ID				0x7807
#define CHIP_TYPE			0x1B
#define CORE_TYPE			0x03
#define TPD_WIDTH			2048

#define MAX_TOUCH_NUM			10

#define ILI7807G_SLAVE_I2C_ADDR		0x41
#define ILI7807G_ICE_MODE_ADDR		0x181062
#define ILI7807G_PID_ADDR		0x4009C
#define ILI7807G_WDT_ADDR		0x5100C
#define ILI7807G_IC_RESET_ADDR		0x040050
#define ILI7807G_IC_RESET_KEY		0x00019878
#define ILI7807G_IC_RESET_GESTURE_ADDR	0x040054
#define ILI7807G_IC_RESET_GESTURE_KEY	0xF38A94EF
#define ILI7807G_IC_RESET_GESTURE_RUN	0xA67C9DFE
...
struct ili7807g_touch_abs_data {
	u8 y_high:4;
	u8 x_high:4;
	u8 x_low;
	u8 y_low;
...
/*
struct ili7807g_touch_shape_data {
	s8 degree;
	u8 width_major_high;
	u8 width_major_low;
	u8 width_minor_high;
...
*/
/* report packet */
struct ili7807g_touch_info {
	u8 packet_id;
	struct ili7807g_touch_abs_data abs_data[10];
	u8 p_sensor:4;
	u8 key:4;
	//struct ili7807g_touch_shape_data shape_data[10];
	u8 checksum;
} __packed;

struct ili7807g_debug_info {
...
struct ili7807g_gesture_info {
	u8 data[GESTURE_MODE_PACKET_INFO_LENGTH];
} __packed;

struct ili7807g_chip_info {
	u32 pid;
	u16 id;
	u8 type;
	u8 ver;
...
struct ili7807g_fw_extra_info {
	u8 build;
} __packed;
struct ili7807g_protocol_info {
	u8 command_id;
	u8 major;
	u8 mid;
	u8 minor;
...
struct ili7807g_ic_info {
	struct ili7807g_chip_info chip_info;
	struct ili7807g_fw_info fw_info;
	struct ili7807g_fw_extra_info fw_extra_info;
	struct ili7807g_protocol_info protocol_info;
	struct ili7807g_core_info core_info;
};

struct ili7807g_panel_info {
	u8 command_id;
	u8 nMinX;
	u8 nMinY;
	u8 nMaxX_Low;
...
struct ili7807g_panel_extra_info {
...
struct ili7807g_tp_info {
	struct ili7807g_panel_info panel_info;
	struct ili7807g_panel_extra_info panel_extra_info;
};

struct ili7807g_data {
	struct device *dev;
	struct kobject kobj;
...
//	struct delayed_work fb_notify_work;
	...
	struct ili7807g_ic_info ic_info;
	struct ili7807g_tp_info tp_info;
	struct ili7807g_touch_info touch_info;
	struct ili7807g_debug_info debug_info;
	struct ili7807g_gesture_info gesture_info;
...
extern int ili7807g_dump_flash_data(struct device *dev, u32 start, u32 end);
extern void ili7807g_dump_iram_data(struct device *dev, u32 start, u32 end);
extern int ili7807g_switch_fw_mode(struct device *dev, u8 mode);
extern int ili7807g_ice_mode_disable(struct device *dev);
extern int ili7807g_ice_mode_enable(struct device *dev, int mcu_status);
extern int ili7807g_ice_reg_write(struct device *dev, u32 addr, u32 data, int size);
extern int ili7807g_ice_reg_read(struct device *dev, u32 addr, void* data, int size);
extern int ili7807g_reg_read(struct device *dev, u8 cmd, void *data, int size);
extern int ili7807g_reg_write(struct device *dev, u8 cmd, void *data, int size);
extern int ili7807g_tp_info(struct device *dev);
extern int ili7807g_reset_ctrl(struct device *dev, int ctrl);
extern int ili7807g_ic_info(struct device *dev);
extern int ili7807g_check_cdc_busy(struct device *dev, int conut, int delay);
extern void ili7807g_dump_packet(void *data, int type, int len, int row_len, const char *name);
extern u8 ili7807g_calc_data_checksum(void *pMsg, u32 nLength);
// int ili7807g_ic_info(struct device *dev);
// int ili7807g_tc_driving(struct device *dev, int mode);
// int ili7807g_irq_abs(struct device *dev);
// int ili7807g_irq_lpwg(struct device *dev);
// int ili7807g_irq_handler(struct device *dev);
// int ili7807g_check_status(struct device *dev);
// int ili7807g_debug_info(struct device *dev);

static inline struct ili7807g_data *to_ili7807g_data(struct device *dev)
{
	return (struct ili7807g_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ili7807g_data *to_ili7807g_data_from_kobj(struct kobject *kobj)
{
	return (struct ili7807g_data *)container_of(kobj,
			struct ili7807g_data, kobj);
}
static inline int ili7807g_read_value(struct device *dev,
		u16 addr, u32 *value)
{
	return ili7807g_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ili7807g_write_value(struct device *dev,
		u16 addr, u32 value)
{
	return ili7807g_reg_write(dev, addr, &value, sizeof(value));
}
...
extern int ili7807g_apk_init(struct device *dev);
extern int ili7807g_check_int_status(struct device *dev, bool high);
extern int ili7807_get_fw_data_wrapper(struct device *dev, u8 *wdata, int wsize, void *rdata, int rsize);

#endif /* LGE_TOUCH_ILI7807G_H */
