...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			...
			TOUCH_D(PRODUCTION, "(key: %s, value:%s) => (ini key: %s, val: %s)\n",
					key,
					value,
					ini_info[i].key_name,
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	TOUCH_D(PRODUCTION, "Read X/Y Channel length = %d, mode = %d\n", len, mode);
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(cmd, 8, sizeof(cmd), 0, "Open SP command");

	/* NOTE: If TP driver is doing MP test and commanding 0xF1 to FW, we add a checksum
	 * to the last index and plus 1 with size. */
	check_sum = ili7807g_calc_data_checksum(&cmd[0], cdc_len - 1);
	cmd[cdc_len - 1] = check_sum;
	header = cmd[0];
	ret = ili7807g_reg_write(core_mp->dev, header, &cmd[1], cdc_len - 1);
	if (ret < 0) {
	...
	if (core_mp->busy_cdc == POLL_CHECK)
		ret = ili7807g_check_cdc_busy(core_mp->dev, 50, 50);
	else
		ret = ili7807g_check_int_status(core_mp->dev, true);
	...
	if (core_mp->core_ver < CORE_VER_1420) {
		...
		if (ili7807g_reg_write(core_mp->dev, CMD_READ_DATA_CTRL, &cmd[1], 1)) {
		...
		if (ili7807g_reg_write(core_mp->dev, CMD_GET_CDC_DATA, cmd, 0)) {
	...
	if (ili7807g_reg_read(core_mp->dev, CMD_NONE, ori, len)) {
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(ori, 8, len, 0, "Open SP CDC original");
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(buf, 10, core_mp->frame_len,  core_mp->xch_len, "Open SP CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	TOUCH_D(PRODUCTION, "Read X/Y Channel length = %d\n", len);
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(cmd, 8, cdc_len, 0, "Mutual CDC command");
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(ori, 8, len, 0, "Mutual CDC original");
	...
	if (MP_DBG_MSG)
		ili7807g_dump_packet(frame_buf, 32, core_mp->frame_len, core_mp->xch_len, "Mutual CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int mutual_test(int index)
{
	...
	TOUCH_D(PRODUCTION, "index = %d, desp = %s, Frame Count = %d\n",
...
static int open_sp_test(int index)
{
	...
	TOUCH_D(PRODUCTION, "open_sp_test: frame_cont %d, AA %d, Border %d, Notch %d, full_open_rate %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG) {
			ili7807g_dump_packet(open[i].cbk_700, 10, core_mp->frame_len, core_mp->xch_len, "cbk 700");
			ili7807g_dump_packet(open[i].cbk_250, 10, core_mp->frame_len, core_mp->xch_len, "cbk 250");
			ili7807g_dump_packet(open[i].cbk_200, 10, core_mp->frame_len, core_mp->xch_len, "cbk 200");
		}
		...
		if (MP_DBG_MSG) {
			ili7807g_dump_packet(open[i].charg_rate, 10, core_mp->frame_len, core_mp->xch_len, "origin charge rate");
			ili7807g_dump_packet(open[i].full_Open, 10, core_mp->frame_len, core_mp->xch_len, "origin full open");
		}
		...
		if (MP_DBG_MSG)
			ili7807g_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after full_open_rate_compare");
		...
		if (MP_DBG_MSG)
			ili7807g_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after compare charge rate");
...
static int open_c_test(int index)
{
	...
	TOUCH_D(PRODUCTION, "open_test_c: frame_cont = %d, gain = %d, tvch = %d, tvcl = %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG)
			ili7807g_dump_packet(open[i].dcl_cap, 10, core_mp->frame_len, core_mp->xch_len, "DCL_Cap");
...
static int mp_init_item(struct device *dev)
{
	...
	core_mp->core_ver = COM_3BYTE(core_info->code_base, core_info->minor, core_info->revision_minor);
...
static void mp_test_run(int index)
{
	...
	TOUCH_D(PRODUCTION, "v_tdf_1 = %d, v_tdf_2 = %d, h_tdf_1 = %d, h_tdf_2 = %d", tItems[i].v_tdf_1,
...
static int mp_write_ret_to_terminal(char *buf, bool lcm_on)
{
	...
	int lpwg_doze_raw_ret = MP_DATA_FAIL;
	int lpwg_doze_p2p_ret = MP_DATA_FAIL;
	...
	for (seq = 0; seq < core_mp->run_num; seq++) {
		i = core_mp->run_index[seq];
		if (tItems[i].item_result == MP_DATA_PASS) {
			if (ipio_strcmp(tItems[i].desp, "raw data(no bk)"))
				rawdata_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "noise peak to peak(ic only)"))
				jitter_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "open test_c"))
				open_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "short test"))
				short_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "raw data_td (lcm off)"))
				lpwg_doze_raw_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "peak to peak_td (lcm off)"))
				lpwg_doze_p2p_ret = MP_DATA_PASS;
		}
	}
	...
	if (lcm_on) {
		...
		if (open_ret == MP_DATA_PASS && short_ret == MP_DATA_PASS) {
			len += snprintf(buf + len, PAGE_SIZE - len, "Channel Status : Pass\n");
			TOUCH_I("Channel Status : Pass\n");
		} else {
			len += snprintf(buf + len, PAGE_SIZE - len,"Channel Status : Fail (open:%d/short:%d)\n",
				(open_ret == MP_DATA_FAIL) ? 0 : 1, (short_ret == MP_DATA_FAIL) ? 0 : 1);
			TOUCH_I("Channel Status : Fail (open:%d/short:%d)\n",
				(open_ret == MP_DATA_FAIL) ? 0 : 1, (short_ret == MP_DATA_FAIL) ? 0 : 1);
		}
	} else {
		if (lpwg_doze_raw_ret == MP_DATA_PASS && lpwg_doze_p2p_ret == MP_DATA_PASS) {
			len += snprintf(buf + len, PAGE_SIZE - len, "LPWG RawData : Pass\n");
			TOUCH_I("LPWG RawData : Pass\n");
		} else {
			len += snprintf(buf + len, PAGE_SIZE - len, "LPWG RawData : Fail (doze_raw:%d/doze_p2p:%d)\n",
					(lpwg_doze_raw_ret == MP_DATA_FAIL)? 0 : 1, (lpwg_doze_p2p_ret == MP_DATA_FAIL)? 0 : 1);
...
int ili7807g_int_action_ctrl(struct device *dev, bool keep_status)
{
	int ret = 0;
	u8 data[2] = {0};
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	/* Switch to test mode */
	if (ili7807g_switch_fw_mode(dev, FIRMWARE_TEST_MODE)) {
		TOUCH_E("Failed to switch test mode\n");
		goto out;
	}

	if (d->int_action)
		ili7807g_int_action_ctrl(dev, true);
...
out:
	...
	if (d->int_action)
		ili7807g_int_action_ctrl(dev, false);

	/* Reset IC to get back to demo mode.  */
	ili7807g_reset_ctrl(dev, HW_RESET_SYNC);
	/* Switch to demo mode */
	//if (ili7807g_switch_fw_mode(dev, FIRMWARE_DEMO_MODE))
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili7807g_data *d = to_ili7807g_data(dev);
	struct ili7807g_debug_info *debug_info = &d->debug_info;
	struct ili7807g_panel_info *panel_info = &d->tp_info.panel_info;
	int ret = 0, try = 10;
	int log_ret = 0;
...
retry:
	...
	ret = ili7807g_reg_write(dev, CMD_READ_DATA_CTRL, &tmp, 1);

	tmp = 0x01; //Set signal once data mode
	ret = ili7807g_reg_write(dev, 0xB7, &tmp, 1);
	if (ret) {
	...
	ret = ili7807g_reg_read(dev, CMD_NONE, debug_info->data, read_length);
	...
	//Compare checksum
	check_sum = ili7807g_calc_data_checksum(debug_info->data, (read_length - 1));
...
retry:
	...
	if (debug_info->data[0] != 0xB7 && try != 0) {
		...
		ili7807g_dump_packet(debug_info->data, 10, read_length, col, "Header");
		try--;
		goto retry;
	}
	...
	if (check_sum != debug_info->data[read_length -1]) {
		TOUCH_E("Packet check sum Error, check_sum = %d, abs_data = %d\n",
				check_sum, debug_info->data[read_length -1]);
		ili7807g_dump_packet(debug_info->data, 10, read_length, col, "raw");
		try--;
		goto retry;
	}
...
int ili7807g_mp_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
