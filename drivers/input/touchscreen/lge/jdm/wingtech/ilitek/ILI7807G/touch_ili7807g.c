...
int ili7807g_gesture_ctrl(struct device *dev, int tci_mode, bool swipe_up_enable)
{
	int ret = 0;
	u8 data[3] = {0, };
...
int ili7807g_gesture_failreason_ctrl(struct device *dev, int onoff)
{
	int ret = 0;
	u8 data[4] = {0, };
...
int ili7807g_grip_suppression_ctrl(struct device *dev, u8 x, u8 y)
{
	int ret = 0;
	u8 data[2] = {0};
...
static int ili7807g_lpwg_control(struct device *dev, int tci_mode, bool swipe_up_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili7807g_data *d = to_ili7807g_data(dev);
	int ret = 0;
	...
	if (tci_mode >= LPWG_NONE && tci_mode <= LPWG_PASSWORD_ONLY) {
		TOUCH_I("ili7807g_lpwg_control - tci_mode = %s, swipe_up_enable = %s\n",
	...
	if (tci_mode || swipe_up_enable) {
		...
		ret = ili7807g_gesture_ctrl(dev, tci_mode, swipe_up_enable);
		ret = ili7807g_gesture_failreason_ctrl(dev, d->lpwg_failreason_ctrl);
	} else {
...
static int ili7807g_lpwg_mode(struct device *dev)
{
	...
	TOUCH_I("lpwg.mode = %d/ ts->swipe[SWIPE_U/L/R].enable = %d/%d/%d, d->lcd_mode = %d\n",
			ts->lpwg.mode, ts->swipe[SWIPE_U].enable, ts->swipe[SWIPE_L].enable, ts->swipe[SWIPE_R].enable,
			d->lcd_mode);
...
static int ili7807g_init(struct device *dev)
{
	...
	ili7807g_grip_suppression_ctrl(dev, 20, 40);
...
static int ili7807g_irq_abs(struct device *dev)
{
	...
	for (i = 0; i < MAX_TOUCH_NUM; i++) {
		...
		/*currently FW Will auto scale the report point*/
		tdata->x = nX ;
		tdata->y = nY ;
...
int ili7807g_irq_lpwg(struct device *dev)
{
	...
	ili7807g_dump_packet(data, 8, sizeof(d->touch_info), 8, "Gesture");
...
static ssize_t store_grip_suppression(struct device *dev, const char *buf, size_t count)
{
	...
	ret = ili7807g_grip_suppression_ctrl(dev, (u8)value_x, (u8)value_y);
	if (ret < 0)
