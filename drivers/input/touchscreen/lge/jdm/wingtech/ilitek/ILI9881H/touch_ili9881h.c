...
int ili9881h_gesture_ctrl(struct device *dev, int tci_mode, bool swipe_enable)
{
	...
	if (swipe_enable)
		dir = (ts->swipe[SWIPE_U].enable << 0 | ts->swipe[SWIPE_L2].enable << 1 | ts->swipe[SWIPE_R2].enable << 2);
...
int ili9881h_report_swipe_fr_buffer(struct device *dev)
{
	...
	if (!ts->swipe[SWIPE_U].enable && !ts->swipe[SWIPE_L2].enable && !ts->swipe[SWIPE_R2].enable) {
...
static int ili9881h_lpwg_mode(struct device *dev)
{
	...
	TOUCH_I("lpwg.mode = %d/ ts->swipe[SWIPE_U/L/R].enable = %d/%d/%d, d->lcd_mode = %d\n",
			ts->lpwg.mode, ts->swipe[SWIPE_U].enable, ts->swipe[SWIPE_L2].enable, ts->swipe[SWIPE_R2].enable, d->lcd_mode);
