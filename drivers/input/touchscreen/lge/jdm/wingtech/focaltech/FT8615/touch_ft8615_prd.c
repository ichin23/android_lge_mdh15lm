...
int ft8615_change_op_mode(struct device *dev, u8 op_mode)
{
	...
	data = 0x00;
	ret = ft8615_reg_read(dev, 0x00, &data, 1);
	if(ret < 0) {
	...
	data = op_mode;
	ret = ft8615_reg_write(dev, 0x00, &data, 1);
	if(ret < 0) {
...
//	mdelay(300);
	...
	if((op_mode == FTS_FACTORY_MODE) && (d->state == TC_STATE_LPWG)) {
		...
		data = 0x01;
		ret = ft8615_reg_write(dev, 0x88, &data, 1);
		if(ret < 0) {
			TOUCH_E("0x88 register 0x01 write  error\n");
			return ret;
		}
	}
...
int ft8615_switch_cal(struct device *dev, u8 cal_en)
{
	u8 ret;
	u8 data;
	...
	ret = ft8615_reg_read(dev, 0xEE, &data, 1);
	if(ret < 0) {
	...
	data = cal_en;
	ret = ft8615_reg_write(dev, 0xEE, &data, 1);
	if(ret < 0) {
...
int ft8615_prd_check_ch_num(struct device *dev)
{
	u8 ret;
	u8 data;
	...
	ret = ft8615_reg_read(dev, 0x02, &data, 1);
	if(ret < 0) {
	...
	ret = ft8615_reg_read(dev, 0x03, &data, 1);
	if(ret < 0) {
...
int ft8615_prd_get_raw_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	data = 0x00;
	ret = ft8615_reg_write(dev, 0x06, &data, 1);
	if (ret < 0) {
	...
	for (k = 0; k < 3; k++)
	{
		...
		if(bscan==true){
			/* Read Raw data */
			rawdata = 0xAD;
			ret = ft8615_reg_write(dev, 0x01, &rawdata, 1);
			if(ret < 0) {
			...
			ret = ft8615_reg_read(dev, 0x6A, &i2c_data[0], MAX_COL*MAX_ROW*2);
			if(ret < 0) {
...
int ft8615_prd_get_noise_data(struct device *dev)
{
	struct ft8615_data *d = to_ft8615_data(dev);
	int i, j, k;
	int ret = 0;
...
//	u8 frame_count;
	...
	//low  byte
	data = 0x32;
	ret = ft8615_reg_write(dev, 0x12, &data, 1);
	if (ret < 0) {
	...
	//high  byte
	data = 0x00;
	ret = ft8615_reg_write(dev, 0x13, &data, 1);
	if (ret < 0) {
	...
	// Start Noise Test
	if (d->ic_info.chip_id_low == 0x06) { /* FT8006M */
		data = 0x00;
	} else if (d->ic_info.chip_id_low == 0x22) { /* FT8615 */
		data = 0x01;
	}
	ret = ft8615_reg_write(dev, 0x11, &data, 1);
	if (ret < 0) {
	...
	// Clear Raw Data Addr
	data = 0xAD;
	ret = ft8615_reg_write(dev, 0x01, &data, 1);
	if (ret < 0) {
	...
	for (i = 0; i < 100; i++)
	{
		ret = ft8615_reg_read(dev, 0x11, &data, 1);
		if (ret < 0) {
...
int ft8615_prd_get_jitter_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
...
int ft8615_prd_get_ib_data(struct device *dev)
{
	int i, j;
	int ret = 0;
	u8 data = 0x00;
	...
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
		...
		ret = ft8615_reg_write(dev, 0x18, &data, 1);
		if (ret < 0) {
		...
		data = (offset & 0x00FF);
		ret = ft8615_reg_write(dev, 0x19, &data, 1);
		if (ret < 0) {
		...
		ret = ft8615_reg_read(dev, 0x6E, &i2c_data[offset], read_len);
		if (ret < 0) {
...
int ft8615_prd_get_delta_data(struct device *dev)
{
	int i, j, k;
	int ret = 0;
	u8 data = 0x00;
	...
	for (i = 0; (total - TEST_PACKET_LENGTH*i) > 0; i++)
	{
		...
		data = (offset & 0xFF00) >> 8;
		ret = ft8615_reg_write(dev, 0x1C, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		data = (offset & 0x00FF);
		ret = ft8615_reg_write(dev, 0x1D, &data, 1);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
		...
		ret = ft8615_reg_read(dev, 0x6A, &i2c_data[offset], read_len);
		if (ret < 0) {
			TOUCH_E("i2c error\n");
			return ret;
		}
...
int ft8615_prd_test_data(struct device *dev, int test_type, int* test_result)
{
	...
	*test_result = TEST_FAIL;
	...
	ret += snprintf(log_buf, sizeof(log_buf) - ret, "IC F/W Version : V%d.%02d\n", d->ic_info.is_official, d->ic_info.fw_version);
	...
	for (i = 0; i < MAX_ROW; i++) {
		...
		if(d->ic_info.chip_id_low == 0x22) /* 8615(Modify due to reverse from top to bottom) */
			k = MAX_ROW-i-1;
		else /* 8006M */
			k = i;
		for (j = 0; j < MAX_COL; j++) {
			...
			if( k == 31 && ( j == 8 || j == 9)) {
				TOUCH_I("ignore test in notch area[%d][%d] = %d / LowerImage[%d][%d] = %d / UpperImage[%d][%d] = %d\n"
						, k, j, fts_data[k][j], k, j, limit_lower, k, j, limit_upper);
				continue;
			}
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	ret = ft8615_change_op_mode(dev, FTS_FACTORY_MODE);
	if (ret < 0) {
	...
	ret = ft8615_prd_check_ch_num(dev);
	if (ret < 0) {
		TOUCH_E("prd check_ch_num error\n");
		goto FAIL;
	}

	ret = ft8615_prd_get_raw_data(dev);
	if (ret < 0) {
		TOUCH_E("prd get raw data error\n");
		goto FAIL;
	}

	ret_size = ft8615_prd_test_data(dev, RAW_DATA_TEST, &rawdata_ret);
	...
	ret = ft8615_prd_get_ib_data(dev);
	if (ret < 0) {
		TOUCH_E("prd get ib data error\n");
		goto FAIL;
	}

	ret_size = ft8615_prd_test_data(dev, IB_DATA_TEST, &ib_ret);
	...
	ret = ft8615_prd_get_noise_data(dev);
	if (ret < 0) {
		TOUCH_E("prd get noise data error\n");
		goto FAIL;
	}

	ret_size = ft8615_prd_test_data(dev, NOISE_TEST, &noise_ret);
	...
	ret = ft8615_change_op_mode(dev, FTS_WORK_MODE);
	if (ret < 0) {
	...
	if ((rawdata_ret == TEST_PASS) && (noise_ret == TEST_PASS)) {
		...
		ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "Raw Data : Fail (raw:%d/noise:%d)\n",
	...
	if (ib_ret == TEST_PASS) {
		...
		ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "Channel Status: Fail (ib:%d)\n",
				(ib_ret == TEST_FAIL)? 0 : 1);
		TOUCH_I("Channel Status : Fail (ib:%d)\n",
				(ib_ret == TEST_FAIL)? 0 : 1);
	}
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret = ft8615_prd_get_delta_data(dev);
	if(ret < 0)
		goto FAIL;
	...
	ret_size = ft8615_prd_test_data(dev, DELTA_SHOW, &test_result);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret_size = ft8615_prd_test_data(dev, RAW_DATA_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret_size = ft8615_prd_test_data(dev, NOISE_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
	TOUCH_I("Noise Test Result : %d\n", test_result);
	//printk("%s\n", log_buf);
	mutex_unlock(&ts->lock);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret = ft8615_prd_get_jitter_data(dev);
	if(ret < 0)
		goto FAIL;
	...
	ret_size = ft8615_prd_test_data(dev, JITTER_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
...
//	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	...
	ret_size = ft8615_prd_test_data(dev, IB_DATA_TEST, &test_result);
	memcpy(buf, log_buf, ret_size);
	TOUCH_I("IB Data Test Result : %d\n", test_result);
	//printk("%s\n", log_buf);
	touch_interrupt_control(ts->dev, INTERRUPT_ENABLE);
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	ret_size = ft8615_prd_test_data(dev, LPWG_RAW_DATA_TEST, &rawdata_ret);
	...
	ret_size = ft8615_prd_test_data(dev, LPWG_IB_DATA_TEST, &ib_ret);
	...
	ret_size = ft8615_prd_test_data(dev, LPWG_NOISE_TEST, &noise_ret);
	...
	if ((rawdata_ret == TEST_PASS) && (ib_ret == TEST_PASS) && (noise_ret == TEST_PASS)) {
		ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "LPWG RawData : Pass\n");
		TOUCH_I("LPWG Raw Data : Pass\n");
	} else {
		ret += snprintf(log_buf + ret, sizeof(log_buf) - ret, "LPWG RawData : Fail (raw:%d/ib:%d/noise:%d)\n",
				(rawdata_ret == TEST_FAIL)? 0 : 1, (ib_ret == TEST_FAIL)? 0 : 1, (noise_ret == TEST_FAIL)? 0 : 1);
		TOUCH_I("LPWG RawData : Fail (raw:%d/ib:%d/noise:%d)\n",
				(rawdata_ret == TEST_FAIL)? 0 : 1, (ib_ret == TEST_FAIL)? 0 : 1, (noise_ret == TEST_FAIL)? 0 : 1);
	}
	...
	char *buf, loff_t off, size_t count)
{
	...
	struct ft8615_data *d = to_ft8615_data(ts->dev);
	...
	if (off == 0) {
		if (d->prd_delta_data == NULL) {
			...
			// Change clb switch
			ret = ft8615_switch_cal(ts->dev, 0);


			ret = ft8615_change_op_mode(ts->dev, FTS_FACTORY_MODE);



			ret = ft8615_prd_get_delta_data(ts->dev);


			ret = ft8615_change_op_mode(ts->dev, FTS_WORK_MODE);


			TOUCH_I("Show Delta Data OK !!!\n");

			ret_size = ft8615_prd_test_data(ts->dev, DELTA_SHOW, &test_result);
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (off == 0) {
		if (d->prd_rawdata_data == NULL) {
			...
			ret = ft8615_prd_get_raw_data(ts->dev);
			// Data Select to raw data
			data = 0x01;
			ret = ft8615_reg_write(ts->dev, 0x06, &data, 1);

			ret = ft8615_change_op_mode(ts->dev, FTS_WORK_MODE);

			TOUCH_I("Show Raw Data OK !!!\n");

			ret_size = ft8615_prd_test_data(ts->dev, RAW_DATA_TEST, &test_result);
...
static int ft8615_prd_attr_delta_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
...
static int ft8615_prd_attr_rawdata_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
...
int ft8615_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ft8615_prd_attr_delta_init(dev);
	ft8615_prd_attr_rawdata_init(dev);
