 ...
 */
#include "touch_ft8615.h"
#include "touch_ft8615_prd.h"
...
*/
#endif
#ifdef FT8615_ESD_SKIP_WHILE_TOUCH_ON
static int finger_cnt = 0;

bool ft8615_check_finger(void)
{
	return finger_cnt==0? false:true;
}
EXPORT_SYMBOL(ft8615_check_finger);
#endif

int ft8615_reg_read(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
...
int ft8615_reg_write(struct device *dev, u16 addr, void *data, int size)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	struct touch_bus_msg msg = {0, };
...
int ft8615_cmd_read(struct device *dev, void *cmd_data, int cmd_len, void *read_buf, int read_len)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	struct touch_bus_msg msg = {0, };
...
void ft8615_dummy_read(struct device *dev)
{
	u8 dummy = 0x00;
	...
	ft8615_reg_read(dev, 0x00, &dummy, 1);
	touch_msleep(40);

}

static int ft8615_reset_ctrl(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 data = 0;
	...
	case SW_RESET:
		TOUCH_I("%s : SW Reset\n", __func__);
		data = 0x55;
		ft8615_reg_write(dev, 0xFC, &data, 1);
		touch_msleep(10);
		data = 0x66;
		ft8615_reg_write(dev, 0xFC, &data, 1);
...
static int ft8615_power(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
//	struct ft8615_data *d = to_ft8615_data(dev);
	TOUCH_TRACE();
	...
	case POWER_HW_RESET_ASYNC:
		...
		ft8615_reset_ctrl(dev, HW_RESET);
		break;
	...
	case POWER_SW_RESET:
	    TOUCH_I("%s, SW Reset\n", __func__);
		ft8615_reset_ctrl(dev, SW_RESET);
		break;
	default:
...
static void ft8615_get_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void ft8615_get_swipe_info(struct device *dev)
{
...
static struct ft8615_ic_info *ext_ic_info;

int ft8615_ic_info(struct device *dev)
{
//	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0;
...
//	u8 rdata = 0;
	...
	for (i = 0; i < 2; i++) {
		ret |= ft8615_reg_read(dev, FTS_REG_ID, (u8 *)&chip_id, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_ID_LOW, (u8 *)&chip_id_low, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_FW_VER, (u8 *)&fw_version, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_FW_VER_MINOR, (u8 *)&fw_version_minor, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_FW_VER_SUB_MINOR, (u8 *)&fw_version_sub_minor, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_FW_VENDOR_ID, (u8 *)&fw_vendor_id, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_LIB_VER_H, (u8 *)&lib_version_high, 1);
		ret |= ft8615_reg_read(dev, FTS_REG_LIB_VER_L, (u8 *)&lib_version_low, 1);
...
int ft8615_grip_suppression_ctrl(struct device *dev, u8 x, u8 y)
{
        int ret = 0;
        u8 data = 0x0;
	...
	TOUCH_TRACE();
        ...
        ret = ft8615_reg_write(dev, CONTROL_GRIP_SUPPRESSION_X, &data, sizeof(data));
        if (ret < 0) {
        ...
        ret = ft8615_reg_write(dev, CONTROL_GRIP_SUPPRESSION_Y, &data, sizeof(data));
        if (ret < 0) {
...
static int ft8615_tci_control(struct device *dev, int type)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	...
	case TCI_CTRL_SET:
		...
		ret = ft8615_reg_write(dev, 0xF6, &data, 1);
		data = ts->tci.mode;
		ret = ft8615_reg_write(dev, 0xD0, &data, 1);
		break;

	case TCI_CTRL_CONFIG_COMMON:
		data = (((d->tci_debug_type) & 0x02) << 3) | ((d->tci_debug_type) & 0x01);
		ret = ft8615_reg_write(dev, 0xE5, &data, 1);	// Fail Reason Debug Function Enable

		//Resolution 720 x 1560 (6.26"inch) MH45 HD+

		data = 0x28;
		ret |= ft8615_reg_write(dev, 0x92, &data, 1);	// Active Area LSB of X1  // MH4 CPT: FT8615 0x92
		data = 0x00;
		ret |= ft8615_reg_write(dev, 0xB5, &data, 1);	// Active Area MSB of X1 (40)
		data = 0xA8;
		ret |= ft8615_reg_write(dev, 0xB6, &data, 1);	// Active Area LSB of X2
		data = 0x02;
		ret |= ft8615_reg_write(dev, 0xB7, &data, 1);	// Active Area MSB of X2 (680)
		data = 0x28;
		ret |= ft8615_reg_write(dev, 0xB8, &data, 1);	// Active Area LSB of Y1
		data = 0x00;
		ret |= ft8615_reg_write(dev, 0xB9, &data, 1);	// Active Area MSB of Y1 (40)
		data = 0xF0;
		ret |= ft8615_reg_write(dev, 0xBA, &data, 1);	// Active Area LSB of Y2
		data = 0x05;
		ret |= ft8615_reg_write(dev, 0xBB, &data, 1);	// Active Area MSB of Y2 (1520)
		break;

	case TCI_CTRL_CONFIG_TCI_1:
		data = 0x0A;
		ret = ft8615_reg_write(dev, 0xBC, &data, 1);	// Touch Slop (10mm)
		data = 0x0A;
		ret |= ft8615_reg_write(dev, 0xC4, &data, 1);	// Touch Distance (10mm)
		data = 0x46; //0x32;
		ret |= ft8615_reg_write(dev, 0xC6, &data, 1);	// Time Gap Max (700ms)
		data = 0x02;
		ret |= ft8615_reg_write(dev, 0xCA, &data, 1);	// Total Tap Count (2)
		data = info1->intr_delay;
		ret |= ft8615_reg_write(dev, 0xCC, &data, 1);	// Interrupt Delay (700ms or 0ms)
		break;

	case TCI_CTRL_CONFIG_TCI_2:
		data = 0x0A;
		ret = ft8615_reg_write(dev, 0xBD, &data, 1);	// Touch Slop (10mm)
		data = 0xFF; //0xC8;
		ret |= ft8615_reg_write(dev, 0xC5, &data, 1);	// Touch Distance (?? 200mm ??)
		data = 0x46;
		ret |= ft8615_reg_write(dev, 0xC7, &data, 1);	// Time Gap Max (700ms)
		data = info2->tap_count;
		ret |= ft8615_reg_write(dev, 0xCB, &data, 1);	// Total Tap Count (2)
		data = 0x25;
		ret |= ft8615_reg_write(dev, 0xCD, &data, 1);	// Interrupt Delay (370ms ??)
		break;
...
static int ft8615_swipe_control(struct device *dev, u8 idx, int *swipe_mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	u8 data;
	...
	switch (idx) {
		case SWIPE_D:
			*swipe_mode |= 0x1 << 3;
			data = 0x1;
			ret |= ft8615_reg_write(dev, 0x60, &data, 1);	// Select BankSel (which swipe config: 0x00: swipe_up / 0x01: swipe_down / 0x02: swipe_left / 0x03: swipe_right)
			break;
		case SWIPE_U:
			*swipe_mode |= 0x1 << 2;
			data = 0x0;
			ret |= ft8615_reg_write(dev, 0x60, &data, 1);
			break;
		case SWIPE_L:
			*swipe_mode |= 0x1 << 4;
			data = 0x2;
	...
	ret = ft8615_reg_write(dev, 0x95, &data, 1);	// Fail Reason Debug Function Enable

	data = ts->swipe[idx].distance;
	ret |= ft8615_reg_write(dev, 0x61, &data, 1);	// Distance
	data = ts->swipe[idx].ratio_thres;
	ret |= ft8615_reg_write(dev, 0x62, &data, 1);	// Ratio
	data = (ts->swipe[idx].min_time & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x63, &data, 1);	// Min time HighBit
	data = ts->swipe[idx].min_time & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x64, &data, 1);	// Min time LowBit
	data = (ts->swipe[idx].max_time & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x65, &data, 1);	// Max time HighBit
	data = ts->swipe[idx].max_time & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x66, &data, 1);	// Max time LowBit
	data = ts->swipe[idx].wrong_dir_thres;
	ret |= ft8615_reg_write(dev, 0x67, &data, 1);	// Wrong Direction

	data = (ts->swipe[idx].area.x1 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x68, &data, 1);	// Active Area x1 HighBit
	data = ts->swipe[idx].area.x1 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x69, &data, 1);	// Active Area x1 LowBit
	data = (ts->swipe[idx].area.y1 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x6A, &data, 1);	// Active Area y1 HighBit
	data = ts->swipe[idx].area.y1 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x6B, &data, 1);	// Active Area y1 LowBit

	data = (ts->swipe[idx].area.x2 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x6C, &data, 1);	// Active Area x2 HighBit
	data = ts->swipe[idx].area.x2 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x6D, &data, 1);	// Active Area x2 LowBit
	data = (ts->swipe[idx].area.y2 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x6E, &data, 1);	// Active Area y2 HighBit
	data = ts->swipe[idx].area.y2 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x6F, &data, 1);	// Active Area y2 LowBit

	data = (ts->swipe[idx].start_area.x1 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x70, &data, 1);	// Active Start Area x1 HighBit
	data = ts->swipe[idx].start_area.x1 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x71, &data, 1);	// Active Start Area x1 LowBit
	data = (ts->swipe[idx].start_area.y1 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x72, &data, 1);	// Active Start Area y1 HighBit
	data = ts->swipe[idx].start_area.y1 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x73, &data, 1);	// Active Start Area y1 LowBit

	data = (ts->swipe[idx].start_area.x2 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x74, &data, 1);	// Active Start Area x2 HighBit
	data = ts->swipe[idx].start_area.x2 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x75, &data, 1);	// Active Start Area x2 LowBit
	data = (ts->swipe[idx].start_area.y2 & 0xFF00) >> 8;
	ret |= ft8615_reg_write(dev, 0x76, &data, 1);	// Active Start Area y2 HighBit
	data = ts->swipe[idx].start_area.y2 & 0x00FF;
	ret |= ft8615_reg_write(dev, 0x77, &data, 1);	// Active Start Area y2 LowBit

	data = ts->swipe[idx].init_ratio_chk_dist;
	ret |= ft8615_reg_write(dev, 0x78, &data, 1);	// Initial Ratio Check Distance
	data = ts->swipe[idx].init_ratio_thres;
	ret |= ft8615_reg_write(dev, 0x79, &data, 1);	// Initial Ratio Threshold

	return ret;
}

static int ft8615_lpwg_control(struct device *dev, u8 mode, bool swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct tci_info *info1 = &ts->tci.info[TCI_1];
	//struct ft8615_data *d = to_ft8615_data(dev);
	...
	case LPWG_DOUBLE_TAP:
		...
		ret = ft8615_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
		ret |= ft8615_tci_control(dev, TCI_CTRL_CONFIG_COMMON);
		ret |= ft8615_tci_control(dev, TCI_CTRL_SET);
	...
	case LPWG_PASSWORD:
		...
		ret = ft8615_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
		ret |= ft8615_tci_control(dev, TCI_CTRL_CONFIG_TCI_2);
		ret |= ft8615_tci_control(dev, TCI_CTRL_CONFIG_COMMON);
		ret |= ft8615_tci_control(dev, TCI_CTRL_SET);
...
//		ret = ft8615_tci_control(dev, TCI_CTRL_CONFIG_TCI_1);
	...
	if (swipe_enable) {
		for (i = 0; i < sizeof(ts->swipe) / sizeof(struct swipe_ctrl); i++) {
			if (ts->swipe[i].enable) {
				ret |= ft8615_swipe_control(dev, i, &swipe_mode);
			}
		}
	} else { //swipe_disable
		swipe_mode = 0;
	...
	TOUCH_I("ft8615_lpwg_control tci = %d, swipe = %d\n", ts->tci.mode, swipe_mode);
	return ret;
}


static int ft8615_deep_sleep(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u8 data;
	...
	if(mode) {
		...
		return ft8615_reg_write(dev, 0xA5, &data, 1);
	}
	else {
...
#if 0
static void ft8615_debug_tci(struct device *dev)
{
	struct ft8615_data *d = to_ft8615_data(dev);
	...
	ft8615_reg_read(dev, TCI_DEBUG_R, &rdata, sizeof(rdata));
...
int ft8615_lpwg_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_I("ft8615_lpwg_reset\n");
	touch_gpio_direction_output(ts->reset_pin, 0);
	touch_msleep(5);
	touch_gpio_direction_output(ts->reset_pin, 1);
...
static int ft8615_lpwg_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	u8 next_state;
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if ((ts->mfts_lpwg) && (d->state == TC_STATE_ACTIVE)) {
			...
			ft8615_lpwg_control(dev, LPWG_DOUBLE_TAP, false);
			goto RET;
		}
	}
	...
	if (d->state == TC_STATE_ACTIVE) {
		if (ts->lpwg.screen == 0) {
			if (ts->lpwg.sensor == PROX_NEAR) {
				...
				ret = ft8615_lpwg_control(dev, ts->lpwg.mode,
		...
		if (ts->lpwg.screen == 0) {
			...
			if(d->tci_debug_type & TCI_DEBUG_BUFFER)
				ft8615_report_tci_fr_buffer(dev); // Report fr before touch IC reset
			if(d->swipe_debug_type & TCI_DEBUG_BUFFER)
				ft8615_report_swipe_fr_buffer(dev); // Report fr before touch IC reset
...
static int ft8615_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
...
int ft8615_lpwg_set(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
...
static void ft8615_connect(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	...
	*/
	...
	if (ts->lpwg.screen != 0 ) {
		...
		ft8615_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
	}
}

#if 0
static void ft8615_lcd_event_read_reg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	u32 rdata[5] = {0};

//	ft8615_xfer_msg_ready(dev, 5);
...
void ft8615_xfer_msg_ready(struct device *dev, u8 msg_cnt)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
...
static int ft8615_usb_status(struct device *dev, u32 mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
#if 0
static int ft8615_wireless_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ft8615_earjack_status(struct device *dev, u32 onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int ft8615_debug_tool(struct device *dev, u32 value)
{
	struct touch_core_data *ts = to_touch_core(dev);

	if (value == DEBUG_TOOL_ENABLE) {
//		ts->driver->irq_handler = ft8615_sic_abt_irq_handler;
	} else {
		ts->driver->irq_handler = ft8615_irq_handler;
	}

	return 0;
}
static int ft8615_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
	...
	case DEBUG_OPTION_1:
//		if (enable)	/* ASC */
//			ft8615_asc_control(dev, ASC_ON);
//		else
//			ft8615_asc_control(dev, ASC_OFF);
		break;
	case DEBUG_OPTION_2:
...
static void ft8615_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct ft8615_data *d =
			container_of(to_delayed_work(fb_notify_work),
				struct ft8615_data, fb_notify_work);
	int ret = 0;
...
#endif
/*
static int ft8615_fb_notifier_callback(struct notifier_block *self,
...
*/
static int ft8615_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	...
	case NOTIFY_CONNECTION:
		...
		ret = ft8615_usb_status(dev, *(u32 *)data);
		break;
...
#if 0
		ret = ft8615_reg_write(dev, REG_CALL_STATE,
			(u32 *)data, sizeof(u32));
#endif
		break;
	default:
...
static void ft8615_init_locks(struct ft8615_data *d)
{
	mutex_init(&d->rw_lock);
	mutex_init(&d->fb_lock);
}

static int ft8615_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = NULL;
	...
	if (!d) {
		TOUCH_E("failed to allocate ft8615 data\n");
		return -ENOMEM;
	}
	...
	ft8615_get_tci_info(dev);
	ft8615_get_swipe_info(dev);
...
static int ft8615_remove(struct device *dev)
{
	TOUCH_TRACE();
#ifdef FTS_APK_DEBUG
...
static int ft8615_fwboot_upgrade(struct device *dev, const struct firmware *fw_boot)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	for (i = 12; i <= 30; i++) {
		...
		ret = ft8615_reg_write(dev, 0x55, i2c_buf, 0);
		if(ret < 0) {
		...
		// Read FT8615 ID
		ret = ft8615_reg_read(dev, 0x90, i2c_buf, 2);
		if(ret < 0) {
		...
		TOUCH_I("Check ROM Boot ID : 0x%x , 0x%x\n", i2c_buf[0], i2c_buf[1]);

		if(i2c_buf[0] == 0x87 && i2c_buf[1] == 0x19){   // FT8615  ROM Boot ID : 0x87 0x19
			touch_msleep(5);
			break;
		}
	}
	...
	for (i = 0; i < packet_num; i++) {
		...
		for (j = 0; j < packet_len; j++) {
			i2c_buf[5 + j] = fw_data[packet_addr + j];  // FT8615
			pramboot_ecc ^= i2c_buf[5 + j];
		}
		TOUCH_I("#%d : Writing to %d , %d bytes\n", i, packet_addr, packet_len); //kjh
		ret = ft8615_reg_write(dev, 0xAE, i2c_buf, packet_len + 5);
		if(ret < 0) {
	...
	for (i = 0; i < packet_num; i++) {
		...
		ret = ft8615_cmd_read(dev, i2c_buf, 4, fw_check_buf+packet_addr, packet_len);
		if(ret < 0) {
	...
	// Run PramBoot
	TOUCH_I("%s - Start App\n", __func__);
	ret = ft8615_reg_write(dev, 0x08, i2c_buf, 0);
	if(ret < 0) {
...
static int ft8615_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	...
	bin_fw_version = fw->data[0x210E]; // FT8615 FW version offset is 0x210E
...
static int ft8615_fw_upgrade(struct device *dev, const struct firmware *fw)
{
//	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = ft8615_reg_write(dev, 0x55, i2c_buf, 0);  //Enter upgrade mode of ParamBoot
		if(ret < 0) {
		...
		TOUCH_I("Check Pram Boot ID [%d] : 0x%x , 0x%x\n", i, i2c_buf[0], i2c_buf[1]);   // FT8615 Pram Boot ID: 0x87, 0xA9
		if(i2c_buf[0] == 0x87 && i2c_buf[1] == 0xA9)
			break;
...
//----- Change to write flash mode -----
	...
	ret = ft8615_reg_write(dev, 0x05, i2c_buf, 2);
	if(ret < 0) {
	...
	i2c_buf[0] = 0x0A;
	ret = ft8615_reg_write(dev, 0x09, i2c_buf, 1);
	...
	ret = ft8615_reg_write(dev, 0x61, i2c_buf, 0);
	if(ret < 0) {
	...
	i = 0;
	do {
		ret = ft8615_reg_read(dev, 0x6A, i2c_buf, 2);
		if(ret < 0) {
	...
	ret = ft8615_reg_write(dev, 0x7A, i2c_buf, 3);
	...
	packet_num = (fw_size + FTS_PACKET_LENGTH - 1) / FTS_PACKET_LENGTH;
	for (i = 0; i < packet_num; i++) {
		packet_addr = i * FTS_PACKET_LENGTH;//FT8615 all.bin offset is 0
		...
		ret = ft8615_reg_write(dev, 0xBF, i2c_buf, packet_len + 5);
		if(ret < 0) {
	...
	ret = ft8615_reg_write(dev, 0x64, i2c_buf, 0);
	if(ret < 0) {
	...
	for (i = 0; i < packet_num; i++) {
		packet_addr = i * LEN_FLASH_ECC_MAX;//FT8615 all.bin offset is 0
		i2c_buf[0] = (u8)(((u32)(packet_addr) & 0x00FF0000) >> 16);
		i2c_buf[1] = (u8)(((u32)(packet_addr) & 0x0000FF00) >> 8);
		i2c_buf[2] = (u8)((u32)(packet_addr) & 0x000000FF);
	...
	ret = ft8615_reg_write(dev, 0x65, i2c_buf, 5);
		if(ret < 0) {
	...
	ret = ft8615_reg_read(dev, 0x66, i2c_buf, 1);
	if(ret < 0) {
	...
	//Exit download mode
	ret = ft8615_reg_write(dev, 0x07, i2c_buf, 0);
	if(ret < 0) {
...
static int ft8615_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
//	int i = 0;
	...
	if (ts->test_fwpath[0] && (ts->test_fwpath[0] != 't' && ts->test_fwpath[1] != 0)) {
		...
		if (ft8615_panel_type == LCE_FT8615) {
	...
	if (ft8615_fw_compare(dev, fw)) {

//		ret = -EINVAL;
//		touch_msleep(200);
//		for (i = 0; i < 2 && ret; i++)
//			ret = ft8615_fw_upgrade(dev, fw);
		...
		ret = ft8615_fwboot_upgrade(dev, fw_boot);
		if(ret < 0) {
...
static int ft8615_suspend(struct device *dev)
{
//	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	//int mfts_mode = 0;
...
static int ft8615_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int mfts_mode = 0;
	...
	if ((mfts_mode >= TOUCH_MINIOS_MFTS_FOLDER && mfts_mode <= TOUCH_MINIOS_MFTS_CURVED) && !ts->mfts_lpwg) {
		#if 0
		ft8615_power(dev, POWER_ON);
		touch_msleep(ts->caps.hw_reset_delay);
		ft8615_ic_info(dev);
		if (ft8615_upgrade(dev) == 0) {
			ft8615_power(dev, POWER_OFF);
			ft8615_power(dev, POWER_ON);
			touch_msleep(ts->caps.hw_reset_delay);
		}
...
static int ft8615_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
//	u32 data = 1;
...
/*
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		TOUCH_I("ft8615 fb_notif register\n");
		fb_unregister_client(&ts->fb_notif);
		ts->fb_notif.notifier_call = ft8615_fb_notifier_callback;
...
//		ft8615_sic_abt_init(dev);

	ret = ft8615_ic_info(dev);
	if (ret < 0) {
...
#if 0
	ret = ft8615_reg_write(dev, tc_device_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_device_ctrl\', ret:%d\n", ret);

	ret = ft8615_reg_write(dev, tc_interrupt_ctl, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'tc_interrupt_ctrl\', ret:%d\n", ret);
#endif
	TOUCH_I("%s: charger_state = 0x%02X\n", __func__, d->charger);
	ret = ft8615_reg_write(dev, SPR_CHARGER_STS, &d->charger, sizeof(u8));
	if (ret)
		TOUCH_E("failed to write \'spr_charger_sts\', ret:%d\n", ret);

	ret = ft8615_reg_write(dev, SPR_QUICKCOVER_STS, &ts->lpwg.qcover, sizeof(u8));
	if (ret)
		TOUCH_E("failed to write SPR_QUICKCOVER_STS, ret:%d\n", ret);
#if 0
	data = atomic_read(&ts->state.ime);
	TOUCH_I("%s: ime_state = %d\n", __func__, data);
	ret = ft8615_reg_write(dev, REG_IME_STATE, &data, sizeof(data));
	if (ret)
		TOUCH_E("failed to write \'reg_ime_state\', ret:%d\n", ret);
#endif

	/* TODO Control grip suppresion value */
	ft8615_grip_suppression_ctrl(dev, 15, 40);
...
int ft8615_check_status(struct device *dev)
{
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0;
...
#if 0
int ft8615_debug_info(struct device *dev, int mode)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	struct ft8615_touch_debug *debug = &d->info.debug;
	int ret = 0;
	int i = 0;
...
static int ft8615_irq_abs_data(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	struct ft8615_touch_data *data = d->info.data;
...
int ft8615_irq_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	struct ft8615_touch_data *data = d->info.data;
	...
	ret = ft8615_reg_read(dev, 0x02, point_buf+2, POINT_READ_BUF-2);
	...
	return ft8615_irq_abs_data(dev);
}

int ft8615_irq_lpwg(struct device *dev, int tci)
{
	struct touch_core_data *ts = to_touch_core(dev);
	//struct ft8615_data *d = to_ft8615_data(dev);
	...
	ret = ft8615_reg_read(dev, 0xD3, tci_data_buf, 2);
	if (ret < 0) {
	...
	ret = ft8615_reg_read(dev, 0xD3, tci_data_buf, tap_count*4 + 2);
	if (ret < 0) {
		TOUCH_E("Fail to read tci data\n");
		return ret;
	}
...
int ft8615_get_swipe_data(struct device *dev, int swipe) {
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	...
	ret = ft8615_reg_read(dev, 0x02, swipe_data_buf, sizeof(swipe_data_buf));
	if (ret < 0) {
...
int ft8615_irq_report_tci_fr(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0;
	...
	ret = ft8615_reg_read(dev, 0xE6, &data, 1);
	if (ret < 0) {
...
int ft8615_irq_report_swipe_fr(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0;
	...
	ret = ft8615_reg_read(dev, 0x96, &data, 1);
	if (ret < 0) {
...
int ft8615_report_tci_fr_buffer(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0, i;
	...
	ft8615_dummy_read(dev);

	// Knock-on
	for (i = 0; i < 25; i++) {
		ret = ft8615_reg_read(dev, 0xE7, &tci_fr_buffer, sizeof(tci_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
	...
	for (i = 0; i < 25; i++) {
		ret = ft8615_reg_read(dev, 0xE9, &tci_fr_buffer, sizeof(tci_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
...
int ft8615_report_swipe_fr_buffer(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct ft8615_data *d = to_ft8615_data(dev);
	int ret = 0, i;
	...
	// Knock-on
	for (i = 0; i < 25; i++) {
		ret = ft8615_reg_read(dev, 0x93, &swipe_fr_buffer, sizeof(swipe_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
...
int ft8615_irq_handler(struct device *dev)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = ft8615_reg_read(dev, 0x01, &int_status, 1);
	if (ret < 0)
		return ret;

	switch (int_status) {
		case 0x01: //Finger
			ret = ft8615_irq_abs(dev);
			break;
		case 0x02: //TCI_1
			ts->intr_status = TOUCH_IRQ_KNOCK;
			ret = ft8615_irq_lpwg(dev, TCI_1);
			break;
#ifdef KNOCK_CODE
		case 0x03: //TCI_2
			ts->intr_status = TOUCH_IRQ_PASSWD;
			ret = ft8615_irq_lpwg(dev, TCI_2);
			break;
#endif
		case 0x04: //LPWG Fail Reason Report(RealTime)
			ret = ft8615_irq_report_tci_fr(dev);
			break;
		case 0x05: //ESD
		...
		case 0x06: // Swipe Up
			...
			ret = ft8615_get_swipe_data(dev, SWIPE_U);
			break;
		case 0x07: // Swipe Down
			ts->intr_status = TOUCH_IRQ_SWIPE_DOWN;
			ret = ft8615_get_swipe_data(dev, SWIPE_D);
			break;
		case 0x08: // Swipe Left
			ts->intr_status = TOUCH_IRQ_SWIPE_UP;
			ret = ft8615_get_swipe_data(dev, SWIPE_L);
			break;
		case 0x09: // Swipe Right
			ts->intr_status = TOUCH_IRQ_SWIPE_UP;
			ret = ft8615_get_swipe_data(dev, SWIPE_R);
			break;
		case 0x0A: // Swipe Fail Reason Report (RealTime)
			ret = ft8615_irq_report_swipe_fr(dev);
			break;
		case 0x00:
				...
				const char *buf, size_t count)
{
	...
	if (!strcmp(command, "write")) {
		data = (u8)value;
		if (ft8615_reg_write(dev, reg_addr, &data, sizeof(u8)) < 0)
			TOUCH_E("reg addr 0x%x write fail\n", reg_addr);
		else
			TOUCH_I("reg[%x] = 0x%x\n", reg_addr, data);
	} else if (!strcmp(command, "read")) {
		if (ft8615_reg_read(dev, reg_addr, &data, sizeof(u8)) < 0)
...
static struct attribute *ft8615_attribute_list[] = {
...
static const struct attribute_group ft8615_attribute_group = {
	.attrs = ft8615_attribute_list,
};

static int ft8615_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	TOUCH_TRACE();

	ret = sysfs_create_group(&ts->kobj, &ft8615_attribute_group);
	if (ret < 0)
		TOUCH_E("ft8615 sysfs register failed\n");

//	ft8615_watch_register_sysfs(dev);
	ft8615_prd_register_sysfs(dev);
//	ft8615_asc_register_sysfs(dev);	/* ASC */
//	ft8615_sic_abt_register_sysfs(&ts->kobj);

	return 0;
}

static int ft8615_get_cmd_version(struct device *dev, char *buf)
{
	struct ft8615_data *d = to_ft8615_data(dev);
	int offset = 0;
	...
	else if ((d->ic_info.chip_id == 0x87)&&(d->ic_info.chip_id_low == 0x19)){

		offset += snprintf(buf + offset, PAGE_SIZE - offset, "Product-id : [FT8615p]\n\n");
...
static int ft8615_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct ft8615_data *d = to_ft8615_data(dev);
	int offset = 0;
...
static int ft8615_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
...
static int ft8615_swipe_enable(struct device *dev, bool enable)
{
	//struct touch_core_data *ts = to_touch_core(dev);
...
static int ft8615_init_pm(struct device *dev)
{
#if 0
...
static int ft8615_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int ft8615_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = ft8615_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = ft8615_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
static int ft8615_shutdown(struct device *dev)
{
	TOUCH_TRACE();
...
static struct touch_driver touch_driver = {
	.probe = ft8615_probe,
	.remove = ft8615_remove,
	.suspend = ft8615_suspend,
	.shutdown = ft8615_shutdown,
	.resume = ft8615_resume,
	.init = ft8615_init,
	.irq_handler = ft8615_irq_handler,
	.power = ft8615_power,
	.upgrade = ft8615_upgrade,
	.esd_recovery = ft8615_esd_recovery,
	.lpwg = ft8615_lpwg,
	.swipe_enable = ft8615_swipe_enable,
	.notify = ft8615_notify,
	.init_pm = ft8615_init_pm,
	.register_sysfs = ft8615_register_sysfs,
	.set = ft8615_set,
	.get = ft8615_get,
};

#define MATCH_NAME			"focaltech,ft8615"
...
{
	TOUCH_I("FT8615__[%s] touch_bus_device_init\n", __func__);

	if(is_lcm_name("AUO-FT8615")) //DH35
	{
		TOUCH_I("%s, FT8615 found!!lcm_name = %s\n",__func__,lge_get_lcm_name());
		return touch_bus_device_init(&hwif, &touch_driver);
	}

	TOUCH_I("%s, FT8615 not found!!\n",__func__);
