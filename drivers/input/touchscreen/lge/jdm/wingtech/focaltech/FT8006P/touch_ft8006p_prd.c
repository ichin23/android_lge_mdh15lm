...
//	u8 frame_count;
	...
	if (d->ic_info.chip_id_low == 0x06) { /* FT8006M */
		data = 0x00;
	} else if (d->ic_info.chip_id_low == 0x22) { /* FT8006P */
		data = 0x01;
	}
...
int ft8006p_prd_test_data(struct device *dev, int test_type, int* test_result)
{
	...
	*test_result = TEST_FAIL;
	...
	for (i = 0; i < MAX_ROW; i++) {
		...
		if(d->ic_info.chip_id_low == 0x22) /* 8006P(Modify due to reverse from top to bottom) */
			k = MAX_ROW-i-1;
		else /* 8006M */
			k = i;
		for (j = 0; j < MAX_COL; j++) {
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	ret_size = ft8006p_prd_test_data(dev, RAW_DATA_TEST, &rawdata_ret);
	...
	ret_size = ft8006p_prd_test_data(dev, IB_DATA_TEST, &ib_ret);
	...
	ret_size = ft8006p_prd_test_data(dev, NOISE_TEST, &noise_ret);
...
static ssize_t show_lpwg_sd(struct device *dev, char *buf)
{
	...
	ret_size = ft8006p_prd_test_data(dev, LPWG_RAW_DATA_TEST, &rawdata_ret);
	...
	ret_size = ft8006p_prd_test_data(dev, LPWG_IB_DATA_TEST, &ib_ret);
	...
	ret_size = ft8006p_prd_test_data(dev, LPWG_NOISE_TEST, &noise_ret);
