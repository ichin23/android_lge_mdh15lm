...
static int write_test_mode(struct device *dev, u32 type)
{
	...
	case SHORT_NODE_TEST:
		...
		waiting_time = 1000;
		break;
	case SHORT_CH_NODE_TEST:
	case SHORT_MUX_NODE_TEST:
		type_temp = 0xC;
		testmode = (disp_mode << 8) + type_temp;
		waiting_time = 1000;
		break;
	...
	/* TestType Set */
	sw49110_reg_write(dev, TC_PT_TEST_CTL,
			(u32 *)&testmode,
			sizeof(testmode));
	...
	do {
		touch_msleep(50);
		sw49110_reg_read(dev, PT_TEST_STS, (u8 *)&rdata, sizeof(rdata));
		TOUCH_I("rdata = 0x%x\n", rdata);
...
static int prd_read_print_frame(struct device *dev, int offset, int row, int col)
{
	...
	sw49110_write_value(dev, SERIAL_DATA_OFFSET, offset);

	sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (u16 *)data_buf, row*col*sizeof(u16));
	...
	for (i = 0; i < row; i++) {
		...
		for (j = 0; j < col; j++) {
			if (col == 2) {      // U0 M1 Test Swap (COL : 2)
				ret += snprintf(w_buf + ret, PAGE_SIZE - ret, "%5d", data_buf[j * row + i]);
				log_ret += snprintf(log_buf + log_ret, sizeof(log_buf) - log_ret, "%5d", data_buf[j * row + i]);
			} else {
...
static void prd_read_print_data(struct device *dev, u8 type)
{
	...
	case SHORT_NODE_TEST:
		...
		break;
	case SHORT_CH_NODE_TEST:
		prd_short_result_get(dev, PT_FRAME_2);
		break;
	case SHORT_MUX_NODE_TEST:
		prd_short_result_get(dev, PT_FRAME_1);
		break;
	case U3_M1_RAWDATA_TEST:
...
static int prd_compare_data(struct device *dev, u8 type, int *result)
{
	...
	case SHORT_CH_NODE_TEST:
	case SHORT_MUX_NODE_TEST:
		snprintf(lower_str, sizeof(lower_str), "SHORT_M2_Lower");
		snprintf(upper_str, sizeof(upper_str), "SHORT_M2_Upper");
		break;
	case U3_M1_RAWDATA_TEST:
	...
	for (i = 0; i < ROW_SIZE; i++) {
		for (j = 0; j < col_size; j++) {
			if(type == U0_M1_RAWDATA_TEST || type == U0_M1_JITTER_TEST)	// U0 M1 Test Swap
				idx = j*ROW_SIZE + i;
			else
				idx = i*col_size + j;
			...
			if (((i==0) && (j==8)) ||
				((i==0) && (j==9))) {	// Skip compare processing
					//TOUCH_I("Skip compare processing. Unused node(Notch) [%d][%d]\n", i, j);
					/* Unused Node(Notch) [0][8], [0][9] */
			} else {	// Compare with spec data
				if ((data_buf[idx] < LowerImage[i][j]) ||
					(data_buf[idx] > UpperImage[i][j])) {
...
#else
					*result = 1;
					TOUCH_I("F [%d][%d] = %d , %d, %d\n", i, j, data_buf[idx], LowerImage[i][j], UpperImage[i][j]);
...
static int prd_openshort_test(struct device *dev)
{
	...
	/* 2. short_ch_node_test */
	TOUCH_I("\n[SHORT_CH_NODE_TEST]\n");
	write_file(dev, "\n[SHORT_CH_NODE_TEST]", TIME_INFO_SKIP);

	type = SHORT_CH_NODE_TEST;
	write_test_mode_result = write_test_mode(dev, type);
	if (write_test_mode_result == 0) {
		TOUCH_E("write_test_mode fail\n");
		return 0x3;
	...
	/* 3. short_mux_node_test */
	TOUCH_I("\n[SHORT_MUX_NODE_TEST]\n");
	write_file(dev, "\n[SHORT_MUX_NODE_TEST]", TIME_INFO_SKIP);

	type = SHORT_MUX_NODE_TEST;
	write_test_mode_result = write_test_mode(dev, type);
	if (write_test_mode_result == 0) {
		TOUCH_E("write_test_mode fail\n");
		return 0x3;
	...
	sw49110_reg_read(dev, PT_TEST_PF_RESULT, (u8 *)&open_result, sizeof(open_result));
	...
	sw49110_reg_read(dev, PT_TEST_PF_RESULT, (u8 *)&short_result, sizeof(short_result));
...
static void read_tune_code(struct device *dev, u8 type)
{
	...
	sw49110_reg_read(dev, tune_code_addr, &t,
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	*/
	...
	sw49110_write_value(dev, TC_PT_TEST_CTL, pt_command);

	touch_interrupt_control(ts->dev, INTERRUPT_DISABLE);
	sw49110_tc_driving(dev, LCD_MODE_STOP);

	/*
	 * OPEN_SHORT_ALL_TEST
	 * open - pass : 0, fail : 0x1
	 * short - pass : 0, fail : 0x2, 0x4
	 */
	...
	if (openshort_ret == 0) {
		...
		ret += snprintf(buf + ret, PAGE_SIZE - ret,
			"Channel Status : Fail (open:%d/ch_short:%d/mux_short:%d)\n",
			((openshort_ret & 0x1) == 0x1) ? 1 : 0,
			((openshort_ret & 0x2) == 0x2) ? 1 : 0,
			((openshort_ret & 0x4) == 0x4) ? 1 : 0);
	}
...
static int start_firmware(struct device *dev)
{
	...
	ret = sw49110_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, (void *)&cmd,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
	ret = sw49110_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	if (ret < 0) {
		goto error;
	}
...
static int stop_firmware(struct device *dev, u32 wdata)
{
	...
	/* STOP F/W to check */
	sw49110_reg_write(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &wdata, sizeof(u32));
	sw49110_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	sw49110_reg_read(dev, ADDR_CMD_REG_SIC_IMAGECTRL_TYPE, &check_data,
			sizeof(u32));
	...
	do
	{
		...
		sw49110_reg_read(dev, ADDR_CMD_REG_SIC_GETTER_READYSTATUS,
				&read_val, sizeof(u32));
					...
					u8 *buf, int size, u32 cmd, u32 offset, int flag)
{
	...
	ret = sw49110_reg_write(dev, SERIAL_DATA_OFFSET, (u8 *)&offset, sizeof(offset));
	if (ret < 0) {
		goto out;
	}

	ret = sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (void *)buf, size);
	if (ret < 0) {
		goto out;
	}
...
static ssize_t get_data(struct device *dev, int16_t *buf, u32 wdata)
{
	...
	case CMD_RAWDATA:
		...
		sw49110_reg_write(dev, SERIAL_DATA_OFFSET,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (int16_t *)buf, sizeof(int16_t)*ROW_SIZE*COL_SIZE);
		break;
	case CMD_BASE_DATA:
	...
	case CMD_DELTADATA:
		...
		sw49110_reg_write(dev, SERIAL_DATA_OFFSET,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (s16 *)delta_buf,
	...
	case CMD_LABELDATA:
		...
		sw49110_reg_write(dev, SERIAL_DATA_OFFSET,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (s8 *)label_buf,
	...
	case CMD_DEBUGDATA:
		...
		sw49110_reg_write(dev, SERIAL_DATA_OFFSET,
							(u32 *)&m2_data_offset, sizeof(m2_data_offset));
		sw49110_reg_read(dev, SERIAL_DATA_ACCESS, (int16_t *)buf,
...
#define DH50_MAX_BUF 6400
char kobj_log_buf[DH50_MAX_BUF] = {0,};
	...
	char *buf, loff_t off, size_t count)
{
	...
	if (off == 0) {
		...
		kobj_log_buf[DH50_MAX_BUF-1] = '\n';
	}

	if (off + count > DH50_MAX_BUF) {
...
static TOUCH_BIN_ATTR(delta, read_delta, NULL, DH50_MAX_BUF);
static TOUCH_BIN_ATTR(rawdata, read_rawdata, NULL, DH50_MAX_BUF);
static TOUCH_BIN_ATTR(base, read_base, NULL, DH50_MAX_BUF);
static TOUCH_BIN_ATTR(debug, read_debug, NULL, DH50_MAX_BUF);
static TOUCH_BIN_ATTR(label, read_labeldata, NULL, DH50_MAX_BUF);
...
int sw49110_prd_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct sw49110_data *d = to_sw49110_data(dev);
	struct siw_hal_prd_data *prd = (struct siw_hal_prd_data *)d->prd;
