...
static int parser_get_ini_key_value(char *section, char *key, char *value)
{
	...
	for (i = 0; i < g_ini_items; i++) {
		...
		if (ipio_strcmp(key, ini_info[i].key_name) == 0) {
			...
			TOUCH_D(BASE_INFO, "(key: %s, value:%s) => (ini key: %s, val: %s)\n",
					key,
					value,
					ini_info[i].key_name,
	...
	while (true) {
		...
		if (n > 2 && ((ini_buf[0] == M_CFG_SSL && ini_buf[n - 1] != M_CFG_SSR))) {
			...
			if (ini_buf[0] == M_CFG_SSL) {
				...
				TOUCH_D(BASE_INFO, "Section Name: %s, Len: %d, offset = %d\n", seq_item[scount], n - 2, offset);
				scount++;
				continue;
			}
		}
...
{
	return ((dac * 10000 * 131 / 100) - (16384 / 2 - raw) * 20000 * 7 / 16384 * 36 / 10) / (tvch - tvcl) / 2;
}
...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	TOUCH_D(BASE_INFO, "Read X/Y Channel length = %d, mode = %d\n", len, mode);
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(cmd, 8, sizeof(cmd), 0, "Open SP command");

	/* NOTE: If TP driver is doing MP test and commanding 0xF1 to FW, we add a checksum
	 * to the last index and plus 1 with size. */
	check_sum = ili9881x_calc_data_checksum(&cmd[0], cdc_len - 1);
	cmd[cdc_len - 1] = check_sum;
	header = cmd[0];
	ret = ili9881x_reg_write(core_mp->dev, header, &cmd[1], cdc_len - 1);
	if (ret < 0) {
	...
	if (core_mp->busy_cdc == POLL_CHECK)
		ret = ili9881x_check_cdc_busy(core_mp->dev, 50, 50);
	else
		ret = ili9881x_check_int_status(core_mp->dev, false);
	...
	if (core_mp->core_ver < CORE_VER_1420) {
		...
		if (ili9881x_reg_write(core_mp->dev, CMD_READ_DATA_CTRL, &cmd[1], 1)) {
		...
		if (ili9881x_reg_write(core_mp->dev, CMD_GET_CDC_DATA, cmd, 0)) {
	...
	if (ili9881x_reg_read(core_mp->dev, CMD_NONE, ori, len)) {
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(ori, 8, len, 0, "Open SP CDC original");
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(buf, 10, core_mp->frame_len,  core_mp->xch_len, "Open SP CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int allnode_mutual_cdc_data(int index)
{
	...
	TOUCH_D(BASE_INFO, "Read X/Y Channel length = %d\n", len);
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(cmd, 8, cdc_len, 0, "Mutual CDC command");
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(ori, 8, len, 0, "Mutual CDC original");
	...
	if (MP_DBG_MSG)
		ili9881x_dump_packet(frame_buf, 32, core_mp->frame_len, core_mp->xch_len, "Mutual CDC combined");
out:
	ipio_kfree((void **)&ori);
	return ret;
}
...
static int create_mp_test_frame_buffer(int index, int frame_count)
{
	...
	TOUCH_D(BASE_INFO, "Create MP frame buffers (index = %d), count = %d\n",
			index, frame_count);
...
static int mutual_test(int index)
{
	...
	TOUCH_D(BASE_INFO, "index = %d, desp = %s, Frame Count = %d\n",
...
static int open_sp_test(int index)
{
	...
	TOUCH_D(BASE_INFO, "open_sp_test: frame_cont %d, AA %d, Border %d, Notch %d, full_open_rate %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG) {
			ili9881x_dump_packet(open[i].cbk_700, 10, core_mp->frame_len, core_mp->xch_len, "cbk 700");
			ili9881x_dump_packet(open[i].cbk_250, 10, core_mp->frame_len, core_mp->xch_len, "cbk 250");
			ili9881x_dump_packet(open[i].cbk_200, 10, core_mp->frame_len, core_mp->xch_len, "cbk 200");
		}
		...
		if (MP_DBG_MSG) {
			ili9881x_dump_packet(open[i].charg_rate, 10, core_mp->frame_len, core_mp->xch_len, "origin charge rate");
			ili9881x_dump_packet(open[i].full_Open, 10, core_mp->frame_len, core_mp->xch_len, "origin full open");
		}
		...
		if (MP_DBG_MSG)
			ili9881x_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after full_open_rate_compare");
		...
		if (MP_DBG_MSG)
			ili9881x_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after compare charge rate");
...
static int open_c_test(int index)
{
	...
	TOUCH_D(BASE_INFO, "open_test_c: frame_cont = %d, gain = %d, tvch = %d, tvcl = %d\n",
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG)
			ili9881x_dump_packet(open[i].dcl_cap, 10, core_mp->frame_len, core_mp->xch_len, "DCL_Cap");
...
static void mp_test_run(int index)
{
	...
	TOUCH_D(BASE_INFO, "%s: run = %d, max = %d, min = %d, frame_count = %d\n", tItems[i].desp,
		tItems[i].run, tItems[i].max, tItems[i].min, tItems[i].frame_count);

	TOUCH_D(BASE_INFO, "v_tdf_1 = %d, v_tdf_2 = %d, h_tdf_1 = %d, h_tdf_2 = %d", tItems[i].v_tdf_1,
...
static int mp_write_ret_to_terminal(char *buf, bool lcm_on)
{
	...
	if (lcm_on) {
		if (rawdata_ret == MP_DATA_PASS && jitter_ret == MP_DATA_PASS) {
			...
			len += snprintf(buf + len, PAGE_SIZE - len, "Raw Data : Fail (raw:%d/jitter:%d)\n",
...
int ili9881x_int_action_ctrl(struct device *dev, bool keep_status)
{
	int ret = 0;
	u8 data[2] = {0};
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	/* Switch to test mode */
	if (ili9881x_switch_fw_mode(dev, FIRMWARE_TEST_MODE)) {
		TOUCH_E("Failed to switch test mode\n");
		goto out;
	}

	if (d->int_action)
		ili9881x_int_action_ctrl(dev, true);
...
out:
	...
	if (d->int_action)
		ili9881x_int_action_ctrl(dev, false);

	/* Reset IC to get back to demo mode.  */
	ili9881x_reset_ctrl(dev, HW_RESET_SYNC);
	/* Switch to demo mode */
	//if (ili9881x_switch_fw_mode(dev, FIRMWARE_DEMO_MODE))
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct ili9881x_data *d = to_ili9881x_data(dev);
	struct ili9881x_debug_info *debug_info = &d->debug_info;
	struct ili9881x_panel_info *panel_info = &d->tp_info.panel_info;
	int ret = 0, try = 10;
	int log_ret = 0;
...
retry:
	...
	ret = ili9881x_reg_write(dev, CMD_READ_DATA_CTRL, &tmp, 1);

	tmp = 0x01; //Set signal once data mode
	ret = ili9881x_reg_write(dev, 0xB7, &tmp, 1);
	if (ret) {
	...
	ret = ili9881x_reg_read(dev, CMD_NONE, debug_info->data, read_length);
	...
	//Compare checksum
	check_sum = ili9881x_calc_data_checksum(debug_info->data, (read_length - 1));
...
retry:
	...
	if (debug_info->data[0] != 0xB7 && try != 0) {
		...
		ili9881x_dump_packet(debug_info->data, 10, read_length, col, "Header");
		try--;
		goto retry;
	}
	...
	if (check_sum != debug_info->data[read_length -1]) {
		TOUCH_E("Packet check sum Error, check_sum = %d, abs_data = %d\n",
				check_sum, debug_info->data[read_length -1]);
		ili9881x_dump_packet(debug_info->data, 10, read_length, col, "raw");
		try--;
		goto retry;
	}
...
static ssize_t show_noise(struct device *dev, char *buf)
{
	...
	ret = ili9881x_reg_write(dev, CMD_READ_DATA_CTRL, &cmd, 1);
...
retry:
		...
		ret = ili9881x_reg_write(dev, 0xB7, &cmd, 1);
		if (ret) {
			TOUCH_E("Failed to write preparation command, %d\n", ret);
			goto out;
		}
		...
		for (j = 4, index = 0; index < row * col; j += 2, index++) {
			...
			p2p[index] = rawtemp;
			ili9881x_dump_packet(p2p, 16, row * col, col, "p2p raw");
		}
	}

	for (index = 0; index < row * col; index++) {
		p2p[index] = max[index] - min[index];
		ili9881x_dump_packet(max, 16, row * col, col, "max raw");
		ili9881x_dump_packet(min, 16, row * col, col, "min raw");
		ili9881x_dump_packet(p2p, 16, row * col, col, "p2p raw");
	}
...
retry:
		...
		ili9881x_dump_packet(rawdata, 16, row * col, col, "jitter raw");
	}
...
int ili9881x_mp_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
