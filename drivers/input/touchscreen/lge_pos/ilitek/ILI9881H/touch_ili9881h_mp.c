...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	check_sum = ili9881h_calc_data_checksum(&cmd[0], cdc_len - 1);
	cmd[cdc_len - 1] = check_sum;
	header = cmd[0];
	ret = ili9881h_reg_write(core_mp->dev, header, &cmd[1], cdc_len - 1);
	if (ret < 0) {
	...
	if (core_mp->busy_cdc == POLL_CHECK)
		ret = ili9881h_check_cdc_busy(core_mp->dev, 50, 50);
	else
		ret = ili9881h_check_int_status(core_mp->dev, false);
	...
	if (core_mp->core_ver < CORE_VER_1420) {
		...
		if (ili9881h_reg_write(core_mp->dev, CMD_READ_DATA_CTRL, &cmd[1], 1)) {
		...
		if (ili9881h_reg_write(core_mp->dev, CMD_GET_CDC_DATA, cmd, 0)) {
	...
	if (ili9881h_reg_read(core_mp->dev, CMD_NONE, ori, len)) {
...
static int allnode_mutual_cdc_data(int index)
{
	...
	if (MP_DBG_MSG)
		ili9881h_dump_packet(cmd, 8, cdc_len, 0, "Mutual CDC command");
...
static int open_sp_test(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG)
			ili9881h_dump_packet(&tItems[index].buf[(i * core_mp->frame_len)], 10, core_mp->frame_len, core_mp->xch_len, "after full_open_rate_compare");
...
static int open_c_test(int index)
{
	...
	for (i = 0; i < tItems[index].frame_count; i++) {
		...
		if (MP_DBG_MSG)
			ili9881h_dump_packet(open[i].dcl_cap, 10, core_mp->frame_len, core_mp->xch_len, "DCL_Cap");
...
static int mp_write_ret_to_terminal(char *buf, bool lcm_on)
{
	...
	int doze_rawdata_ret = MP_DATA_FAIL;
	int doze_jitter_ret = MP_DATA_FAIL;
	/* LCD OFF TEST ITEM */
	...
	for (seq = 0; seq < core_mp->run_num; seq++) {
		...
		if (tItems[i].item_result == MP_DATA_PASS) {
			...
			if (ipio_strcmp(tItems[i].desp, "doze raw data") == 0)
				doze_rawdata_ret = MP_DATA_PASS;
			if (ipio_strcmp(tItems[i].desp, "doze peak to peak") == 0)
				doze_jitter_ret = MP_DATA_PASS;
	...
	if (lcm_on) {
		if (rawdata_ret == MP_DATA_PASS && jitter_ret == MP_DATA_PASS
				&& doze_rawdata_ret == MP_DATA_PASS && doze_jitter_ret == MP_DATA_PASS) {
			len += snprintf(buf + len, PAGE_SIZE - len, "Raw Data : Pass\n");
			TOUCH_I("Raw Data : Pass\n");
		} else {
			len += snprintf(buf + len, PAGE_SIZE - len, "Raw Data : Fail (raw:%d/jitter:%d/doze_raw:%d/doze_jitter:%d)\n",
					(rawdata_ret == MP_DATA_FAIL) ? 0 : 1, (jitter_ret == MP_DATA_FAIL)? 0 : 1,
					(doze_rawdata_ret == MP_DATA_FAIL) ? 0 : 1, (doze_jitter_ret == MP_DATA_FAIL)? 0 : 1);
			TOUCH_I("Raw Data : Fail (raw:%d/jitter:%d/doze_raw:%d/doze_jitter:%d)\n",
					(rawdata_ret == MP_DATA_FAIL) ? 0 : 1, (jitter_ret == MP_DATA_FAIL)? 0 : 1,
					(doze_rawdata_ret == MP_DATA_FAIL) ? 0 : 1, (doze_jitter_ret == MP_DATA_FAIL)? 0 : 1);
		}
...
int ili9881h_int_action_ctrl(struct device *dev, bool keep_status)
{
	int ret = 0;
	u8 data[2] = {0};
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if (d->int_action)
		ili9881h_int_action_ctrl(dev, true);
...
out:
	...
	if (d->int_action)
		ili9881h_int_action_ctrl(dev, false);
...
retry:
	...
	ret = ili9881h_reg_write(dev, CMD_READ_DATA_CTRL, &tmp, 1);
...
retry:
	...
	if (debug_info->data[0] != 0xB7 && try != 0) {
		...
		ili9881h_dump_packet(debug_info->data, 10, read_length, col, "Header");
		try--;
		goto retry;
	}
	...
	if (check_sum != debug_info->data[read_length -1]) {
		TOUCH_E("Packet check sum Error, check_sum = %d, abs_data = %d\n",
				check_sum, debug_info->data[read_length -1]);
		ili9881h_dump_packet(debug_info->data, 10, read_length, col, "raw");
		try--;
		goto retry;
	}
...
static ssize_t show_noise(struct device *dev, char *buf)
{
	...
	max = kcalloc(DEBUG_MODE_PACKET_LENGTH, sizeof(int16_t), GFP_KERNEL);
	if(ERR_ALLOC_MEM(max)) {
		TOUCH_E("Failed to allocate max mem\n");
		goto out;
	}

	min = kcalloc(DEBUG_MODE_PACKET_LENGTH, sizeof(int16_t), GFP_KERNEL);
	if(ERR_ALLOC_MEM(min)) {
		TOUCH_E("Failed to allocate min mem\n");
		goto out;
	}

	p2p = kcalloc(DEBUG_MODE_PACKET_LENGTH, sizeof(int16_t), GFP_KERNEL);
	if(ERR_ALLOC_MEM(p2p)) {
		TOUCH_E("Failed to allocate p2p mem\n");
		goto out;
	}

	memset(max, INT_MIN, DEBUG_MODE_PACKET_LENGTH * sizeof(int16_t));
	memset(min, INT_MAX, DEBUG_MODE_PACKET_LENGTH * sizeof(int16_t));
	memset(p2p, 0, DEBUG_MODE_PACKET_LENGTH * sizeof(int16_t));

	ret = ili9881h_reg_write(dev, CMD_READ_DATA_CTRL, &cmd, 1);
...
retry:
		...
		ret = ili9881h_reg_write(dev, 0xB7, &cmd, 1);
		if (ret) {
			TOUCH_E("Failed to write preparation command, %d\n", ret);
			goto out;
		}
		...
		for (j = 4, index = 0; index < row * col; j += 2, index++) {
			...
			p2p[index] = rawtemp;
			ili9881h_dump_packet(p2p, 16, row * col, col, "p2p raw");
		}
	}

	for (index = 0; index < row * col; index++) {
		p2p[index] = max[index] - min[index];
		ili9881h_dump_packet(max, 16, row * col, col, "max raw");
		ili9881h_dump_packet(min, 16, row * col, col, "min raw");
		ili9881h_dump_packet(p2p, 16, row * col, col, "p2p raw");
	}
...
static ssize_t show_jitter(struct device *dev, char *buf)
{
	...
	int i, x, y, j;
	int read_length = 0;
	int frame_cnt = 30, rawtemp, jittertemp;
	u8 cmd = 0x0;
	u8 check_sum = 0;
	...
	jitter = kcalloc(DEBUG_MODE_PACKET_LENGTH, sizeof(int16_t), GFP_KERNEL);
	...
	memset(jitter, 0, DEBUG_MODE_PACKET_LENGTH * sizeof(int16_t));
	memset(rawdata, 0, DEBUG_MODE_PACKET_LENGTH * sizeof(int16_t));
...
retry:
		...
		ili9881h_dump_packet(rawdata, 16, row * col, col, "jitter raw");
	}
