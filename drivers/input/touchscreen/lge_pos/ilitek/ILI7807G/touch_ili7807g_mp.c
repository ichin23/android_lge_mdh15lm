...
static int allnode_open_cdc_data(int mode, int *buf)
{
	...
	if (core_mp->busy_cdc == POLL_CHECK)
		...
		ret = ili7807g_check_int_status(core_mp->dev, false);
...
static ssize_t show_noise(struct device *dev, char *buf)
{
	...
	ret = ili7807g_reg_write(dev, CMD_READ_DATA_CTRL, &cmd, 1);
...
retry:
		...
		ret = ili7807g_reg_write(dev, 0xB7, &cmd, 1);
		if (ret) {
			TOUCH_E("Failed to write preparation command, %d\n", ret);
			goto out;
		}
		...
		for (j = 4, index = 0; index < row * col; j += 2, index++) {
			...
			p2p[index] = rawtemp;
			ili7807g_dump_packet(p2p, 16, row * col, col, "p2p raw");
		}
	}

	for (index = 0; index < row * col; index++) {
		p2p[index] = max[index] - min[index];
		ili7807g_dump_packet(max, 16, row * col, col, "max raw");
		ili7807g_dump_packet(min, 16, row * col, col, "min raw");
		ili7807g_dump_packet(p2p, 16, row * col, col, "p2p raw");
	}
...
retry:
		...
		ili7807g_dump_packet(rawdata, 16, row * col, col, "jitter raw");
	}
