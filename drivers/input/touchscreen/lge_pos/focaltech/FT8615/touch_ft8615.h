...
#ifndef LGE_TOUCH_FT8615_H
#define LGE_TOUCH_FT8615_H

#define FT8615_ESD_SKIP_WHILE_TOUCH_ON
/* focaltech APK debug */
...
struct ft8615_touch_info {
	...
	u32 button_cnt:3;
	u32 palm_bit:16;
	struct ft8615_touch_data data[10];
	/* debug info */
	struct ft8615_touch_debug debug;
} __packed;
...
#if 0
struct ft8615_version {
	u8 build : 4;
	u8 major : 4;
	u8 minor;
};

struct ft8615_ic_info {
	struct ft8615_version version;
	u8 product_id[8];
	u8 image_version[2];
...
struct ft8615_data {
	struct device *dev;
	struct kobject kobj;
	struct ft8615_touch_info info;
	struct ft8615_ic_info ic_info;
//	struct ft8615_asc_info asc;	/* ASC */
...
#ifdef CONFIG_LGD_FT8615_FHD_VIDEO_LCD_PANEL
...
static inline struct ft8615_data *to_ft8615_data(struct device *dev)
{
	return (struct ft8615_data *)touch_get_device(to_touch_core(dev));
}

static inline struct ft8615_data *to_ft8615_data_from_kobj(struct kobject *kobj)
{
	return (struct ft8615_data *)container_of(kobj,
			struct ft8615_data, kobj);
}
...
int ft8615_tc_driving(struct device *dev, int mode);
...
static inline int ft8615_read_value(struct device *dev,
					u16 addr, u32 *value)
{
	return ft8615_reg_read(dev, addr, value, sizeof(*value));
}

static inline int ft8615_write_value(struct device *dev,
					 u16 addr, u32 value)
{
	return ft8615_reg_write(dev, addr, &value, sizeof(value));
}

//void ft8615_xfer_msg_ready(struct device *dev, u8 msg_cnt);

extern int get_smartcover_status(void);
extern int ft8615_lpwg_set(struct device *dev);
#endif /* LGE_TOUCH_FT8615_H */
