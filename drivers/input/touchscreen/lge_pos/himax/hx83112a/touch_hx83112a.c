...
#include "touch_hx83112a.h"
#include "touch_hx83112a_core.h"
...
#ifdef CONFIG_LGE_TOUCH_HIMAX_HX83112A_DEBUG
extern int hx83112a_debug_init(struct device *dev);
extern int hx83112a_debug_remove(struct device *dev);
#endif
extern int hx83112a_proc_init(void);
extern void hx83112a_proc_deinit(void);
extern int hx83112a_register_sysfs(struct device *dev);
void hx83112a_rst_gpio_set(int pinnum, uint8_t value);
static int hx83112a_cable_connect(struct device *dev);
...
static int hx83112a_bus_read(struct device *dev, uint8_t command, uint8_t *data, uint32_t length)
{
...
static int hx83112a_bus_write(struct device *dev, uint8_t command, uint8_t *data, uint32_t length)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
	struct touch_bus_msg msg;
...
static void hx83112a_in_parse_assign_cmd(uint32_t addr, uint8_t *cmd, int len)
{
	/*TOUCH_I("%s: Entering!\n", __func__);*/
	switch (len) {
	case 1:
...
void hx83112a_burst_enable(struct device *dev, uint8_t auto_add_4_byte)
{
	uint8_t tmp_data[DATA_LEN_4];
	/*TOUCH_I("%s,Entering \n",__func__);*/
	tmp_data[0] = IC_CMD_CONTI;

	if (hx83112a_bus_write(dev, IC_ADR_CONTI, tmp_data, 1) < 0) {
	...
	if (hx83112a_bus_write(dev, IC_ADR_INCR4, tmp_data, 1) < 0) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return;
	}
}

int hx83112a_register_read(struct device *dev, uint32_t addr, uint32_t read_length, uint8_t *read_data, uint8_t addr_len)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	if (read_length > FLASH_RW_MAX_LEN) {
		...
		hx83112a_in_parse_assign_cmd(addr, tmp_data, addr_len);
	}

		if (read_length > DATA_LEN_4) {
			hx83112a_burst_enable(dev, 1);
		} else {
			hx83112a_burst_enable(dev, 0);
		}
	...
	tmp_data[3] = (uint8_t)(i >> 24);*/
		...
		if (hx83112a_bus_write(dev, IC_ADR_AHB_ADDR_BYTE_0, tmp_data, DATA_LEN_4) < 0) {
			TOUCH_E("%s: i2c access fail!\n", __func__);
			return I2C_FAIL;
		}
		...
		if (hx83112a_bus_write(dev, IC_ADR_AHB_ACCESS_DIRECTION, tmp_data, 1) < 0) {
			TOUCH_E("%s: i2c access fail!\n", __func__);
			return I2C_FAIL;
		}

		if (hx83112a_bus_read(dev, IC_ADR_AHB_RDATA_BYTE_0, read_data, read_length) < 0) {
			TOUCH_E("%s: i2c access fail!\n", __func__);
			return I2C_FAIL;
		}
...
static int hx83112a_flash_write_burst_lenth(struct device *dev, uint8_t *reg_byte, uint8_t *write_data, uint32_t length)
{
	uint8_t *data_byte;
	int i = 0, j = 0;
	...
	if (hx83112a_bus_write(dev, IC_ADR_AHB_ADDR_BYTE_0, data_byte, length + ADDR_LEN_4) < 0) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		kfree(data_byte);
		return I2C_FAIL;
	}
...
int hx83112a_register_write(struct device *dev, uint32_t addr, uint32_t write_length, uint8_t *write_data, uint8_t addr_len)
{
	/*int address;*/
	...
	hx83112a_in_parse_assign_cmd(addr, tmp_addr, addr_len);
	...
	if (hx83112a_flash_write_burst_lenth(dev, tmp_addr, write_data, write_length) < 0) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return I2C_FAIL;
	}
...
void hx83112a_interface_on(struct device *dev)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	if (hx83112a_bus_read(dev, IC_ADR_AHB_RDATA_BYTE_0, tmp_data, DATA_LEN_4) < 0) {/* to knock I2C*/
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return;
	}
	...
	do {
		...
		/*Check cmd*/
		hx83112a_bus_read(dev, IC_ADR_CONTI, tmp_data, 1);
		hx83112a_bus_read(dev, IC_ADR_INCR4, tmp_data2, 1);
...
static void hx83112a_hw_reset(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	TOUCH_TRACE();
...
static void hx83112a_reload_config(struct device *dev)
{
	if (hx83112a_report_data_init()) {
		TOUCH_E("%s: allocate data fail\n", __func__);
	}
	hx83112a_sense_on(dev, 0x00);
}

void hx83112a_ic_reset(struct device *dev, uint8_t loadconfig, uint8_t int_off)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	if (d->rst_gpio >= 0) {
		...
		hx83112a_hw_reset(dev);

		if (loadconfig) {
			hx83112a_reload_config(dev);
		}
...
void hx83112a_sense_on(struct device *dev, uint8_t FlashMode)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	hx83112a_interface_on(dev);
	hx83112a_register_write(dev, FW_ADDR_CTRL_FW,	DATA_LEN_4, w_data, ADDR_LEN_4);
	touch_msleep(10);

	if (!FlashMode) {
		hx83112a_ic_reset(dev, false, false);
	} else {
		do {
			for (i = 0; i < ADDR_LEN_4; i++) {
				w_data[i] = (uint8_t)(FW_DATA_SAFE_MODE_RELEASE_PW_ACTIVE >> i * 8);
			}
			hx83112a_register_write(dev, FW_ADDR_SAFE_MODE_RELEASE_PW, DATA_LEN_4, w_data, ADDR_LEN_4);

			hx83112a_register_read(dev, FW_ADDR_FLAG_RESET_EVENT, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		...
		if (retry >= 5) {
			...
			if (hx83112a_bus_write(dev, IC_ADR_I2C_PSW_LB, tmp_data, 1) < 0) {
				TOUCH_E("%s: i2c access fail!\n", __func__);
			}

			if (hx83112a_bus_write(dev, IC_ADR_I2C_PSW_UB, tmp_data, 1) < 0) {
				TOUCH_E("%s: i2c access fail!\n", __func__);
			}
...
bool hx83112a_sense_off(struct device *dev, bool check_en)
{
	uint8_t cnt = 0;
	uint8_t tmp_data[DATA_LEN_4];
	int i = 0;
	...
	do {
		if (cnt == 0 || (tmp_data[0] != 0xA5 && tmp_data[0] != 0x00 && tmp_data[0] != 0x87)) {
		 hx83112a_register_write(dev, FW_ADDR_CTRL_FW, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		}
		touch_msleep(20);

		/* check fw status */
		hx83112a_register_read(dev, IC_ADR_CS_CENTRAL_STATE, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		...
		hx83112a_register_read(dev, FW_ADDR_CTRL_FW, 4, tmp_data, ADDR_LEN_4);
	...
	do {
		...
		hx83112a_register_read(dev, IC_ADR_CS_CENTRAL_STATE, ADDR_LEN_4, tmp_data, ADDR_LEN_4);
		...
		if (tmp_data[0] == 0x0C) {
			...
			hx83112a_register_write(dev, IC_ADR_TCON_ON_RST, DATA_LEN_4, tmp_data, ADDR_LEN_4);
			touch_msleep(1);
			tmp_data[0] = 0x01;
			hx83112a_register_write(dev, IC_ADR_TCON_ON_RST, DATA_LEN_4, tmp_data, ADDR_LEN_4);
			//tmp_addr[3] = 0x80; tmp_addr[2] = 0x02; tmp_addr[1] = 0x00; tmp_addr[0] = 0x94;
			tmp_data[0] = 0x00;
			hx83112a_register_write(dev, IC_ADDR_ADC_ON_RST, DATA_LEN_4, tmp_data, ADDR_LEN_4);
			touch_msleep(1);
			tmp_data[0] = 0x01;
			hx83112a_register_write(dev, IC_ADDR_ADC_ON_RST, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
uint8_t hx83112a_int_gpio_read(int pinnum)
{
	return gpio_get_value(pinnum);
}

static int hx83112a_power(struct device *dev, int ctrl)
{
//	struct touch_core_data *ts = to_touch_core(dev);
...
static int hx83112a_fw_compare(struct device *dev, const struct firmware *fw)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
...
static void hx83112a_init_psl(struct device *dev) /*power saving level*/
{
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83112a_register_write(dev, IC_ADR_PSL, DATA_LEN_4, w_data, ADDR_LEN_4);
	TOUCH_I("%s: power saving level reset OK!\n", __func__);
}

static bool hx83112a_wait_wip(struct device *dev, int Timing)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83112a_register_write(dev, FLASH_ADDR_SPI200_TRANS_FMT, DATA_LEN_4, w_data, ADDR_LEN_4);
	tmp_data[0] = 0x01;
	...
	do {
		...
		hx83112a_register_write(dev, FLASH_ADDR_SPI200_TRANS_CTRL, DATA_LEN_4, w_data, ADDR_LEN_4);
		for (i = 0; i < ADDR_LEN_4; i++) {
			w_data[i] = (uint8_t)(FLASH_DATA_SPI200_CMD_1 >> i * 8);
		}
		hx83112a_register_write(dev, FLASH_ADDR_SPI200_CMD, DATA_LEN_4, w_data, ADDR_LEN_4);
		tmp_data[0] = tmp_data[1] = tmp_data[2] = tmp_data[3] = 0xFF;
		hx83112a_register_read(dev, FLASH_ADDR_SPI200_DATA, 4, tmp_data, ADDR_LEN_4);
...
static bool hx83112a_block_erase(struct device *dev, int start_addr, int length) /*complete not yet*/
{
	...
	for (page_prog_start = start_addr; page_prog_start < start_addr + length; page_prog_start = page_prog_start + block_size) {
		...
		hx83112a_register_write(dev, FLASH_ADDR_SPI200_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		for (i = 0; i < ADDR_LEN_4; i++) {
...
static void hx83112a_flash_programming(struct device *dev, uint8_t *FW_content, int FW_Size)
{
	...
	for (page_prog_start = 0; page_prog_start < FW_Size; page_prog_start += FLASH_RW_MAX_LEN) {
		...
		if (hx83112a_bus_write(dev, IC_ADR_AHB_ADDR_BYTE_0, buring_data, ADDR_LEN_4 + 16) < 0) {
			TOUCH_E("%s: i2c access fail!\n", __func__);
			return;
		}
		for (i = 0; i < ADDR_LEN_4; i++) {
		...
		for (j = 0; j < 5; j++) {
			...
			if (hx83112a_bus_write(dev, IC_ADR_AHB_ADDR_BYTE_0, buring_data, program_length + ADDR_LEN_4) < 0) {
				TOUCH_E("%s: i2c access fail!\n", __func__);
				return;
			}
		}

		if (!hx83112a_wait_wip(dev, 1)) {
			TOUCH_E("%s:Flash_Programming Fail\n", __func__);
		}
	}
}

static uint32_t hx83112a_check_CRC(struct device *dev, uint8_t *start_addr, int reload_length)
{
	uint32_t result = 0;
	uint8_t tmp_data[DATA_LEN_4];
	int cnt = 0, ret = 0;
	...
	ret = hx83112a_register_write(dev, FW_ADDR_RELOAD_ADDR_FROM, DATA_LEN_4, start_addr, ADDR_LEN_4);
	if (ret < NO_ERR) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	}
	...
	ret = hx83112a_register_write(dev, FW_ADDR_RELOAD_ADDR_CMD_BEAT, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	if (ret < NO_ERR) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return HW_CRC_FAIL;
	}
	...
	do {
		ret = hx83112a_register_read(dev, FW_ADDR_RELOAD_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		if (ret < NO_ERR) {
			TOUCH_E("%s: i2c access fail!\n", __func__);
			return HW_CRC_FAIL;
		}
		...
		if ((tmp_data[0] & 0x01) != 0x01) {
			ret = hx83112a_register_read(dev, FW_ADDR_RELOAD_CRC32_RESULT, DATA_LEN_4, tmp_data, ADDR_LEN_4);
			if (ret < NO_ERR) {
				TOUCH_E("%s: i2c access fail!\n", __func__);
				return HW_CRC_FAIL;
			}
			...
			TOUCH_D(FW_UPGRADE, "Waiting for HW ready!\n");
			touch_msleep(1);
		}
...
int hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k(struct device *dev, unsigned char *fw, int len)
{
	...
	hx83112a_ic_reset(dev, false, false);
	hx83112a_sense_off(dev, true);
	hx83112a_block_erase(dev, 0x00, FW_SIZE_64k);
	hx83112a_flash_programming(dev, fw, FW_SIZE_64k);

	for (i = 0; i < ADDR_LEN_4; i++) {
		tmp_data[i] = (uint8_t)(FW_ADDR_PROGRAM_RELOAD_FROM >> i * 8);
	}

	if (hx83112a_check_CRC(dev, tmp_data, FW_SIZE_64k) == 0) {
		burnFW_success = 1;
	}
	...
	hx83112a_register_write(dev, FW_ADDR_RAW_OUT_SEL, DATA_LEN_4, w_data, ADDR_LEN_4);
	/*DSRAM func initial*/
	hx83112a_assign_sorting_mode(dev, w_data);
	hx83112a_ic_reset(dev, false, false);

	return burnFW_success;
}

static int hx83112a_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	char fwpath[256] = {0};
	...
	if (hx83112a_fw_compare(dev, fw)) {
update_retry:
		ret = hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k(dev, (unsigned char *)fw->data, fw->size);
		if(ret < 0) {
			upgrade_times++;
...
bool hx83112a_calculateChecksum(struct device *dev)
{
	...
	CRC_result = hx83112a_check_CRC(dev, tmp_data, FW_SIZE_64k);
	touch_msleep(50);
...
static bool hx83112a_flash_lastdata_check(struct device *dev)
{
	...
	for (temp_addr = start_addr; temp_addr < (start_addr + flash_page_len); temp_addr = temp_addr + flash_page_len) {
		...
		hx83112a_register_read(dev, temp_addr, flash_page_len, &flash_tmp_buffer[0], ADDR_LEN_4);
	}
...
int hx83112a_read_FW_ver(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	while (reload_status == 0) {
		hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_FLASH_RELOAD, DATA_LEN_4, data, ADDR_LEN_4);
		hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_2ND_FLASH_RELOAD, DATA_LEN_4, data_2, ADDR_LEN_4);
	...
	/*=====================================
	 Read FW version
	=====================================*/
	hx83112a_sense_off(dev, false);
	hx83112a_register_read(dev, FW_ADDR_FW_VER_ADDR, DATA_LEN_4, data, ADDR_LEN_4);
	d->ic_info.vendor_panel_ver =  data[0];
	d->ic_info.vendor_fw_ver = data[1] << 8 | data[2];

	hx83112a_register_read(dev, FW_ADDR_FW_CFG_ADDR, DATA_LEN_4, data, ADDR_LEN_4);
	...
	hx83112a_register_read(dev, FW_ADDR_FW_VENDOR_ADDR, DATA_LEN_4, data, ADDR_LEN_4);
	d->ic_info.vendor_cid_maj_ver = data[2] ;
	d->ic_info.vendor_cid_min_ver = data[3];

	hx83112a_register_read(dev, FW_ADDR_LGE_FW_VER_ADDR, DATA_LEN_4, data, ADDR_LEN_4);
...
void hx83112a_touch_information(struct device *dev)
{
#ifndef HX_FIX_TOUCH_INFO
	char data[DATA_LEN_8] = {0};

	hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, DATA_LEN_8, data, ADDR_LEN_4);
	...
	hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_XY_RES_ENABLE, DATA_LEN_4, data, ADDR_LEN_4);
	...
	hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_X_Y_RES, DATA_LEN_4, data, ADDR_LEN_4);
	...
	hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_INT_IS_EDGE, DATA_LEN_4, data, ADDR_LEN_4);
	...
	hx83112a_register_read(dev, SRAM_ADR_MKEY, DATA_LEN_4, data, ADDR_LEN_4);
...
static int hx83112a_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct swipe_ctrl *ctrl[HX83112A_SWIPE_NUM] = {	/* U, D, L, R */
		&ts->swipe[SWIPE_U],
		&ts->swipe[SWIPE_D],
		&ts->swipe[SWIPE_L],
		&ts->swipe[SWIPE_R],
	...
	int i, ret = 0;
	uint32_t swipe_mode_addr[HX83112A_SWIPE_NUM] =
	...
	TOUCH_I("%s: SWIPE U(%d),D(%d),L(%d),R(%d)\n", __func__,
			ctrl[HX83112A_SWIPE_U]->enable,
			ctrl[HX83112A_SWIPE_D]->enable,
			ctrl[HX83112A_SWIPE_L]->enable,
			ctrl[HX83112A_SWIPE_R]->enable);

	if (enable) {
		for (i = 0; i < HX83112A_SWIPE_NUM; i++) {
			if (i == HX83112A_SWIPE_D)
				continue;
			...
			ret = hx83112a_register_write(dev, swipe_mode_addr[i], sizeof(data), data, ADDR_LEN_4);
			if (ret < 0) {
		...
		for (i = 0; i < HX83112A_SWIPE_NUM; i++) {	/* U, D, L, R*/
			data[0] = 0;
			ret = hx83112a_register_write(dev, swipe_mode_addr[i], ADDR_LEN_4, data, ADDR_LEN_4);
			if (ret < 0) {
				TOUCH_E("failed to register write %d (ret: %d)\n", i, ret);
				goto error;
...
static int himax_mcu_ulpm_in(struct device *dev)
{
	...
	do {
		...
		if (hx83112a_bus_write(dev, fw_addr_ulpm_34, tmp_data, 1) < 0) {
			TOUCH_D(TRACE, "%s: 1/7 write fail!\n", __func__);
			continue;
		}
		if (hx83112a_bus_read(dev, fw_addr_ulpm_34, tmp_data, 1) < 0) {
	...
	do {
		...
		if (hx83112a_bus_write(dev, fw_addr_ulpm_33, tmp_data, 1) < 0) {
			TOUCH_D(TRACE, "%s: 3/7 write fail!\n", __func__);
			continue;
		}
		if (hx83112a_bus_read(dev, fw_addr_ulpm_33, tmp_data, 1) < 0) {
...
static int hx83112a_deep_sleep(struct device *dev, bool onoff)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int hx83112a_tci_active_area(struct device *dev)
{
	int ret = 0;
	...
	ret = hx83112a_register_read(dev, FW_ADDR_TCI_AREA, ADDR_LEN_4, tci_data, ADDR_LEN_4);
	if (ret < 0) {
	...
	ret = hx83112a_register_write(dev, FW_ADDR_TCI_AREA, ADDR_LEN_4, tci_data, ADDR_LEN_4);
	if (ret < 0)
...
static int hx83112a_tci_knock(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = hx83112a_register_read(dev, FW_ADDR_TCI_MODE, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	if (ret < 0) {
		TOUCH_E("failed to register read (ret: %d)\n", ret);
		goto error;
	}
	...
	ret = hx83112a_register_write(dev, FW_ADDR_TCI_MODE, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	if (ret < 0) {
		TOUCH_E("failed to register write (ret: %d)\n", ret);
		goto error;
	}
	...
	ret = hx83112a_register_write(dev, FW_ADDR_TCI_SET, ADDR_LEN_4, lpwg_data, ADDR_LEN_4);
	if (ret < 0) {
		TOUCH_E("failed to register write (ret: %d)\n", ret);
		goto error;
	}

	ret = hx83112a_tci_active_area(dev);
	if (ret < 0)
...
static int hx83112a_lpwg_control(struct device *dev, int tci_mode, int swipe_enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = hx83112a_swipe_enable(dev, swipe_enable);
	if (ret < 0)
...
static int hx83112a_lpwg_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		if (ts->mfts_lpwg){
			...
			ret = hx83112a_lpwg_control(dev, LPWG_DOUBLE_TAP, SWIPE_ENABLE);
			if (ret < 0) {
		...
		if (ts->lpwg.screen) {
			...
			if (ts->lpwg.mode == LPWG_NONE &&
				...
				if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
					ret = hx83112a_deep_sleep(dev, IC_NORMAL);
					if (ret < 0) {
				...
				ret = hx83112a_lpwg_control(dev, ts->lpwg.mode,
						(ts->swipe[SWIPE_U].enable ||
						 ts->swipe[SWIPE_L].enable ||
						 ts->swipe[SWIPE_R].enable));
				if (ret < 0) {
	...
	if (ts->lpwg.screen) {
		...
		ret = hx83112a_lpwg_control(dev, LPWG_NONE, SWIPE_DISABLE);
		if (ret < 0) {
			TOUCH_E("failed to set lpwg control (ret: %d)\n", ret);
			goto error;
		}
...
static int hx83112a_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int *value = (int *)param;
	int ret = 0;
...
static void hx83112a_lcd_mode(struct device *dev, u32 mode)
{
	struct himax_ts_data *d = to_himax_data(dev);
...
static int hx83112a_fb_notifier_callback(struct notifier_block *self,
...
static int hx83112a_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
	...
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		ts->fb_notif.notifier_call = hx83112a_fb_notifier_callback;
		fb_register_client(&ts->fb_notif);
		update_flag = (!hx83112a_calculateChecksum(dev));
		update_flag |= hx83112a_flash_lastdata_check(dev);
		if (update_flag) {
			ts->force_fwup = 1;
			TOUCH_I("Calculate CRC fail - force fwup:%d\n", ts->force_fwup);
		}
		hx83112a_read_FW_ver(dev);
		hx83112a_sense_on(dev, 0x00);
		hx83112a_set_SMWP_enable(dev, true);
	}
	if (atomic_read(&ts->state.core) == CORE_UPGRADE) {
		hx83112a_read_FW_ver(dev);
		hx83112a_touch_information(dev);
		hx83112a_sense_on(dev, 0x00);
	}
...
static void hx83112a_calcDataSize(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
...
static void hx83112a_calculate_point_number(void)
{
...
static void hx83112a_esd_ic_reset(struct device *dev)
{
	hx83112a_hw_reset(dev);
	TOUCH_I("%s:\n", __func__);
}

static void hx83112a_esd_hw_reset(struct device *dev)
{
	if (g_ts_dbg != 0)
		TOUCH_I("%s: Entering \n", __func__);

	TOUCH_I("START_Himax TP: ESD - Reset\n");
	hx83112a_esd_ic_reset(dev);
	TOUCH_I("END_Himax TP: ESD - Reset\n");
}

static int hx83112a_parse_tci_data(struct device *dev, uint8_t *buf, uint8_t tap_count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int i = 0;
	int j = 0;
...
static int hx83112a_parse_swipe_data(struct device *dev, uint8_t *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int count = 2;
	int time = 0;
...
static int hx83112a_parse_lpwg_debug_data(struct device *dev, uint8_t *buf, uint8_t tap_count)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int i = 0;
	int j = 0;
...
static int hx83112a_wake_event_parse(struct device *dev, int ts_status)
{
	uint8_t *buf;
	int i = 0, check_FC = 0;
	...
	if (check_FC != GEST_PTLG_ID_LEN) {
		hx83112a_parse_lpwg_debug_data(dev, buf, tap_count);
		kfree(buf);
		return 0;
	}
...
static void hx83112a_wake_event_report(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int hx83112a_get_touch_data_size(void)
{
	return HIMAX_TOUCH_DATA_SIZE;
}

static int hx83112a_cal_data_len(int raw_cnt_rmd, int HX_MAX_PT, int raw_cnt_max)
{
	int RawDataLen;
...
int hx83112a_report_data_init(void)
{
	...
	hx_touch_data->event_size = hx83112a_get_touch_data_size();

	if (hx_touch_data->hx_event_buf != NULL) {
		kfree(hx_touch_data->hx_event_buf);
	}

	hx_touch_data->touch_all_size = hx83112a_get_touch_data_size();
	...
	if (hx_touch_data->raw_cnt_rmd != 0x00) {
		hx_touch_data->rawdata_size = hx83112a_cal_data_len(hx_touch_data->raw_cnt_rmd, ic_data->HX_MAX_PT, hx_touch_data->raw_cnt_max);
...
/*start ts_work*/
static int hx83112a_ts_work_status(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	/* 1: normal, 2:SMWP */
...
static bool hx83112a_read_event_stack(struct device *dev, uint8_t *buf, uint8_t length)
{
	//uint8_t cmd[DATA_LEN_4];
	/*  AHB_I2C Burst Read Off */
	/*cmd[0] = FW_DATA_AHB_DIS;

	if (hx83112a_bus_write(dev, FW_ADDR_AHB_ADDR, cmd, 1) < 0) {
		TOUCH_E("%s: i2c access fail!\n", __func__);
		return 0;
	}*/

	hx83112a_bus_read(dev, FW_ADDR_EVENT_ADDR, buf, length);
...
#if 0
static void hx83112a_touch_info_get(struct device *dev)
{
	uint8_t cnt = 50;

	//tmp_addr[3] = 0x10; tmp_addr[2] = 0x00; tmp_addr[1] = 0x05; tmp_addr[0] = 0x78;
	do {
		hx83112a_register_read(dev, DRIVER_ADDR_FW_WIDTH_ANGLE, 40, hx_touch_data->hx_width_buf, ADDR_LEN_4);
...
static int hx83112a_touch_get(struct device *dev, uint8_t *buf, int ts_path, int ts_status)
{
	if (g_ts_dbg != 0)
		TOUCH_I("%s: Entering, ts_status=%d! \n", __func__, ts_status);
	...
	case HX_REPORT_COORD:
		if (!hx83112a_read_event_stack(dev, buf, CUST_TP_SIZE)) { //hx_touch_data->touch_info_size
			TOUCH_E("%s: can't read data from chip!\n", __func__);
			ts_status = HX_TS_GET_DATA_FAIL;
			goto END_FUNCTION;
		}
		/* read touch width info from SRAM */
		/*hx83112a_touch_info_get(dev);*/
		break;
	...
	case HX_REPORT_SMWP_EVENT:
		...
		if (!hx83112a_read_event_stack(dev, buf, hx_touch_data->event_size)) {
			TOUCH_E("%s: can't read data from chip!\n", __func__);
			ts_status = HX_TS_GET_DATA_FAIL;
			goto END_FUNCTION;
		}
	...
	case HX_REPORT_COORD_RAWDATA:
		if (!hx83112a_read_event_stack(dev, buf, 128)) {
			TOUCH_E("%s: can't read data from chip!\n", __func__);
			ts_status = HX_TS_GET_DATA_FAIL;
			goto END_FUNCTION;
		}
...
/* start error_control*/
static int hx83112a_checksum_cal(struct device *dev, uint8_t *buf, int ts_path, int ts_status)
{
...
static int hx83112a_ic_esd_recovery(int hx_esd_event, int hx_zero_event, int length)
{
	int ret_val = NO_ERR;
...
static int hx83112a_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
	...
	TOUCH_I("hx83112a_esd_recovery!!\n");

	return 0;
}

static int hx83112a_ts_event_check(struct device *dev, uint8_t *buf, int ts_path, int ts_status)
{
	int hx_EB_event = 0;
	int hx_EC_event = 0;
	int hx_ED_event = 0;
...
/* SMWP */
	...
	if ((hx_esd_event == length || hx_zero_event == length)
		...
		shaking_ret = hx83112a_ic_esd_recovery(hx_esd_event, hx_zero_event, length);

		if (shaking_ret == HX_ESD_EVENT) {
			hx83112a_esd_hw_reset(dev);
			ret_val = HX_ESD_EVENT;
...
static int hx83112a_err_ctrl(struct device *dev, uint8_t *buf, int ts_path, int ts_status)
{
	ts_status = hx83112a_checksum_cal(dev, buf, ts_path, ts_status);
...
CHK_FAIL:
	ts_status = hx83112a_ts_event_check(dev, buf, ts_path, ts_status);
...
static int hx83112a_distribute_touch_data(uint8_t *buf, int ts_path, int ts_status)
{
...
int hx83112a_parse_report_points(struct device *dev, int ts_path, int ts_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
	int x = 0, y = 0, w = 0;
...
static int hx83112a_parse_report_data(struct device *dev, int ts_path, int ts_status)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	switch (ts_path) {
	case HX_REPORT_COORD:
		ts_status = hx83112a_parse_report_points(dev, ts_path, ts_status);
		break;
	case HX_REPORT_COORD_RAWDATA:
		...
		break;
	case HX_REPORT_SMWP_EVENT:
		hx83112a_wake_event_parse(dev, ts_status);
		break;
	default:
		TOUCH_E("%s:Fail Path!\n", __func__);
		ts_status = HX_PATH_FAIL;
...
/* start report_point*/
static void hx83112a_finger_report(struct device *dev)
{
	...
	for (i = 0; i < d->nFinger_support; i++) {
		...
		if (valid) {
			...
			tdata[i].width_major = 10;
			tdata[i].width_minor = 10;
...
static void hx83112a_finger_leave(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
...
static void hx83112a_report_points(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	if (d->hx_point_num != 0) {
		hx83112a_finger_report(dev);
	} else {
		hx83112a_finger_leave(dev);
	}
...
static int hx83112a_report_data(struct device *dev, int ts_path, int ts_status)
{
	struct himax_ts_data *d = to_himax_data(dev);
...
static int hx83112a_ts_operation(struct device *dev, int ts_path, int ts_status)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ts_status = hx83112a_touch_get(dev, buf, ts_path, ts_status);
	if (ts_status == HX_TS_GET_DATA_FAIL)
		goto END_FUNCTION;

	ts_status = hx83112a_err_ctrl(dev, buf, ts_path, ts_status);
	if (ts_status == HX_REPORT_DATA || ts_status == HX_TS_NORMAL_END) {
		ts_status = hx83112a_distribute_touch_data(buf, ts_path, ts_status);
		ts_status = hx83112a_parse_report_data(dev, ts_path, ts_status);
	...
	ts_status = hx83112a_report_data(dev, ts_path, ts_status);
...
static int hx83112a_irq_handler(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	ts_path = hx83112a_ts_work_status(dev);
	switch (ts_path) {
	case HX_REPORT_COORD:
		ts_status = hx83112a_ts_operation(dev, ts_path, ts_status);
		break;
	case HX_REPORT_SMWP_EVENT:
		ts_status = hx83112a_ts_operation(dev, ts_path, ts_status);
		break;
...
void hx83112a_set_SMWP_enable(struct device *dev, uint8_t SMWP_enable)
{
	uint8_t tmp_data[DATA_LEN_4] = {0, };
	...
	do {
		...
		hx83112a_register_write(dev, FW_ADDR_SMWP_ENABLE, DATA_LEN_4, w_data, ADDR_LEN_4);

		hx83112a_register_read(dev, FW_ADDR_SMWP_ENABLE, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
void hx83112a_SMWP_dbg_msg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (ts->lpwg.mode) {
		hx83112a_register_read(dev, HX_ADDR_DBG_KNOCK_ON, read_len, DBG_BUF, ADDR_LEN_4);
		for (i = 0; i < read_len; i = i + HX_MSG_SIZE)
			hx83112a_parse_lpwg_debug_data(dev, &DBG_BUF[i], tap_count);
	}
	if (ts->swipe[SWIPE_U].enable ||
			ts->swipe[SWIPE_L].enable ||
			ts->swipe[SWIPE_R].enable) {
		hx83112a_register_read(dev, HX_ADDR_DBG_SWIPE, read_len, DBG_BUF, ADDR_LEN_4);
		for (i = 0; i < read_len; i = i + HX_MSG_SIZE)
			hx83112a_parse_lpwg_debug_data(dev, &DBG_BUF[i], tap_count);
	}
}

static int hx83112a_suspend(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	struct touch_core_data *ts = to_touch_core(dev);
	...
	d->pre_finger_mask = 0;
	hx_touch_data->hx_palm = 0;
	hx83112a_lpwg_mode(dev);

	return 0;
}

static int hx83112a_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
...
static bool hx83112a_chip_detect(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	uint8_t tmp_data[DATA_LEN_4];
	bool ret_data = false;
	...
	hx83112a_hw_reset(dev);
	hx83112a_sense_off(dev, false); //hx83102ab_sense_off(false);

	for (i = 0; i < 5; i++) {
		hx83112a_register_read(dev, FW_ADDR_ICID_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		...
		if ((tmp_data[3] == 0x83) && (tmp_data[2] == 0x11) && (tmp_data[1] == 0x2a)) {
			strlcpy(d->chip_name, HX_83112A_SERIES_PWON, 30);
			TOUCH_I("%s:detect IC HX83112A successfully\n", __func__);
			ret_data = true;
			break;
		} else {
			ret_data = false;
...
static void hx83112a_chip_init(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	...
	FW_LGE_VER_MAJ_FLASH_ADDR = 49160;	/*0xC008*/
	/*PANEL_VERSION_ADDR		= 49156;*/  /*0x00C004*/
	/*PANEL_VERSION_LENG		= 1;*/
}

static void hx83112a_power_on_init(struct device *dev)
{
	uint8_t tmp_data[DATA_LEN_4];
	int i = 0;
	...
	hx83112a_touch_information(dev);
	/*RawOut select initial*/
	hx83112a_register_write(dev, FW_ADDR_RAW_OUT_SEL, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	/*DSRAM func initial*/
	hx83112a_assign_sorting_mode(dev, tmp_data);
	hx83112a_sense_on(dev, 0x00);
}

static int hx83112a_common_init(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	int err = 0;
	...
	if (hx83112a_chip_detect(dev) != false) {
		hx83112a_chip_init(dev);
	} else {
...
//	hx83112a_read_FW_ver(dev);
	...
	hx83112a_power_on_init(dev);
	hx83112a_calculate_point_number();

	/*calculate the i2c data size*/
	hx83112a_calcDataSize(dev);
...
*/
	...
	/*touch data init*/
	err = hx83112a_report_data_init();

	if (err) {
		return -ENOMEM;
	}

	if (hx83112a_proc_init()) {
		TOUCH_E(" %s: himax_common proc_init failed!\n", __func__);
		return -ENOMEM;
	}

#ifdef CONFIG_LGE_TOUCH_HIMAX_HX83112A_DEBUG
	if (hx83112a_debug_init(dev))
		TOUCH_E(" %s: debug initial failed!\n", __func__);
#endif
	return 0;
}

static int hx83112a_init_pm(struct device *dev)
{
	...
	ts->drm_notif.notifier_call = hx83112a_drm_notifier_callback;
#endif
...
static void hx83112a_fb_notify_work_func(struct work_struct *fb_notify_work)
{
...
static void hx83112a_init_works(struct himax_ts_data *d)
{
	INIT_DELAYED_WORK(&d->fb_notify_work, hx83112a_fb_notify_work_func);
}

static void hx83112a_init_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void hx83112a_init_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int hx83112a_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	if (!d) {
		TOUCH_E("failed to allocate hx83112a data\n");
		return -ENOMEM;
	}
	...
	hx83112a_init_works(d);
	hx83112a_common_init(dev);
	d->lcd_mode = LCD_MODE_U3;
	hx83112a_init_tci_info(dev);
	hx83112a_init_swipe_info(dev);

	return 0;
}

static int hx83112a_remove(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);

#ifdef CONFIG_LGE_TOUCH_HIMAX_HX83112A_DEBUG
	hx83112a_debug_remove(dev);
#endif

	hx83112a_proc_deinit();
...
static int hx83112a_shutdown(struct device *dev)
{
	return 0;
}

static void hx83112a_usb_detect_set(struct device *dev, uint8_t *cable_config)
{
	uint8_t tmp_data[DATA_LEN_4] = {0, };
	uint8_t back_data[DATA_LEN_4] = {0, };
	uint8_t w_data[DATA_LEN_4] = {0, };
	...
	do {
		...
		hx83112a_register_write(dev, FW_USB_DETECT_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);

		hx83112a_register_read(dev, FW_USB_DETECT_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
static int hx83112a_cable_connect(struct device *dev)
{
	...
	hx83112a_usb_detect_set(dev, d->cable_config);
	TOUCH_I("%s: Cable status change: 0x%2.2X\n", __func__, d->cable_config[1]);

	return 0;
}

static int hx83112a_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct himax_ts_data *d = to_himax_data(dev);
	int ret = 0;
...
static int hx83112a_get_cmd_version(struct device *dev, char *buf)
{
	struct himax_ts_data *d = to_himax_data(dev);
	int offset = 0;
...
;
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset,
			...
			d->ic_info.vendor_panel_ver, d->ic_info.vendor_touch_cfg_ver,
			d->ic_info.vendor_display_cfg_ver);
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset, " Product_id : [HX83112-A]\n");
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset,
			"======================================\n");
...
static int hx83112a_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct himax_ts_data *d = to_himax_data(dev);
	int offset = 0;
...
static int hx83112a_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
	TOUCH_I("%s\n", __func__);
...
static int hx83112a_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
	case CMD_VERSION:
		ret = hx83112a_get_cmd_version(dev, (char *)output);
		break;

	case CMD_ATCMD_VERSION:
		ret = hx83112a_get_cmd_atcmd_version(dev, (char *)output);
		break;
...
static struct touch_driver touch_driver = {
	...
	.shutdown = hx83112a_shutdown,
	.suspend = hx83112a_suspend,
	.resume = hx83112a_resume,
	.init = hx83112a_init,
	.irq_handler = hx83112a_irq_handler,
	.power = hx83112a_power,
	.upgrade = hx83112a_upgrade,
	.esd_recovery = hx83112a_esd_recovery,
	.lpwg = hx83112a_lpwg,
	.notify = hx83112a_notify,
	.init_pm = hx83112a_init_pm,
	.swipe_enable = hx83112a_swipe_enable,
	.register_sysfs = hx83112a_register_sysfs,
	.set = hx83112a_set,
	.get = hx83112a_get,
};

#define MATCH_NAME			"himax,hx83112a"
...
static int __init touch_device_init(void)
{
	...
	if (is_lcm_name("TXD-HX83112")) {
		TOUCH_I("%s, HX83112 found!!! lcm_name = %s\n", __func__, lge_get_lcm_name());
	} else {
		TOUCH_I("%s, HX83112 not found.\n", __func__);
		return 0;
	}
