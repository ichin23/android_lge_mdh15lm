...
static char *line;
static int UpperImage[ROW_SIZE][COL_SIZE];
static int LowerImage[ROW_SIZE][COL_SIZE];
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (temp_buf == NULL){
			TOUCH_E("%s, alloc fail\n", __func__);
			return ret;
		}
		hx83112a_sense_off(dev, true);
		touch_msleep(20);
		result = hx83112a_calculateChecksum(dev);
		hx83112a_sense_on(dev, 0x01);
...
static ssize_t show_layout(struct file *file, char *buf, size_t len, loff_t *pos)
{
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		hx83112a_register_read(dev, addr, 128, data, ADDR_LEN_4);
	...
	if ((buf[0] == 'r' || buf[0] == 'w') && buf[1] == ':' && buf[2] == 'x') {
		...
		if (buf[0] == 'r') {
			...
			if (count < 3) {
				...
				addr = (register_command[3] << 24) + (register_command[2] << 16) + (register_command[1] << 8) + register_command[0];
				hx83112a_register_write(dev, addr, byte_length, w_data, ADDR_LEN_4);
			} else {
				...
				if (strcmp(HX_85XX_H_SERIES_PWON, private_ts->chip_name) == 0 && cfg_flag == 0)
					cfg_flag = 2;
				addr = (register_command[3] << 24) + (register_command[2] << 16) + (register_command[1] << 8) + register_command[0];
				hx83112a_register_write(dev, addr, byte_length, &w_data[0], ADDR_LEN_4);
			}
		} else {
			return len;
		}
...
int hx83112a_get_limit(struct device *dev, char *breakpoint,
...
static bool hx83112a_diag_check_sum(struct himax_report_data *hx_touch_data)
{
...
static void hx83112a_diag_parse_raw_data(struct himax_report_data *hx_touch_data, int mul_num, int self_num, uint8_t diag_cmd, int32_t *mutual_data, int32_t *self_data)
{
	int RawDataLen_word;
	int index = 0;
	int temp1, temp2, i;
...
int hx83112a_set_diag_cmd(struct himax_ic_data *ic_data, struct himax_report_data *hx_touch_data)
{
	struct himax_ts_data *d = private_ts;
	int32_t *mutual_data;
	int32_t *self_data;
	...
	if (hx_touch_data->diag_cmd >= 1 && hx_touch_data->diag_cmd <= 7) {
		...
		if (!hx83112a_diag_check_sum(hx_touch_data)) {
		...
		hx83112a_diag_parse_raw_data(hx_touch_data, mul_num, self_num, hx_touch_data->diag_cmd, mutual_data, self_data);
...
void hx83112a_log_touch_data(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
	int print_size = 0;
	uint8_t *buf;
...
void hx83112a_log_touch_event(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
	struct touch_core_data *ts = to_touch_core(g_hx_dev);
...
void hx83112a_log_touch_int_devation(int touched)
{
...
void hx83112a_log_touch_event_detail(struct himax_ts_data *d, int start)
{
	int loop_i = 0;
	struct touch_core_data *ts = to_touch_core(g_hx_dev);
...
void hx83112a_ts_dbg_func(struct himax_ts_data *d, int start)
{
	switch (d->debug_log_level) {
	case 1:
		hx83112a_log_touch_data(d, start);
		break;
	case 2:
		hx83112a_log_touch_event(d, start);
		break;
	case 4:
		hx83112a_log_touch_int_devation(start);
		break;
	case 8:
		hx83112a_log_touch_event_detail(d, start);
		break;
	}
}
...
static void hx83112a_print_state_info(struct seq_file *s)
{
...
static void hx83112a_diag_arrange_print(struct seq_file *s, int i, int j, int transpose)
{
	if (transpose)
...
static void hx83112a_diag_arrange_inloop(struct seq_file *s, int in_init, int out_init, bool transpose, int j)
{
	...
	if (in_init > 0) { /* bit0 = 1 */
		for (i = in_init - 1; i >= 0; i--) {
			hx83112a_diag_arrange_print(s, i, j, transpose);
		}
...
/* print first step which is row */
static void hx83112a_diag_arrange_outloop(struct seq_file *s, int transpose, int out_init, int in_init)
{
	int j;
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	...
	if (out_init > 0) { /* bit1 = 1 */
		...
		for (j = out_init - 1; j >= 0; j--) {
			...
			hx83112a_diag_arrange_inloop(s, in_init, out_init, transpose, j);
...
static void hx83112a_diag_arrange(struct seq_file *s)
{
	int x_channel = ic_data->HX_RX_NUM;
	int y_channel = ic_data->HX_TX_NUM;
	int bit2, bit1, bit0;
	...
	if (g_diag_arr_num < 4) {
		...
		seq_printf(s, "\n");
		hx83112a_diag_arrange_outloop(s, bit2, bit1 * y_channel, bit0 * x_channel);
		seq_printf(s, "%6c", ' ');
		...
		seq_printf(s, "\n");
		hx83112a_diag_arrange_outloop(s, bit2, bit1 * x_channel, bit0 * y_channel);
		seq_printf(s, "%6c", ' ');
...
/* DSRAM thread */
static int hx83112a_write_read_reg(struct device *dev, uint32_t tmp_addr, uint8_t *tmp_data, uint8_t hb, uint8_t lb)
{
	int cnt = 0;
	...
	do {
		if (r_data[1] != lb || r_data[0] != hb)
			hx83112a_register_write(dev, tmp_addr, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		touch_msleep(5);
		hx83112a_register_read(dev, tmp_addr, 4, r_data, ADDR_LEN_4);
...
static bool hx83112a_get_DSRAM_data(struct device *dev, uint8_t *info_data, bool DSRAM_Flag)
{
	int i = 0;
	...
	fw_run_flag = hx83112a_write_read_reg(dev, SRAM_ADR_RAWDATA_ADDR, tmp_data, Data_PWD0, Data_PWD1); //A5,5A
	...
	for (i = 0; i < total_read_times; i++) {
		...
		if (total_size_temp >= max_i2c_size) {
			...
			hx83112a_register_read(dev, addr, max_i2c_size, &temp_info_data[i * max_i2c_size], ADDR_LEN_4);
			total_size_temp = total_size_temp - max_i2c_size;
			TOUCH_I("addr = 0x%08X\n", addr);
		} else {
			/*TOUCH_I("last total_size_temp=%d\n",total_size_temp);*/
			addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
			hx83112a_register_read(dev, addr, total_size_temp % max_i2c_size, &temp_info_data[i * max_i2c_size], ADDR_LEN_4);
		}
	}
	...
	w_data[1] = 0x00;
	w_data[0] = 0x00;
	hx83112a_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
...
bool hx83112a_ts_diag_func(void)
{
	struct device *dev = g_hx_dev;
	...
	if (hx83112a_get_DSRAM_data(dev, info_data, DSRAM_Flag) == false)
		return false;
...
void hx83112a_reload_disable(struct device *dev, int disable)
{
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83112a_register_write(dev, DRIVER_ADDR_FW_DEFINE_FLASH_RELOAD, DATA_LEN_4, w_data, ADDR_LEN_4);

	TOUCH_I("%s: setting OK!\n", __func__);
}

static int hx83112a_determin_diag_rawdata(int diag_command)
{
	return diag_command % 10;
}

static int hx83112a_determin_diag_storage(int diag_command)
{
	return diag_command / 10;
}

static void hx83112a_return_event_stack(struct device *dev)
{
	int retry = 20;
	int i = 0;
	...
	do {
		...
		hx83112a_register_write(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		hx83112a_register_read(dev, SRAM_ADR_RAWDATA_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		retry--;
		touch_msleep(10);
...
static void hx83112a_idle_mode(struct device *dev, int disable)
{
	int retry = 20;
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		...
		hx83112a_register_read(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		...
		hx83112a_register_write(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		hx83112a_register_read(dev, FW_ADDR_FW_MODE_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
int hx83112a_assign_sorting_mode(struct device *dev, uint8_t *tmp_data)
{
	...
	hx83112a_register_write(dev, FW_ADDR_SORTING_MODE_EN, DATA_LEN_4, tmp_data, ADDR_LEN_4);

	return NO_ERR;
}

static int hx83112a_check_sorting_mode(struct device *dev, uint8_t *tmp_data)
{

	hx83112a_register_read(dev, FW_ADDR_SORTING_MODE_EN, DATA_LEN_4, tmp_data, ADDR_LEN_4);
	TOUCH_I("%s: tmp_data[0]=%x,tmp_data[1]=%x\n", __func__, tmp_data[0], tmp_data[1]);

	return NO_ERR;
}

static int hx83112a_switch_mode(struct device *dev, int mode)
{
	uint8_t tmp_data[DATA_LEN_4];
	uint8_t w_data[DATA_LEN_4];
	int i = 0;
	...
	hx83112a_idle_mode(dev, 1);
	hx83112a_reload_disable(dev, 1);
	...
	if (mode) {
		...
		hx83112a_register_write(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, DATA_LEN_4, w_data, ADDR_LEN_4);
	} else {
		for (i = 0; i < ADDR_LEN_4; i++) {
			w_data[i] = (uint8_t)(FW_DATA_SET_FRAME >> i * 8);
		}
		hx83112a_register_write(dev, FW_ADDR_SET_FRAME_ADDR, DATA_LEN_4, w_data, ADDR_LEN_4);
		for (i = 0; i < ADDR_LEN_4; i++) {
	...
	while (retry != 0) {
		TOUCH_I("[%d] %s Read\n", retry, __func__);
		hx83112a_check_sorting_mode(dev, tmp_data);
		touch_msleep(100);
		...
		hx83112a_register_read(dev, FW_ADDR_CHK_FW_STATUS, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
void hx83112a_diag_register_set(struct device *dev, uint8_t diag_command, uint8_t storage_type)
{
	uint8_t tmp_data[DATA_LEN_4];
	...
	do {
		...
		hx83112a_register_read(dev, FW_ADDR_RAW_OUT_SEL, DATA_LEN_4, back_data, ADDR_LEN_4);
								 ...
								 size_t len, loff_t *pos)
{
	...
	storage_type = hx83112a_determin_diag_storage(d->diag_cmd);
	rawdata_type = hx83112a_determin_diag_rawdata(d->diag_cmd);
	...
	if (storage_type == 0 && rawdata_type > 0 && rawdata_type < 8) {
		...
		if (DSRAM_Flag) {
			...
			hx83112a_return_event_stack(dev);
		}

		if (g_switch_mode == 2) {
			hx83112a_idle_mode(dev, 0);
			g_switch_mode = hx83112a_switch_mode(dev, 0);
		}
		...
		hx83112a_diag_register_set(dev, command[0], storage_type);
		...
		hx83112a_idle_mode(dev, 1);
		g_switch_mode = hx83112a_switch_mode(dev, 1);
								...
								size_t len, loff_t *pos)
{
	...
	hx83112a_set_SMWP_enable(dev, d->SMWP_enable);
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf_tmp[0] == '1') {
		...
		hx83112a_ic_reset(dev, false, true);
	} else if (buf_tmp[0] == '3') {
		hx83112a_ic_reset(dev, true, false);
	} else if (buf_tmp[0] == '4') {
		hx83112a_ic_reset(dev, true, true);
	}
...
static int hx83112a_read_ic_trigger_type(struct device *dev)
{
	uint8_t tmp_data[DATA_LEN_4];
	int trigger_type = false;
	hx83112a_register_read(dev, FW_ADDR_TRIGGER_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
...
static int hx83112a_read_i2c_status(void)
{
								...
								size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (debug_level_cmd == 't') {
			...
			if (hx83112a_read_ic_trigger_type(dev) == 1) /* Edgd = 1, Level = 0 */
				ret += touch_snprintf(temp_buf + ret, len - ret, "IC Interrupt type is edge trigger.\n");
			else if (hx83112a_read_ic_trigger_type(dev) == 0)
								 ...
								 size_t len, loff_t *pos)
{
	...
	if (buf[0] == 'v') { /* firmware version */
		...
		switch (fw_type) {
		case 64:
			if (hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k(dev, (unsigned char *)fw->data, fw->size) == 0) {
...
static int hx83112a_read_FW_status(struct device *dev, uint8_t *state_addr, uint8_t *tmp_addr)
{
	uint8_t i;
	uint8_t req_size = 0;
	...
	if (state_addr[0] == 0x01) {
		...
		hx83112a_register_read(dev, addr, req_size, tmp_addr, ADDR_LEN_4);
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (hx83112a_read_FW_status(dev, cmd_set, tmp_data) == NO_ERR) {
...
uint8_t hx83112a_read_DD_status(struct device *dev, uint8_t *cmd_set, uint8_t *tmp_data)
{
	int cnt = 0;
	uint8_t req_size = cmd_set[0];
	cmd_set[3] = FW_DATA_DD_REQUEST;
	hx83112a_register_write(dev, FW_ADDR_DD_HANDSHAK_ADDR, DATA_LEN_4, cmd_set, ADDR_LEN_4);
	...
	for (cnt = 0; cnt < 100; cnt++) {
		hx83112a_register_read(dev, FW_ADDR_DD_HANDSHAK_ADDR, DATA_LEN_4, tmp_data, ADDR_LEN_4);
		touch_msleep(10);
	...
	hx83112a_register_read(dev, FW_ADDR_DD_DATA_ADDR, req_size, tmp_data, ADDR_LEN_4);
	return NO_ERR;
}
										...
										size_t len, loff_t *pos)
{
	...
	if (!HX_PROC_SEND_FLAG) {
		...
		if (mutual_set_flag == 1) {
			if (hx83112a_read_DD_status(dev, cmd_set, tmp_data) == NO_ERR) {
...
static void hx83112a_flash_dump_func(struct device *dev, uint8_t local_flash_command, int Flash_Size, uint8_t *flash_buffer)
{
	...
	for (page_prog_start = 0; page_prog_start < Flash_Size; page_prog_start += 128) {
		...
		hx83112a_register_read(dev, addr, 128, buffer, ADDR_LEN_4);
...
void hx83112a_ts_flash_func(void)
{
	struct device *dev = g_hx_dev;
	struct himax_ts_data *d = to_himax_data(dev);
	uint8_t local_flash_command = 0;
	...
	if ((local_flash_command == 1 || local_flash_command == 2) || (local_flash_command == 0x0F)) {
		hx83112a_flash_dump_func(dev, local_flash_command, Flash_Size, flash_buffer);
	}
...
static void hx83112a_dbg_data_init(void)
{
	debug_data->fp_ts_dbg_func = hx83112a_ts_dbg_func;
	debug_data->fp_set_diag_cmd = hx83112a_set_diag_cmd;
	debug_data->flash_dump_going = false;
}

static void hx83112a_ts_flash_work_func(struct work_struct *work)
{
	hx83112a_ts_flash_func();
}

static void hx83112a_ts_diag_work_func(struct work_struct *work)
{
	hx83112a_ts_diag_func();
}

int hx83112a_touch_proc_init(void)
{
...
void hx83112a_touch_proc_deinit(void)
{
...
int hx83112a_debug_init(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);
	int err = 0;
	...
	INIT_WORK(&d->flash_work, hx83112a_ts_flash_work_func);
	setSysOperation(0);
	setFlashBuffer();
	...
	INIT_DELAYED_WORK(&d->himax_diag_delay_wrok, hx83112a_ts_diag_work_func);
...
int hx83112a_debug_remove(struct device *dev)
{
	struct himax_ts_data *d = to_himax_data(dev);

	hx83112a_touch_proc_deinit();
...
void hx83112a_proc_deinit(void)
{
...
static int prd_switch_mode(struct device *dev, int mode)
{
	...
	hx83112a_register_write(dev, SRAM_ADR_RAWDATA_ADDR, 4, tmp_data, ADDR_LEN_4);
...
static int prd_get_rawdata(struct device *dev, uint32_t RAW[], uint32_t datalen)
{
	...
	while (retry < 300) {
		...
		hx83112a_register_read(dev, SRAM_ADR_RAWDATA_ADDR, 4, tmp_data, ADDR_LEN_4);
	...
	while (retry < 10) {
		...
		for (i = 0; i < (total_read_times); i++) {
			if (total_size_temp >= max_i2c_size) {
				addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
				hx83112a_register_read(dev, addr, max_i2c_size, &tmp_rawdata[i*max_i2c_size], ADDR_LEN_4);
				total_size_temp = total_size_temp - max_i2c_size;
			} else {
				/*TOUCH_I("last total_size_temp=%d\n", total_size_temp);*/
				addr = (tmp_addr[3] << 24) + (tmp_addr[2] << 16) + (tmp_addr[1] << 8) + tmp_addr[0];
				hx83112a_register_read(dev, addr, total_size_temp % max_i2c_size, &tmp_rawdata[i*max_i2c_size], ADDR_LEN_4);
			}
	...
	Min_DATA = (int16_t)RAW[0];
	Max_DATA = (int16_t)RAW[0];

	for (i = 0; i < ic_data->HX_TX_NUM; i++) {
		printk("TX%2d", i + 1);
		for (j = 0; j < ic_data->HX_RX_NUM; j++) {
			printk("%5d ", RAW[index]);
			if ((int16_t)RAW[index] > Max_DATA) {
				Max_DATA = (int16_t)RAW[index];
			}
			if ((int16_t)RAW[index] < Min_DATA) {
				Min_DATA = (int16_t)RAW[index];
			}
			index++;
		}
		printk("\n");
...
static void prd_switch_data_type(struct device *dev, uint8_t checktype)
{
	...
	hx83112a_diag_register_set(dev, datatype, 0x00);
}
...
static void prd_cut_cc_ctrl(struct device *dev, uint8_t cut_cc_en)
{
	...
	hx83112a_register_read(dev, FW_ADDR_FW_NEW_METHOLD, 4, tmp_data, ADDR_LEN_4);
	if ((tmp_data[3] & 0x20) == 0x20) {
		hx83112a_register_read(dev, FW_ADDR_CC_CUT_NEW, 4, tmp_data, ADDR_LEN_4);
		if (cut_cc_en)
		...
		hx83112a_register_read(dev, FW_ADDR_CC_CUT_OLD, 4, tmp_data, ADDR_LEN_4);
		if (!cut_cc_en)
...
static void prd_neg_noise_sup(struct device *dev, uint8_t *data)
{
	...
	hx83112a_register_read(dev, FW_ADDR_NEG_NOISE_SUP, 4, tmp_data, ADDR_LEN_4);
...
static void prd_set_N_frame(struct device *dev, uint16_t Nframe, uint8_t checktype)
{
	...
	hx83112a_register_write(dev, FW_ADDR_SET_FRAME_ADDR, 4, tmp_data, ADDR_LEN_4);

	/*skip frame*/
	hx83112a_register_read(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, 4, tmp_data, ADDR_LEN_4);
	...
	hx83112a_register_write(dev, DRIVER_ADDR_FW_DEFINE_RXNUM_TXNUM_MAXPT, 4, tmp_data, ADDR_LEN_4);
}
...
static uint32_t prd_wait_sorting_mode(struct device *dev, uint8_t checktype)
{
	...
	do {
		...
		hx83112a_register_read(dev, FW_ADDR_CHK_FW_STATUS, 4, tmp_data, ADDR_LEN_4);
		TOUCH_I("%s: 0x900000A8, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n", __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		hx83112a_register_read(dev, FW_ADDR_CHK_FW_STATUS_2, 4, tmp_data, ADDR_LEN_4);
		TOUCH_I("%s: 0x900000E4, tmp_data[0]=%x,tmp_data[1]=%x,tmp_data[2]=%x,tmp_data[3]=%x \n", __func__, tmp_data[0], tmp_data[1], tmp_data[2], tmp_data[3]);

		hx83112a_register_read(dev, FW_ADDR_CHK_FW_STATUS_3, 4, tmp_data, ADDR_LEN_4);
...
int prd_get_raw_data(struct device *dev, uint8_t diag_cmd, uint16_t mutual_num, int16_t RAW[])
{
	...
	hx83112a_diag_register_set(dev, diag_cmd, 0);
	hx83112a_get_DSRAM_data(dev, info_data, true);
	hx83112a_diag_register_set(dev, 0, 0);
...
static int prd_compare_rawdata(struct device *dev, char *buf, u8 type, uint32_t RAW[])
{
	...
	switch (type) {
	case OPEN_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "OPEN_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "OPEN_LOWER");
		break;
	case MICRO_OPEN_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "MICRO_OPEN_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "MICRO_OPEN_LOWER");
		break;
	case SHORT_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "SHORT_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "SHORT_LOWER");
		break;
	case RAW_DATA_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "RAWDATA_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "RAWDATA_LOWER");
		break;
	case JITTER_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "JITTER_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "JITTER_LOWER");
		break;
	case LPWG_RAW_DATA_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "LPWG_RAWDATA_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "LPWG_RAWDATA_LOWER");
		break;
	case LPWG_JITTER_TEST:
		touch_snprintf(upper_str, sizeof(upper_str), "LPWG_JITTER_UPPER");
		touch_snprintf(lower_str, sizeof(lower_str), "LPWG_JITTER_LOWER");
		break;
	default:
	...
	retval = hx83112a_get_limit(dev, upper_str, row_size, col_size, (int *)UpperImage);
	if (retval < 0) {
		TOUCH_E("failed to get upper limit\n");
		result = 0;
		goto exit;
	}
	retval = hx83112a_get_limit(dev, lower_str, row_size, col_size, (int *)LowerImage);
	if (retval < 0) {
		TOUCH_E("failed to get lower limit\n");
		result = 0;
		goto exit;
	}
	...
	if (type == RAW_DATA_TEST || type == OPEN_TEST || type == MICRO_OPEN_TEST
		...
		for (i = 0; i < row_size; i++) {
			for (j = 0; j < col_size; j++) {
				if ((RAW[i * col_size + j] < LowerImage[i][j]) ||
						(RAW[i * col_size + j] > UpperImage[i][j])) {
					result = 0;
					fail_num++;
	...
	if (type == JITTER_TEST || type == LPWG_JITTER_TEST) {
		for (i = 0; i < row_size; i++) {
			for (j = 0; j < col_size; j++) {
				if ((int16_t)RAW[i * col_size + j] > UpperImage[i][j] ||
						((int16_t)RAW[i * col_size + j] < LowerImage[i][j])) {
					result = 0;
					fail_num++;
					ret += touch_snprintf(W_Buf + ret, sizeof(W_Buf) - ret,
...
static int firmware_version_log(struct device *dev)
{
	...
	case TOUCH_MINIOS_MFTS_FLAT:
	case TOUCH_MINIOS_MFTS_CURVED:
		fw_ver_ret = hx83112a_read_FW_ver(dev);
		if (fw_ver_ret < 0) {
...
static ssize_t show_sd(struct device *dev, char *buf)
{
	...
	if (!open_ret || !micro_open_ret || !short_ret)
...
static ssize_t show_rawdata(struct device *dev, char *buf)
{
	...
	hx83112a_register_write(dev, FW_ADDR_CONTROL_IDLE_MODE, DATA_LEN_4, w_data, ADDR_LEN_4);
	touch_msleep(200);
...
int hx83112a_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	int ret = 0;
	TOUCH_TRACE();
