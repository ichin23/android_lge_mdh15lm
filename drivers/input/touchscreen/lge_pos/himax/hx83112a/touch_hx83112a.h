...
#define HIMAX_DRIVER_VER "1.2.2.3_LGE_DH40_04"
...
enum {
	...
	HX83112A_SWIPE_L = 2,
	HX83112A_SWIPE_R = 3,
	HX83112A_SWIPE_NUM = 4,
};

struct hx83112a_active_area {
	s16 x1;
	s16 y1;
	s16 x2;
	s16 y2;
...
struct hx83112a_swipe_data {
	u8 enable;
	u8 distance;
	u8 ratio_thres;
	u16 min_time;
	u16 max_time;
	struct hx83112a_active_area area;
	struct hx83112a_active_area start_area;
	u8 wrong_dir_thres;
	u8 init_ratio_chk_dist;
	u8 init_ratio_thres;
} __packed;

struct hx83112a_swipe_buf {
	struct hx83112a_swipe_data data[HX83112A_SWIPE_NUM];
} __packed;
...
extern int hx83112a_report_data_init(void);
extern uint8_t hx83112a_int_gpio_read(int pinnum);
extern void hx83112a_burst_enable(struct device *dev, uint8_t auto_add_4_byte);
extern int hx83112a_register_read(struct device *dev, uint32_t addr, uint32_t read_length, uint8_t *read_data, uint8_t addr_len);
extern int hx83112a_register_write(struct device *dev, uint32_t addr, uint32_t write_length, uint8_t *read_data, uint8_t addr_len);
extern void hx83112a_interface_on(struct device *dev);
extern void hx83112a_sense_on(struct device *dev, uint8_t FlashMode);
extern bool hx83112a_sense_off(struct device *dev, bool check_en);
extern void hx83112a_set_SMWP_enable(struct device *dev, uint8_t SMWP_enable);
extern int hx83112a_read_FW_ver(struct device *dev);
extern bool hx83112a_calculateChecksum(struct device *dev);
extern int hx83112a_assign_sorting_mode(struct device *dev, uint8_t *tmp_data);
extern int hx83112a_fts_ctpm_fw_upgrade_with_sys_fs_64k(struct device *dev, unsigned char *fw, int len);
extern void hx83112a_ic_reset(struct device *dev, uint8_t loadconfig, uint8_t int_off);
extern void hx83112a_touch_information(struct device *dev);
extern void hx83112a_SMWP_dbg_msg(struct device *dev);
#endif
