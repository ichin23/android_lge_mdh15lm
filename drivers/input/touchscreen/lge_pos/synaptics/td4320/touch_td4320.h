...
#ifndef LGE_TOUCH_td4320_H
#define LGE_TOUCH_td4320_H
...
struct td4320_coord_data {
	struct td4320_point point[MAX_LPWG_CODE];
} __packed;

struct td4320_swipe_coord_data {
...
struct td4320_lpwg_data {
	u8 buf[40]; // 160bit (GESTURE_INFO) + 160bit (GESTURE_INFO2)
	struct td4320_coord_data coord_data;
	struct td4320_swipe_coord_data swipe_coord_data;
};

struct td4320_touch_data {
	u8 index:4;
	u8 classification:4;
	u16 x_pos:12;
	u16 y_pos:12;
...
struct td4320_touch_info {
	struct td4320_touch_data data[MAX_FINGER];
	u8 gesture_detected;
	struct td4320_lpwg_data lpwg_data;
	u16 palm_detected;
...
struct td4320_failreason_info {
	u8 knock_on[LPWG_FAILREASON_MAX_BUFFER];
	u8 swipe[LPWG_FAILREASON_MAX_BUFFER];
} __packed;

struct td4320_blank_status {
	int prev;
	int curr;
};

struct td4320_active_area {
	s16 x1;
	s16 y1;
	s16 x2;
	s16 y2;
...
// LPWG Write td4320 BUF struct
struct td4320_tci_data {
	u16 enable;
	u16 tap_count;
	u16 min_intertap;
	u16 max_intertap;
...
struct td4320_tci_buf {
	u8 gesture_config_code;
	struct td4320_tci_data data;
} __packed;

struct td4320_swipe_data {
	u16 enable;
	u8 distance;
	u8 ratio_thres;
	u16 min_time;
	u16 max_time;
	struct td4320_active_area area;
	struct td4320_active_area start_area;
	u16 wrong_dir_thres;
	u8 init_ratio_chk_dist;
	u8 init_ratio_thres;
} __packed;

struct td4320_swipe_buf {
	u8 gesture_config_code;
	struct td4320_swipe_data data;
} __packed;

struct td4320_lpwg_abs_buf {
	u8 gesture_config_code;
	u16 enable;
	struct td4320_active_area area;
} __packed;

struct td4320_lpwg_longpress_buf {
	u8 gesture_config_code;
	u8 enable;
	u8 touch_slop;
	u8 min_press_time;
...
struct td4320_tci_active_area_buf {
	u8 gesture_config_code;
	struct td4320_active_area area;
} __packed;

// LPWG Ctrl struct
struct td4320_lpwg_abs_ctrl {
	u8 enable;
	struct td4320_active_area area;
	struct td4320_active_area border_area;
	s16 offset_y;
};

struct td4320_ai_pick_ctrl {
	u8 enable;
	struct td4320_active_area area;
	struct td4320_active_area border_area;
	struct td4320_active_area total_area;
};

struct td4320_lpwg_longpress_ctrl {
	u8 enable;
	int touch_slop;
	int min_press_time;
	...
	struct td4320_active_area around_area;
#endif
};

struct td4320_lpwg_onetap_ctrl {
	u8 enable;
	struct td4320_active_area area;
	int touch_slop;
	int max_press_time;
...
struct td4320_exp_fhandler {
	struct td4320_exp_fn *exp_fn;
	bool insert;
	bool initialized;
	bool remove;
};

struct td4320_id_info {
	u8 version;
	u8 mode;
	u8 part_number[16];
	u32 build_id;
 ...
 * Supplier/4bit	0:Korea 1:Korea & Longcheer 2:Longcheer
 * Inch0/4bit		Decimal
 * Inch1/4bit		Decimal
 * Panel/4bit		Decimal
 * Release(Major)/2bit		0:Korea 1:Korea & Longcheer 2:Longcheer
 * Version(Minor)/14bit		Decimal
 * ex) 0x6006510001 */
struct td4320_customer_config_id {
	u8 key:4;
	u8 maker:4;
	u8 inch0:4;
	u8 supplier:4;
...
struct td4320_app_info {
	...
	struct td4320_customer_config_id customer_config_id;
	u16 max_x;
	u16 max_y;
	u16 max_objects;
	u16 num_of_buttons;
...
struct td4320_bin_fw_version_info {
	u32 build_id;
	u8 release;
	u16 version;
} __packed;

struct td4320_boot_info {
	u8 packet_version;
	u8 status;
	u16 asic_id;
...
struct td4320_buffer {
	u8 buf[BUFFER_SIZE];
	u32 data_length;
	struct mutex buf_mutex;
};

struct td4320_message_header {
	u8 marker;
	u8 code;
	u16 length;
};
...
struct td4320_data {
	struct td4320_id_info id_info;
	struct td4320_app_info app_info;
	struct td4320_bin_fw_version_info bin_fw_version_info;
	struct td4320_boot_info boot_info;
	struct td4320_romboot_info romboot_info;
	struct td4320_touch_info touch_info;
	struct td4320_failreason_info failreason_info;
	struct td4320_state_info state;
	struct td4320_prd_info prd_info;
	struct td4320_lpwg_abs_ctrl lpwg_abs;
	struct td4320_ai_pick_ctrl ai_pick;
	struct td4320_lpwg_longpress_ctrl lpwg_longpress;
	struct td4320_lpwg_onetap_ctrl lpwg_onetap;

	struct td4320_buffer in;
	struct td4320_buffer out;
	struct td4320_buffer response;
	struct td4320_buffer report;
	struct td4320_buffer config;
	struct td4320_buffer temp;
	struct td4320_blank_status blank_status;
	...
	// touch_td4320_reflash.c
	struct mutex extif_mutex;

	// touch_td4320_device.c
	bool isDeviceOpen;
	int (*report_touch)(struct device *);

	// divide f/w by hw_revision
	u8 board_rev;
};

static inline struct td4320_data *to_td4320_data(struct device *dev)
{
	return (struct td4320_data *)touch_get_device(to_touch_core(dev));
}
...
bool td4320_is_product(struct td4320_data *d, const char *product_id, size_t len);
int td4320_set_dynamic_config(struct device *dev, u8 dynamic_config_id, u16 value);

/* extern function */
extern int td4320_fw_updater(const unsigned char *fw_data);
extern int td4320_get_bin_fw_version(const unsigned char *fw_data);
extern bool is_ddic_name(char *ddic_name);

#endif /* LGE_TOUCH_td4320_H */
