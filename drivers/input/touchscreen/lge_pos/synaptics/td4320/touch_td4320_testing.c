...
#include "touch_td4320_testing.h"
...
{ \
	...
	struct td4320_data *d = testing_hcd->d; \
\
	mutex_lock(&ts->lock); \
...
struct testing_hcd {
	...
	struct td4320_buffer resp;
	struct td4320_buffer report;
	struct td4320_buffer process;
	struct td4320_buffer output;
	struct td4320_buffer buffer;
...
static int testing_dynamic_range(struct device *dev);

static int testing_open_detection(struct device *dev);
...
static int firmware_version_log(struct device *dev)
{
	...
	case TOUCH_MINIOS_MFTS_FLAT:
	case TOUCH_MINIOS_MFTS_CURVED:
		ic_info_ret = td4320_ic_info(dev);
		if (ic_info_ret < 0) {
...
static int td4320_sd(struct device *dev, char *buf)
{
	struct td4320_data *d = to_td4320_data(dev);

	int ret = 0;
	int noise_ret = 0;
	int dynamic_range_ret = 0;
	int open_detection_ret = 0;
	int prd_id_ret = 0;
	...
	prd_id_ret = td4320_is_product(d, "PLG679", 6);

	mutex_lock(&d->extif_mutex);
	testing_dynamic_range(dev);
	dynamic_range_ret = testing_hcd->result;
	mutex_unlock(&d->extif_mutex);
	...
	mutex_lock(&d->extif_mutex);
	testing_open_detection(dev);
	open_detection_ret = testing_hcd->result;
	mutex_unlock(&d->extif_mutex);

	ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "\n========RESULT=======\n");
	ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "Channel Status : %s",
				open_detection_ret ? "Pass\n" : "Fail ");

	if (!open_detection_ret)
	{
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "(%s)\n",
					open_detection_ret != 1 ? "0" : "1");
	}

	ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "Raw Data : %s",
				(dynamic_range_ret && noise_ret && prd_id_ret)
				? "Pass\n" : "Fail ");

	if (!dynamic_range_ret || !noise_ret || !prd_id_ret) {
		ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "(%s/%s/%s)\n",
					(dynamic_range_ret != 1 ? "0" : "1"),
					(noise_ret != 1 ? "0" : "1"),
					(prd_id_ret != 1 ? "0" : "1"));
	}
	ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "=====================\n");
...
static int td4320_lpwg_sd(struct device *dev, char *buf)
{
	...
	ret += touch_snprintf(buf + ret, PAGE_SIZE - ret, "LPWG RawData : %s",
				(dynamic_range_ret && noise_ret)
				? "Pass\n" : "Fail ");

	if (!dynamic_range_ret || !noise_ret) {
...
static int enable_raw_delta_report(bool enable, char report_type)
{
	...
	if (enable) {
		retval = td4320_write_message(ts->dev,
						CMD_DISABLE_REPORT,
						&report_touch,
						1,
						d->response.buf,
		...
		retval = td4320_set_dynamic_config(ts->dev, DC_NO_DOZE, 1);
		if (retval < 0) {
		...
		retval = td4320_set_dynamic_config(ts->dev, DC_NO_DOZE, 0);
		if (retval < 0) {
			TOUCH_E("failed to set %s Enable (ret: %d)\n", STR(DC_NO_DOZE), retval);
			return -EINVAL;
		}
...
*/
	...
	ret = td4320_write_message(ts->dev,
			CMD_PRODUCTION_TEST,
			testing_hcd->out.buf,
			1,
...
static void testing_get_frame_size_words(unsigned int *size, bool image_only)
{
	...
	struct td4320_app_info *app_info = NULL;
	struct td4320_data *d = testing_hcd->d;
...
static ssize_t show_delta(struct device *dev, char *buf)
{
	...
	mutex_lock(&testing_hcd->buffer.buf_mutex);

	memcpy(testing_hcd->buffer.buf, d->report.buf, d->report.data_length);
	testing_hcd->buffer.data_length = d->report.data_length;

	mutex_unlock(&testing_hcd->buffer.buf_mutex);
	...
	p_data_16 = (short *)testing_hcd->buffer.buf;
...
static ssize_t show_rawdata(struct device *dev, char *buf)
{
	...
	ret = testing_run_prod_test_item(TEST_DYNAMIC_RANGE_PT7);
	if (ret < 0) {
	...
	idx = 0;
	p_data = testing_hcd->resp.buf;

	report_data = kzalloc(sizeof(unsigned short) * rows * cols, GFP_KERNEL);
	if (!report_data) {
		TOUCH_E("Failed to alloc memory for report_data");
		mutex_unlock(&testing_hcd->resp.buf_mutex);
		retval = -ENOMEM;
	...
	for (row = 0; row < rows; row++) {
		...
		for (col = 0; col < cols; col++) {
			i = row * cols + col;
			report_data[i] = (short)le2_to_uint(&p_data[idx * 2]);
			idx++;
			size += touch_snprintf(w_buf + size, sizeof(w_buf) - size, "%5d", report_data[i]);
			log_size += touch_snprintf(log_buf + log_size, LOG_BUF_SIZE - log_size, "%5d", report_data[i]);
		}
...
{
	struct td4320_data *d = testing_hcd->d;
	struct td4320_id_info *id_info = NULL;
	char *strptr = NULL;
...
exit:
	...
	if (d->lcd_mode == LCD_MODE_U3)
		td4320_reset_ctrl(ts->dev, SW_RESET_NO_INIT);
...
static int testing_dynamic_range(struct device *dev) {
	...
	TOUCH_I("Start Dynamic Range testing\n");
	testing_hcd->result = false;

	memset(w_buf, 0x00, sizeof(w_buf));

	size = touch_snprintf(w_buf, sizeof(w_buf), "[%sDynamic Range Test]\n",
				(d->lcd_mode != LCD_MODE_U3) ? "LPWG " : "");
	...
	limits_rows = sizeof(pt7_dynamic_range_hi_limits)
			/ sizeof(pt7_dynamic_range_hi_limits[0]);
	limits_cols = sizeof(pt7_dynamic_range_hi_limits[0])
			/ sizeof(pt7_dynamic_range_hi_limits[0][0]);
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			...
			if (tmp_buf[i] > pt7_dynamic_range_hi_limits[row][col] ||
					tmp_buf[i] < pt7_dynamic_range_lo_limits[row][col]) {
				testing_hcd->result = false;

				TOUCH_I("fail at (%-2d, %-2d) data = %d, limit = (%d - %d)\n",
					row, col, tmp_buf[i],
					pt7_dynamic_range_lo_limits[row][col],
					pt7_dynamic_range_hi_limits[row][col]);
				size += touch_snprintf(w_buf + size, sizeof(w_buf) - size,
...
static int testing_open_detection(struct device *dev) {
	...
	TOUCH_I("Start Open Detection testing\n");
	testing_hcd->result = false;

	memset(w_buf, 0x00, sizeof(w_buf));

	size = touch_snprintf(w_buf, sizeof(w_buf), "[Open Detection Test]\n");
	...
	ret = testing_run_prod_test_item(TEST_OPEN_DETECTION_PT11);
	if (ret < 0) {
		TOUCH_E("Failed to run test\n");
		goto exit;
	}
	...
	limits_rows = sizeof(pt11_open_detection_hi_limits)
			/ sizeof(pt11_open_detection_hi_limits[0]);
	limits_cols = sizeof(pt11_open_detection_hi_limits[0])
			/ sizeof(pt11_open_detection_hi_limits[0][0]);
	...
	for (row = 0; row < rows; row++) {
		for (col = 0; col < cols; col++) {
			...
			if (tmp_buf[i] > pt11_open_detection_hi_limits[row][col] ||
					tmp_buf[i] < pt11_open_detection_lo_limits[row][col]) {
				testing_hcd->result = false;

				TOUCH_I("fail at (%-2d, %-2d) data = %d, limit = (%d - %d)\n",
					row, col, tmp_buf[i],
					pt11_open_detection_lo_limits[row][col],
					pt11_open_detection_hi_limits[row][col]);
				size += touch_snprintf(w_buf + size, sizeof(w_buf) - size,
					"fail at (%-2d, %-2d) data = %d, limit = (%d - %d)\n",
					row, col, tmp_buf[i],
					pt11_open_detection_lo_limits[row][col],
...
//	result_pt1 = ret;  // copy the result of pt1
	...
	// do sw reset
//	td4320_reset_ctrl(ts->dev, SW_RESET);
...
static int testing_reset_open(struct device *dev)
{
	...
	if (d->id_info.mode == MODE_APPLICATION_FIRMWARE) {
		ret = td4320_switch_mode(ts->dev, MODE_TDDI_BOOTLOADER);
		if (ret < 0) {
...
//run_app_firmware:
	ret = td4320_switch_mode(ts->dev, MODE_APPLICATION_FIRMWARE);
	if (ret < 0) {
...
static int testing_init(struct device *dev)
{
	...
	mutex_init(&testing_hcd->buffer.buf_mutex);
...
static struct td4320_exp_fn testing_module = {
	.init = testing_init,
...
static int __init testing_module_init(void)
{
	//return syna_tcm_add_module(&testing_module, true);
		...
		td4320_testing_function(&testing_module, true);
...
static void __exit testing_module_exit(void)
{
	//syna_tcm_add_module(&testing_module, false);
		...
		td4320_testing_function(&testing_module, false);
