 ...
 */
#include "touch_td4320.h"

extern int lge_get_mfts_mode(void);

const char *td4320_lpwg_failreason_tci_str[LPWG_FAILREASON_TCI_NUM] = {
	"NONE",
	"DISTANCE_INTER_TAP",
	"DISTANCE_TOUCHSLOP",
	"TIMEOUT_INTER_TAP",
...
const char *td4320_lpwg_failreason_swipe_str[LPWG_FAILREASON_SWIPE_NUM] = {
	"NONE",
	"FAST_RELEASE",
	"MULTI_FINGER",
	"FAST_SWIPE",
...
const char *td4320_gesture_config_code_str[CONFIG_CODE_MAX] = {
...
//static struct td4320_exp_fhandler rmidev_fhandler;
static struct td4320_exp_fhandler reflash_fhandler;
static struct td4320_exp_fhandler testing_fhandler;
static struct td4320_exp_fhandler device_fhandler;


static int td4320_lpwg_mode(struct device *dev);
//static int td4320_report_control(struct device *dev, int enable, int report_type);
static int td4320_tci_active_area(struct device *dev, u16 x1, u16 y1, u16 x2, u16 y2);
extern int reflash_read_product_info(void);

bool td4320_is_product(struct td4320_data *d,
...
int td4320_read(struct device *dev, void *data, int size)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
*/
	if (size > CHUNK_SIZE) {
		TOUCH_I("td4320 read size overflow!!!");
		ret = -ENOMEM;
		goto exit;
	}
...
static int td4320_get_report_data(struct device *dev, u32 offset, u32 bits, u32 *data)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_parse_data(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0, bits = 0;
	u8 *config_data = NULL;
	...
	while (config_idx < config_size) {
		...
		case TOUCH_CUSTOMER_GESTURE_DETECTED:
			...
			config_idx++;
			ret = td4320_get_report_data(dev, offset, bits, &data);
			if (ret < 0) {
		...
		case TOUCH_CUSTOMER_GESTURE_INFO:
			...
			while (bits > 0) {
				if (buf_idx >= 20)
					break;
				td4320_get_report_data(dev, offset, 8, &data);
				if (ret < 0) {
...
static void td4320_lpwg_abs_filter(struct device *dev, u8 touch_id)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_report_abs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	...
	for (i = 0; i < MAX_FINGER; i++) {
		...
		if (d->touch_info.data[i].classification == FINGER) {
			...
			tdata->width_major = 10;
			tdata->width_minor = 10;
...
static int td4320_get_lpwg_data(struct device *dev, int count) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_coord_data *coord_data = NULL;
	int i = 0;
	...
	coord_data = (struct td4320_coord_data *)d->touch_info.lpwg_data.buf;
...
static int td4320_get_swipe_data(struct device *dev) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_swipe_coord_data *swipe_coord_data = NULL;
	char str[8] = {0};
	int ret = 0;

	TOUCH_TRACE();

	swipe_coord_data = (struct td4320_swipe_coord_data *)d->touch_info.lpwg_data.buf;
	...
	switch (swipe_coord_data->index) {
		...
		case td4320_SWIPE_R:
			...
			break;
		case td4320_SWIPE_L2:
			TOUCH_I("SWIPE_LEFT2\n");
			//touch_snprintf(str, sizeof(str), "LEFT2");
			ts->intr_status = TOUCH_IRQ_SWIPE_LEFT2;
			break;
		case td4320_SWIPE_R2:
...
static bool td4320_check_ai_pick_event(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_active_area *area = &d->ai_pick.total_area;
...
static int td4320_check_udf_sensor_inside(struct device *dev, int x, int y)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
#endif
static int td4320_report_lpwg(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	...
	switch (d->touch_info.gesture_detected) {
		case DETECT_KNOCK_ON:
			ret = td4320_get_lpwg_data(dev, ts->tci.info[TCI_1].tap_count);
			if (ret < 0) {
			...
			if (d->ai_pick.enable) {
				if (td4320_check_ai_pick_event(dev) && d->lcd_mode < LCD_MODE_U3) {
		...
		case DETECT_KNOCK_CODE:
			ret = td4320_get_lpwg_data(dev, ts->tci.info[TCI_2].tap_count);
			if (ret < 0) {
		...
		case DETECT_LONG_PRESS_DOWN:
			ret = td4320_get_lpwg_data(dev, 1);
			if (ret < 0) {
...
#if defined(USE_LONGPRESS_AROUND)
			if(td4320_check_udf_sensor_inside(dev, ts->lpwg.code[0].x, ts->lpwg.code[0].y)) {
...
static int td4320_report_touch(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
static int td4320_dispatch_report(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
static int td4320_dispatch_response(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_continued_read(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	u8 marker = 0, code = 0;
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		ret = td4320_read(dev, d->temp.buf, xfer_length + 2);
		if (ret < 0) {
...
int td4320_read_message(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_message_header *message_header = NULL;
	int ret = 0;
	bool retry = true;
	u32 total_length = 0;
...
retry:
	mutex_lock(&d->in.buf_mutex);
	ret = td4320_read(dev, d->in.buf, MESSAGE_HEADER_SIZE);
	if (ret < 0) {
	...
	message_header = (struct td4320_message_header *)d->in.buf;
...
check_padding:
	...
	if (d->status_report_code >= REPORT_IDENTIFY) {
		ret = td4320_dispatch_report(dev);
		if (ret < 0) {
			TOUCH_E("Failed to dispatch report\n");
			goto exit;
		}
	} else {
		ret = td4320_dispatch_response(dev);
		if (ret < 0) {
...
int td4320_write(struct device *dev, void *data, int size)
{
	//struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct i2c_client *client = to_i2c_client(dev);
...
*/
	if (size > CHUNK_SIZE) {
		TOUCH_I("td4320 write size overflow!!!");
		ret = -ENOMEM;
		goto exit;
	}
...
int td4320_write_message(struct device *dev, u8 command, u8 *payload, u32 length,
		u8 *response_buffer, u32 *response_length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		ret = td4320_write(dev, d->out.buf, xfer_length + 1);
		if (ret < 0) {
	...
	if (ret == 0) {
		...
		td4320_reset_ctrl(dev, HW_RESET);
		ret = -ETIME;
		goto exit;
	}
...
int td4320_identify(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
static int td4320_set_report_config(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	u32 idx = 0;
...
static int td4320_get_report_config(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
int td4320_set_dynamic_config(struct device *dev, u8 dynamic_config_id, u16 value)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	u8 payload[3] = {0,};
...
static int td4320_sleep_control(struct device *dev, int enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
*/
void td4320_reset_ctrl(struct device *dev, int ctrl)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
...
static int td4320_tci_enable(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct tci_info *info = NULL;
	struct td4320_tci_buf buf = {0, };
	...
	for (i = 0; i < 2; i++) {
		if ((ts->tci.mode & (1 << i)) == 0x0) {
			...
			TOUCH_I("Response: %d / Write Command: %s / Config Code: %s\n",
					d->response_code,
					STR(CMD_SET_LGE_GESTURE_CONFIG),
					td4320_gesture_config_code_str[gesture_config_code[i]]);
		} else {
			info = &ts->tci.info[i];
	...
	ret = td4320_tci_active_area(dev, 0 + LPWG_SENSELESS_AREA_W,
...
static int td4320_swipe_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct swipe_ctrl *ctrl[td4320_SWIPE_NUM] = {	/* U, L, R, L2, R2 */
		&ts->swipe[SWIPE_U],
		&ts->swipe[SWIPE_L],
		&ts->swipe[SWIPE_R],
		&ts->swipe[SWIPE_L2],
	...
	u8 gesture_config_code[td4320_SWIPE_NUM] = {
		CONFIG_CODE_SWIPE_UP,
		CONFIG_CODE_SWIPE_LEFT,
		CONFIG_CODE_SWIPE_RIGHT,
		CONFIG_CODE_SWIPE_LEFT2,
	...
	TOUCH_I("%s: Enable: %d SWIPE U(%d),L(%d),R(%d),L2(%d),R2(%d)\n", __func__,
			enable,
			ctrl[td4320_SWIPE_U]->enable,
			ctrl[td4320_SWIPE_L]->enable,
			ctrl[td4320_SWIPE_R]->enable,
			ctrl[td4320_SWIPE_L2]->enable,
			ctrl[td4320_SWIPE_R2]->enable);

	for (i = 0; i < td4320_SWIPE_NUM; i++) {	/* U, D, L, R */
		if (enable) {
...
static int td4320_lpwg_abs_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_lpwg_abs_buf buf = {0, };
	int ret = 0;
	...
	TOUCH_I("Response: %d / Write Command: %s / Config Code: %s\n",
			d->response_code,
			STR(CMD_SET_LGE_GESTURE_CONFIG),
			td4320_gesture_config_code_str[buf.gesture_config_code]);
...
static int td4320_ai_pick_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	...
	if (enable) {
		if (ts->lpwg.mode == LPWG_NONE) {
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				...
				td4320_sleep_control(dev, IC_NORMAL);
			}
			...
			ret = td4320_tci_enable(dev);
			if (ret < 0) {
				TOUCH_E("failed to set ai_pick gesture (ret: %d)\n", ret);
				goto error;
			}

			ret = td4320_tci_active_area(dev, d->ai_pick.total_area.x1,
...
static int td4320_lpwg_longpress_enable(struct device *dev, int enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_lpwg_longpress_buf buf = {0, };
	int ret = 0;
...
static int td4320_lpwg_onetap_enable(struct device *dev, bool enable)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_lpwg_onetap_buf buf = {0, };
	int ret = 0;
...
{
	struct td4320_data *d = to_td4320_data(dev);
	struct td4320_tci_active_area_buf buf = {0, };
	int ret = 0;
...
static int td4320_lpwg_control(struct device *dev,
		...
		int lpwg_longpress_enable, int lpwg_onetap_enable)
{
	...
	ret = td4320_swipe_enable(dev, swipe_enable);
	if (ret < 0) {
	...
	ret = td4320_lpwg_abs_enable(dev, lpwg_abs_enable);
	if (ret < 0) {
	...
	ret = td4320_ai_pick_enable(dev, ai_pick_enable);
	if (ret < 0) {
	...
	ret = td4320_lpwg_longpress_enable(dev, lpwg_longpress_enable);
	if (ret < 0) {
	...
	ret = td4320_lpwg_onetap_enable(dev, lpwg_onetap_enable);
	if (ret < 0) {
	...
	if (tci_mode || swipe_enable || lpwg_abs_enable || ai_pick_enable || lpwg_longpress_enable || lpwg_onetap_enable) {
		ret = td4320_set_dynamic_config(dev, DC_IN_WAKEUP_GESTURE_MODE, 1);
		if (ret < 0) {
		...
		ret = td4320_set_dynamic_config(dev, DC_IN_WAKEUP_GESTURE_MODE, 0);
		if (ret < 0) {
...
static void td4320_longpress_release(struct device *dev) {
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_lpwg_debug(struct device *dev, int mode)
{
	struct td4320_data *d = to_td4320_data(dev);
	u8 i = 0, failreason = 0;
	int ret = 0;
	...
	for (i = 0; i <= sizeof(d->failreason_info.knock_on); i++) {
		...
		TOUCH_I("TCI(%d)-Fail[%d] : %s\n", mode, i, td4320_lpwg_failreason_tci_str[failreason]);
	}
	...
	for (i = 0; i <= sizeof(d->failreason_info.swipe); i++) {
		...
		TOUCH_I("SWIPE-Fail[%d] : %s\n", i, td4320_lpwg_failreason_swipe_str[failreason]);
	}
error:
	return ret;
}
...
static int td4320_lpwg_mode(struct device *dev)
{
	...
	if (atomic_read(&ts->state.fb) == FB_SUSPEND) {
		...
		if (ts->lpwg.screen) {
			...
			if (atomic_read(&ts->state.sleep) == IC_DEEP_SLEEP) {
				ret = td4320_sleep_control(dev, IC_NORMAL);
				if (ret < 0) {
					TOUCH_E("failed to set IC_NORMAL (ret: %d)\n", ret);
					goto error;
				}
			}
			td4320_longpress_release(dev);
			ret = td4320_lpwg_debug(dev, ts->lpwg.mode);
			if (ret < 0) {
			...
			if (atomic_read(&ts->state.sleep) == IC_NORMAL) {
				ret = td4320_sleep_control(dev, IC_DEEP_SLEEP);
				if (ret < 0) {
...
static void td4320_init_tci_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static void td4320_init_swipe_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ts->swipe[SWIPE_L2].distance = 7;
	ts->swipe[SWIPE_L2].ratio_thres = 100;
	ts->swipe[SWIPE_L2].min_time = 0;
	ts->swipe[SWIPE_L2].max_time = 150;
	ts->swipe[SWIPE_L2].wrong_dir_thres = 2;
	ts->swipe[SWIPE_L2].init_ratio_chk_dist = 2;
	...
	ts->swipe[SWIPE_L2].area.y2 = 200;
	ts->swipe[SWIPE_L2].start_area.x1 = 0;
	ts->swipe[SWIPE_L2].start_area.y1 = 0;
	ts->swipe[SWIPE_L2].start_area.x2 = ts->caps.max_x;
	ts->swipe[SWIPE_L2].start_area.y2 = 200;
	ts->swipe[SWIPE_L2].border_area.x1 = 150;
	ts->swipe[SWIPE_L2].border_area.y1 = 75;
	ts->swipe[SWIPE_L2].border_area.x2 = 150;
	ts->swipe[SWIPE_L2].border_area.y2 = 300;
	ts->swipe[SWIPE_L2].start_border_area.x1 = 75;
	ts->swipe[SWIPE_L2].start_border_area.y1 = 75;
	ts->swipe[SWIPE_L2].start_border_area.x2 = 75;
	ts->swipe[SWIPE_L2].start_border_area.y2 = 150;

	ts->swipe[SWIPE_R2].enable = false;
	ts->swipe[SWIPE_R2].debug_enable = false;
	ts->swipe[SWIPE_R2].distance = 7;
	ts->swipe[SWIPE_R2].ratio_thres = 100;
	ts->swipe[SWIPE_R2].min_time = 0;
	ts->swipe[SWIPE_R2].max_time = 150;
	ts->swipe[SWIPE_R2].wrong_dir_thres = 2;
	ts->swipe[SWIPE_R2].init_ratio_chk_dist = 2;
	...
	ts->swipe[SWIPE_R2].area.y2 = 200;
	ts->swipe[SWIPE_R2].start_area.x1 = 0;
	ts->swipe[SWIPE_R2].start_area.y1 = 0;
	ts->swipe[SWIPE_R2].start_area.x2 = ts->caps.max_x;
	ts->swipe[SWIPE_R2].start_area.y2 = 200;
	ts->swipe[SWIPE_R2].border_area.x1 = 150;
	ts->swipe[SWIPE_R2].border_area.y1 = 75;
	ts->swipe[SWIPE_R2].border_area.x2 = 150;
	ts->swipe[SWIPE_R2].border_area.y2 = 300;
	ts->swipe[SWIPE_R2].start_border_area.x1 = 75;
	ts->swipe[SWIPE_R2].start_border_area.y1 = 75;
	ts->swipe[SWIPE_R2].start_border_area.x2 = 75;
	ts->swipe[SWIPE_R2].start_border_area.y2 = 150;
...
static void td4320_init_lpwg_abs_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static void td4320_init_ai_pick_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static void td4320_init_longpress_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static void td4320_init_onetap_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_irq_handler(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	...
	ret = td4320_read_message(dev);
	if (ret < 0) {
		TOUCH_E("td4320_read_message failed, ret : %d\n", ret);
		goto error;
	}
error:
...
static int td4320_drm_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
	struct touch_core_data *ts =
		container_of(self, struct touch_core_data, drm_notif);
	struct td4320_data *d = to_td4320_data(ts->dev);
...
static int td4320_fb_notifier_callback(struct notifier_block *self,
		unsigned long event, void *data)
{
...
void td4320_reflash_function(struct td4320_exp_fn *fn, bool insert)
{
	TOUCH_TRACE();
...
static int td4320_reflash_init(struct device *dev)
{
	int ret = 0;
...
void td4320_testing_function(struct td4320_exp_fn *fn, bool insert)
{
	TOUCH_TRACE();
...
static int td4320_testing_init(struct device *dev)
{
	int ret = 0;
...
void td4320_device_function(struct td4320_exp_fn *fn, bool insert)
{
	TOUCH_TRACE();
...
static int td4320_device_init(struct device *dev)
{
	int ret = 0;
...
static int td4320_power(struct device *dev, int ctrl)
{
	struct td4320_data *d = to_td4320_data(dev);
	...
	case POWER_OFF:
		...
		TOUCH_I("%s, off\n", __func__);
		atomic_set(&d->state.power, POWER_OFF);
#if 0	/* not power control(In-cell) */
		touch_gpio_direction_output(ts->reset_pin, 0);
		touch_msleep(1);
	...
	case POWER_SW_RESET:
		TOUCH_I("%s, sw reset\n", __func__);
		td4320_reset_ctrl(dev, SW_RESET);
		break;
...
static int td4320_esd_recovery(struct device *dev)
{
	TOUCH_TRACE();
	...
	TOUCH_I("td4320_esd_recovery!!\n");

	return 0;
}

static int td4320_suspend(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_resume(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_lpwg(struct device *dev, u32 code, void *param)
{
	struct touch_core_data *ts = to_touch_core(dev);
...
static int td4320_get_cmd_version(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int offset = 0;
	...
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset,
			" Config ID: 0x%x%x%x%x%x%x%x%x%02x\n",
			...
			d->app_info.customer_config_id.release,
			d->app_info.customer_config_id.ver_upper,
			d->app_info.customer_config_id.ver_lower);
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset,
			" Product ID: [%s]\n", str);
	offset += touch_snprintf(buf + offset, PAGE_SIZE - offset,
...
static int td4320_get_cmd_atcmd_version(struct device *dev, char *buf)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int offset = 0;
...
static void td4320_lcd_mode(struct device *dev, u32 mode)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_check_mode(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	...
	if (d->lcd_mode != LCD_MODE_U3) {
		if (d->lcd_mode == LCD_MODE_U2) {
			...
			if (d->prev_lcd_mode == LCD_MODE_U2_UNBLANK) {
				...
				ret = 1;
			} else if (d->prev_lcd_mode == LCD_MODE_U0) {
				TOUCH_I("U0 -> U0 mode\n");
				ret = 1;
			} else
		...
		if (d->prev_lcd_mode == LCD_MODE_U3) {
			TOUCH_I("U3 -> U3 mode\n");
			ret = 1;
		}
	}
...
static int td4320_debug_option(struct device *dev, u32 *data)
{
	u32 chg_mask = data[0];
	u32 enable = data[1];
	int ret = 0;
...
static int td4320_notify(struct device *dev, ulong event, void *data)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	...
	case LCD_EVENT_LCD_MODE:
		...
		td4320_lcd_mode(dev, *(u32 *)data);
		ret = td4320_check_mode(dev);
		if (ret == 0)
	...
	case NOTIFY_CONNECTION:
		...
		ret = td4320_set_dynamic_config(dev, DC_CHARGER_CONNECTED, *(u16 *)data);
		if (ret < 0) {
	...
	case NOTIFY_WIRELESS:
		...
		break;	// bring up
		ret = td4320_set_dynamic_config(dev, DC_WIRELESS_CHARGER_CONNECTED, *(u16 *)data);
		if (ret < 0) {
			TOUCH_E("failed to set %s %s (ret: %d)\n",
	...
	case NOTIFY_IME_STATE:
		...
		ret = td4320_set_dynamic_config(dev, DC_IME_MODE_ENABLE, *(u16 *)data);
		if (ret < 0) {
			TOUCH_E("failed to set %s %s (ret: %d)\n",
	...
	case NOTIFY_DEBUG_OPTION:
		...
		ret = td4320_debug_option(dev, (u32 *)data);
		if (ret < 0)
...
static int td4320_init_pm(struct device *dev)
{
#if defined(CONFIG_DRM_MSM)
#if defined(CONFIG_LGE_TOUCH_USE_PANEL_NOTIFY)
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ts->drm_notif.notifier_call = td4320_drm_notifier_callback;
#endif
#elif defined(CONFIG_FB)
	struct touch_core_data *ts = to_touch_core(dev);

	TOUCH_I("%s: fb_notif change\n", __func__);
	ts->fb_notif.notifier_call = td4320_fb_notifier_callback;
#endif

	return 0;
}
static void td4320_fb_notify_work_func(struct work_struct *fb_notify_work)
{
	struct td4320_data *d =
			container_of(to_delayed_work(fb_notify_work),
				struct td4320_data, fb_notify_work);
	int ret = 0;
		...
		const char *buf, size_t count)
{
	...
	mutex_lock(&ts->lock);
	ret = td4320_lpwg_abs_enable(dev, d->lpwg_abs.enable);
	if (ret < 0)
		...
		const char *buf, size_t count)
{
	...
	mutex_lock(&ts->lock);
	ret = td4320_ai_pick_enable(dev, d->ai_pick.enable);
	if (ret < 0)
		...
		const char *buf, size_t count)
{
	...
	ret = td4320_lpwg_longpress_enable(dev, d->lpwg_longpress.enable);
	if (ret < 0)
		...
		const char *buf, size_t count)
{
	...
	ret = td4320_lpwg_onetap_enable(dev, d->lpwg_onetap.enable);
	if (ret < 0)
...
static struct attribute *td4320_attribute_list[] = {
...
static const struct attribute_group td4320_attribute_group = {
	.attrs = td4320_attribute_list,
};

static int td4320_register_sysfs(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	...
	ret = sysfs_create_group(&ts->kobj, &td4320_attribute_group);
	if (ret < 0)
		TOUCH_E("td4320 sysfs register failed\n");

	//td4320_prd_register_sysfs(dev);

	return 0;
}
static int td4320_set(struct device *dev, u32 cmd, void *input, void *output)
{
	TOUCH_TRACE();
...
static int td4320_get(struct device *dev, u32 cmd, void *input, void *output)
{
	int ret = 0;
	...
	switch (cmd) {
		case CMD_VERSION:
			ret = td4320_get_cmd_version(dev, (char *)output);
			break;

		case CMD_ATCMD_VERSION:
			ret = td4320_get_cmd_atcmd_version(dev, (char *)output);
			break;
...
static int td4320_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_app_upgrade(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	const struct firmware *fw = NULL;
...
static void td4320_init_works(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	...
	INIT_DELAYED_WORK(&d->fb_notify_work, td4320_fb_notify_work_func);
}

static void td4320_init_locks(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
int td4320_switch_mode(struct device *dev, enum firmware_mode mode)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	int command = 0;
...
static int td4320_get_app_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	int timeout = 0;
...
retry:
	...
	d->app_info.customer_config_id.version = ((u16)(d->app_info.customer_config_id.ver_upper << 8) & 0xff00)
						| (u16)(d->app_info.customer_config_id.ver_lower & 0xff);

error:
	return ret;
}

static int td4320_get_boot_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	int timeout = 0;
...
static int td4320_get_romboot_info(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	int timeout = 0;
...
int td4320_ic_info(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	...
	switch (d->id_info.mode) {
		...
		case MODE_HOSTDOWNLOAD_FIRMWARE:
			...
			if (atomic_read(&ts->state.core) == CORE_PROBE) {
				...
				ret = td4320_get_bin_fw_version(fw->data);
				if (ret < 0) {
			...
			TOUCH_I(" Bin Version: v%d.%02d, Bin Build_id: %d\n",
					...
					d->bin_fw_version_info.version,
					d->bin_fw_version_info.build_id);
			TOUCH_I(" Config ID: 0x%x%x%x%x%x%x%x%x%02x\n",
					d->app_info.customer_config_id.maker,
					d->app_info.customer_config_id.key,
					d->app_info.customer_config_id.supplier,
					d->app_info.customer_config_id.inch0,
		...
		case MODE_ROMBOOTLOADER:
			...
			ret = td4320_get_romboot_info(dev);
			if (ret < 0) {
...
int td4320_init(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = to_td4320_data(dev);
	int ret = 0;
	...
	if (atomic_read(&ts->state.core) == CORE_PROBE) {
		...
		// Irq handler is not registered at probe time. Therefore, Read manually.
			...
			if (ret < 0) {
				...
				// F35 Check and Recovery but, not supported in td4320
				return -ENODEV;
			}
			atomic_set(&d->state.power, POWER_ON);
		}

		ret = td4320_reflash_init(dev);
		if (ret < 0) {
			TOUCH_E("Failed to init reflash module\n");
			goto error;
		}

		ret = td4320_testing_init(dev);
		if (ret < 0) {
			TOUCH_E("Failed to init testing module\n");
			goto error;
		}

		ret = td4320_device_init(dev);
		if (ret < 0) {
	...
	ret = td4320_set_report_config(dev);
	if (ret < 0) {
		TOUCH_E("Failed to set input parameters\n");
		goto error;
	}

	ret = td4320_get_report_config(dev);
	if (ret < 0) {
	...
	// Charger Status
	ret = td4320_set_dynamic_config(dev,
...
static int td4320_remove(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_shutdown(struct device *dev)
{
	struct td4320_data *d = to_td4320_data(dev);
...
static int td4320_probe(struct device *dev)
{
	struct touch_core_data *ts = to_touch_core(dev);
	struct td4320_data *d = NULL;
	...
	if (!d) {
		TOUCH_E("failed to allocate td4320 data\n");
		return -ENOMEM;
	}
	...
	td4320_init_works(dev);
	td4320_init_locks(dev);

	td4320_init_tci_info(dev);
	td4320_init_swipe_info(dev);
	td4320_init_lpwg_abs_info(dev);
	td4320_init_ai_pick_info(dev);
	td4320_init_longpress_info(dev);
	td4320_init_onetap_info(dev);
	pm_qos_add_request(&d->pm_qos_req, PM_QOS_CPU_DMA_LATENCY, PM_QOS_DEFAULT_VALUE);

	d->chunk_size = CHUNK_SIZE;
	d->lcd_mode = LCD_MODE_U3;
	d->report_touch = td4320_report_touch;

	d->board_rev = lge_get_board_revno();
	TOUCH_I("board_rev : %d\n", d->board_rev);
...
static struct touch_driver touch_driver = {
	.probe = td4320_probe,
	.remove = td4320_remove,
	.shutdown = td4320_shutdown,
	.suspend = td4320_suspend,
	.resume = td4320_resume,
	.init = td4320_init,
	.upgrade = td4320_upgrade,
#if defined(CONFIG_LGE_TOUCH_APP_FW_UPGRADE)
	.app_upgrade = td4320_app_upgrade,
#endif
	.irq_handler = td4320_irq_handler,
	.power = td4320_power,
	.lpwg = td4320_lpwg,
	.notify = td4320_notify,
	.init_pm = td4320_init_pm,
	.esd_recovery = td4320_esd_recovery,
	.swipe_enable = td4320_swipe_enable,
	.register_sysfs = td4320_register_sysfs,
	.set = td4320_set,
	.get = td4320_get,
};

#define MATCH_NAME			"synaptics,td4320"
...
static int __init touch_device_init(void)
{
	...
	if (is_lcm_name("TIANMA-TD4320")) {
		TOUCH_I("%s, TD4320 found!!! lcm_name = %s\n", __func__, lge_get_lcm_name());
	} else {
		TOUCH_I("%s, TD4320 not found.\n", __func__);
		return 0;
	}
