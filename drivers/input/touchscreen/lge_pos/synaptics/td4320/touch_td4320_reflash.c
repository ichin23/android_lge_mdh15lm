		...
		struct device_attribute *attr, const char *buf, size_t count)
{
	...
	struct td4320_data *d = reflash_hcd->d;
...
int reflash_read_product_info(void)
{
	...
	mutex_lock(&reflash_hcd->read.buf_mutex);
	memcpy(&d->prd_info, &reflash_hcd->read.buf[0], sizeof(d->prd_info));
...
run_app_firmware:
	...
	if (td4320_switch_mode(ts->dev, MODE_APPLICATION_FIRMWARE) < 0) {
...
static int reflash_do_reflash()
{
	...
	case FIRMWARE_CONFIG:
		...
		// FALL-THROUGH There is no break
	case CONFIG_ONLY:
...
int td4320_fw_updater(const unsigned char *fw_data)
{
	int ret = 0;
...
EXPORT_SYMBOL(td4320_fw_updater);

int td4320_get_bin_fw_version(const unsigned char *fw_data)
{
	...
	d->bin_fw_version_info.version = ((u16)(app_config_header->customer_config_id.ver_upper << 8) & 0xff00)
					| (u16)(app_config_header->customer_config_id.ver_lower & 0xff);

	reflash_hcd->image = NULL;
error:
	return ret;
}
EXPORT_SYMBOL(td4320_get_bin_fw_version);
...
static struct td4320_exp_fn reflash_module = {
	//.type = TCM_REFLASH,
	.init = reflash_init,
...
static int __init reflash_module_init(void)
{
		...
		td4320_reflash_function(&reflash_module, true);
...
static void __exit reflash_module_exit(void)
{
		...
		td4320_reflash_function(&reflash_module, false);
