...
static int device_raw_read(unsigned char *in_buf, unsigned int length)
{
	...
	struct td4320_data*d = device_hcd->d;
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = td4320_read(ts->dev, d->temp.buf, xfer_length + 2);
		if (retval < 0) {
				...
				unsigned char *data, unsigned int length)
{
	...
	for (idx = 0; idx < chunks; idx++) {
		...
		retval = td4320_write(ts->dev, d->out.buf, xfer_length + 1);
		if (retval < 0) {
...
static struct td4320_exp_fn device_module = {
//	.type = TCM_DEVICE,
	.init = device_init,
...
static int __init device_module_init(void)
{
		...
		td4320_device_function(&device_module, true);
...
static void __exit device_module_exit(void)
{
		...
		td4320_device_function(&device_module, false);
