...
//	u8 tci_fr_buffer[1 + TCI_FR_BUF_LEN], tci_fr_cnt, tci_fr;
	u8 swipe_fr_buffer[1 + SWIPE_FR_BUF_LEN];

	if (d->swipe_debug_type != SWIPE_DEBUG_BUFFER && d->swipe_debug_type != SWIPE_DEBUG_BUFFER_ALWAYS) {
	...
	// Swipe-on
	for (i = 0; i < 25; i++) {
		ret = ft8006p_reg_read(dev, 0xE7, &swipe_fr_buffer, sizeof(swipe_fr_buffer));
		if (!ret) {
			break;
		}
		msleep(2);
	...
	tci_fr_cnt = swipe_fr_buffer[0];
	if (tci_fr_cnt > SWIPE_FR_BUF_LEN) {
		TOUCH_I("Swipe-up Failure Reason Buffer Count Invalid\n");
	}
	else if (tci_fr_cnt == 0) {
		TOUCH_I("Swipe-on Failure Reason Buffer NONE\n");
	}
	else {
		for (i = 0; i < tci_fr_cnt; i++) {
			tci_fr = swipe_fr_buffer[1 + i];
			if (tci_fr < SWIPE_FR_NUM) {
				TOUCH_I("Swipe-up Failure Reason Buffer [%02d] : [%s]\n", i+1, swipe_debug_str[tci_fr]);
			}
			else {
				TOUCH_I("Swipe-up Failure Reason Buffer [%02d] : [%s]\n", i+1, swipe_debug_str[SWIPE_FR_NUM]);
			}
		}
	}
...
static int __init touch_device_init(void)
{
	...
	if(is_lcm_name("LCE-FT8006P"))//MH4
	{
