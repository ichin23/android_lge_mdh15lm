...
static void cpufreq_persistent_stats_free_table(unsigned int cpu)
{
	unsigned long irq_flags;
	struct cpu_persistent_stats *cpu_stats = &per_cpu(pcpu_stats, cpu);
...
static int cpufreq_persistent_stats_create_table(unsigned int cpu,
		...
		unsigned long val, void *data)
{
	...
	if (table_created_count < num_possible_cpus()) {
		for_each_cpu(cpu, policy->cpus) {
			if (cpufreq_persistent_stats_create_table(cpu, policy) == 0)
				table_created_count++;
		}
	}
...
static int cpu_pm_callback(struct notifier_block *nb,
...
static int __init cpufreq_persistent_stats_init(void)
{
	...
	if (ret)
		goto abort_register_cpu_pm_failed;
...
static void __exit cpufreq_persistent_stats_exit(void)
{
	...
	for_each_possible_cpu(cpu)
		cpufreq_persistent_stats_free_table(cpu);
