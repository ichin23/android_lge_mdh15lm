					 ...
					 unsigned long arg)
{
	...
	case ALSPS_GET_PS_MODE:
		pr_notice("ALSPS_GET_PS_MODE\n");
		if (alsps_factory.fops != NULL &&
		    alsps_factory.fops->ps_get_enable != NULL) {
			err = alsps_factory.fops->ps_get_enable(&data);
			if (err < 0) {
				pr_err(
...
#if defined(CONFIG_CUSTOM_OFFSET_CALIBRATION)
			pr_err("ps_offset = %d %x\n", data >> 16, data);
			data &= 0xffff;
#endif
			if (copy_to_user(ptr, &data, sizeof(data)))
				return -EFAULT;
