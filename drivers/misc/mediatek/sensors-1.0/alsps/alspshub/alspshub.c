...
long alspshub_read_ps_raw(u16 *ps)
{
	long res;
	struct data_unit_t data_t;
				...
				const char *buf, size_t count)
{
	...
	if (!obj) {
		pr_err("alspshub_store_trace obj_ipi_data is null!!\n");
		return 0;
	}
	if (sscanf(buf, "%d", &trace) == 1) {
		atomic_set(&obj->trace, trace);
		pr_err("alspshub_store_trace trace = %d \n", trace);
		res = sensor_set_cmd_to_hub(ID_PROXIMITY,
				CUST_ACTION_SET_TRACE, &trace);
		if (res < 0) {
			pr_err("alspshub_store_trace sensor_set_cmd_to_hub fail,(ID: %d),(action: %d)\n",
...
static ssize_t alspshub_store_threshold(struct device_driver *ddri,
				const char *buf, size_t count)
{
	int32_t threshold[2] = {0, 0};
	int32_t threshold_high = 0;
	...
	if (sscanf(buf, "%d %d", &threshold_high, &threshold_low) == 2) {
		atomic_set(&obj->ps_thd_val_high, threshold_high + obj->ps_cali);
		atomic_set(&obj->ps_thd_val_low, threshold_low + obj->ps_cali);

		threshold[0] = threshold_low;
		threshold[1] = threshold_high;
		pr_err("alspshub_store_threshold threshold_high = %d, threshold_low = %d \n", threshold[1], threshold[0]);
		res = sensor_set_cmd_to_hub(ID_PROXIMITY,
				CUST_ACTION_SET_PS_THRESHOLD, &threshold);
		if (res < 0) {
			pr_err("alspshub_store_threshold sensor_set_cmd_to_hub fail,(ID: %d),(action: %d)\n",
...
static ssize_t alspshub_store_pd_reset(struct device_driver *ddri,
				const char *buf, size_t count)
{
	int reset = 0;
	struct alspshub_ipi_data *obj = obj_ipi_data;
	...
	if (sscanf(buf, "%d", &reset) == 1) {
		pr_err("alspshub_store_pd_reset reset = %d \n", reset);
		reset = 999;
		res = sensor_set_cmd_to_hub(ID_PROXIMITY,
				CUST_ACTION_SET_TRACE, &reset);
		if (res < 0) {
			pr_err("alspshub_store_pd_reset sensor_set_cmd_to_hub fail,(ID: %d),(action: %d)\n",
				ID_PROXIMITY, CUST_ACTION_SET_TRACE);
			return 0;
		}
	} else {
...
static ssize_t alspshub_store_pd_select(struct device_driver *ddri,
				const char *buf, size_t count)
{
	int select = 0;
	struct alspshub_ipi_data *obj = obj_ipi_data;
	...
	if (sscanf(buf, "%d", &select) == 1) {
		pr_err("alspshub_store_pd_select select = %d \n", select);
		res = sensor_set_cmd_to_hub(ID_PROXIMITY,
				CUST_ACTION_SET_TRACE, &select);
		if (res < 0) {
			pr_err("alspshub_store_trace sensor_set_cmd_to_hub fail,(ID: %d),(action: %d)\n",
				ID_PROXIMITY, CUST_ACTION_SET_TRACE);
			return 0;
...
static ssize_t alspshub_show_ps(struct device_driver *ddri, char *buf)
{
	...
	if (res)
		...
		return snprintf(buf, PAGE_SIZE, "PS - Cal = %d\n", obj->ps);
}
...
static ssize_t alspshub_show_ps_raw(struct device_driver *ddri, char *buf)
{
	...
	res = alspshub_read_ps_raw(&obj->ps);
	if (res)
		return snprintf(buf, PAGE_SIZE, "ERROR: %d\n", (int)res);
	else
...
static DRIVER_ATTR(ps_raw, 0644, alspshub_show_ps_raw, NULL);
...
static DRIVER_ATTR(threshold, 0644, NULL,	alspshub_store_threshold);
static DRIVER_ATTR(pd_reset, 0644, NULL,	alspshub_store_pd_reset);
static DRIVER_ATTR(pd_select, 0644, NULL,	alspshub_store_pd_select);
...
static struct driver_attribute *alspshub_attr_list[] = {
	...
	&driver_attr_pd_reset,
	&driver_attr_pd_select,
	&driver_attr_alslv,
	&driver_attr_alsval,
	&driver_attr_reg,
};
...
static int pshub_factory_get_enable(int32_t *data)
{
	int status = 0;
	struct alspshub_ipi_data *obj = obj_ipi_data;
...
static struct alsps_factory_fops alspshub_factory_fops = {
	...
	.ps_get_enable = pshub_factory_get_enable,
