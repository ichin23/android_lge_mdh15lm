...
static int inpocket_flush(void)
{
	return sensor_flush_to_hub(ID_IN_POCKET);
}
static int inpocket_recv_data(struct data_unit_t *event, void *reserved)
{
	...
	if (event->flush_action == FLUSH_ACTION)
		err = situation_flush_report(ID_IN_POCKET);
	else if (event->flush_action == DATA_ACTION)
		err = situation_data_report(ID_IN_POCKET, event->inpocket_event.state);
	return err;
}
