...
int tdmb_ldo_power_on(void)
{
    ...
    if(fc8080_ctrl_info.dmb_ldo_en > 0) {
        ...
        printk("%s: tdmb_ldo_en_gpio_value to 1\n", __func__);
        mdelay(10);
    }
    return rc;
}
...
void tdmb_ldo_power_off(void)
{
    if(fc8080_ctrl_info.dmb_ldo_en > 0) {
        ...
        printk("%s: tdmb_ldo_en_gpio_value to 0\n", __func__);
        mdelay(10);
    }
...
void tdmb_lna_power_off(void)
{
    if(fc8080_ctrl_info.dmb_lna_gc > 0) {
        if(fc8080_ctrl_info.dmb_lna_en > 0) {
            gpio_set_value(fc8080_ctrl_info.dmb_lna_gc, 0); // OFF mode
        } else {
            gpio_set_value(fc8080_ctrl_info.dmb_lna_gc, 1); // Bypass mode
        }
    }
