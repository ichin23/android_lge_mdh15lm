...
#ifdef CONFIG_LGE_VIBRATOR_TEST
		...
		struct device_attribute *attr, const char *buf, size_t size)
{
	...
	if (level < VOL_1_2 || level > VOL_3_3)
		goto error;
	...
	vibr_set_power_level((unsigned int)hw->vib_vol);
	return size;
...
static DEVICE_ATTR(level, 0660, vibr_level_show, vibr_level_store);

static struct attribute *level_attrs[] = {
	&dev_attr_level.attr,
	NULL,
};

static struct attribute_group level_group = {
	.attrs = level_attrs,
};

static ssize_t vibr_on_store(struct device *dev,
		struct device_attribute *attr, const char *buf, size_t size)
{
	if (buf != NULL && size != 0) {
		if (buf[0] == '0')
...
static DEVICE_ATTR(vibr_on, 0220, NULL, vibr_on_store);

static struct attribute *vibr_on_attrs[] = {
	&dev_attr_vibr_on.attr,
	NULL,
};

static struct attribute_group vibr_on_group = {
	.attrs = vibr_on_attrs,
};
#endif
