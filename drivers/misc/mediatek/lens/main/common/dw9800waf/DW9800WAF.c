...
int DW9800WAF_Release(struct inode *a_pstInode, struct file *a_pstFile)
{
	...
	if (*g_pAF_Opened == 2) {
        ...
        unsigned long af_step = 30;
        unsigned long nextPosition = 0;
        unsigned long endPos = 512;
        // >512 multiple times
        // first step move fast
		LOG_INF("g_pAF_Opened g_u4CurrPosition=%lu \n",g_u4CurrPosition);
        if (g_u4CurrPosition > endPos) {
			/*
            if (g_u4CurrPosition > 800) {
                nextPosition = 800;
                LOG_INF("0:nextPosition = %lu g_u4CurrPosition = %lu g_u4AF_MACRO = %lu", nextPosition, g_u4CurrPosition,g_u4AF_MACRO);
...
*/
            while (g_u4CurrPosition > endPos + af_step) {
                if (g_u4CurrPosition > 700) {
                 ...
                 LOG_INF("1:af_step = %lu g_u4CurrPosition = %lu", af_step, g_u4CurrPosition);
                 if (s4AF_WriteReg((unsigned short)nextPosition) == 0) {
                     g_u4CurrPosition = nextPosition;
                     ret = 0;
                 } else {
			  ...
			  spinlock_t *pAF_SpinLock, int *pAF_Opened)
{
	...
	LOG_INF("DW9800WAF_SetI2Cclient Start\n");
