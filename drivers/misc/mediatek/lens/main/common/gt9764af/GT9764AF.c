...
{
	LOG_INF("gt9764 initAF:+\n");

	if (*g_pAF_Opened == 1) {

		int i4RetValue = 0;
		char puSendCmd[2] = {0x02, 0x02}; /* AAC mode */
		...
		if (i4RetValue < 0) {
			LOG_INF("gt9764 I2C send 0x00 failed!!\n");
			return -1;
		}
		...
		if (i4RetValue < 0) {
			LOG_INF("gt9764 I2C send 0x01 failed!!\n");
			return -1;
		}
		...
		if (i4RetValue < 0) {
			LOG_INF("gt9764 I2C send 0x05 failed!!\n");
			return -1;
		}

		LOG_INF("gt9764 driver init success!!\n");
...
static inline int moveAF(unsigned long a_u4Position)
{
	...
	if (s4AF_WriteReg((unsigned short)a_u4Position) == 0) {
		...
		LOG_INF("gt9764 set I2C failed when moving the motor\n");
		ret = -1;
	}
			...
			unsigned int a_u4Command, unsigned long a_u4Param)
{
	...
	case AFIOC_G_MOTORINFO:
		//LOG_INF("GT9764AF_Ioctl():AFIOC_G_MOTORINFO = %d\n", AFIOC_G_MOTORINFO);
		i4RetValue = getAFInfo((__user struct stAF_MotorInfo *) (a_u4Param));
		break;

	case AFIOC_T_MOVETO:
		//LOG_INF("GT9764AF_Ioctl():AFIOC_T_MOVETO = %d\n", AFIOC_T_MOVETO);
		i4RetValue = moveAF(a_u4Param);
		break;

	case AFIOC_T_SETINFPOS:
		//LOG_INF("GT9764AF_Ioctl():AFIOC_T_SETINFPOS = %d\n", AFIOC_T_SETINFPOS);
		i4RetValue = setAFInf(a_u4Param);
		break;

	case AFIOC_T_SETMACROPOS:
		//LOG_INF("GT9764AF_Ioctl():AFIOC_T_SETMACROPOS = %d\n", AFIOC_T_SETMACROPOS);
