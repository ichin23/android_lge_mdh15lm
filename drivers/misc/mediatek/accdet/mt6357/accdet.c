...
extern int es9218_get_status(void);
extern int es9218_set_status(int mode);
enum es9218_chipset_mode {
    E_es9218_STATUS_STANDBY,
    E_es9218_STATUS_AUX,
    E_es9218_STATUS_HIFI
};
#endif
...
static u32 button_press_debounce = 0x210; //LGE Modification
...
#ifdef CONFIG_LGE_ACCDET
static bool auxCableInt = false;
static struct switch_dev accdet_data;
#endif

#ifdef CONFIG_LGE_ACCDET_NEW_4POLE_TH
#define FOUR_POLE_THRESHOLD 30
unsigned int micVol = 0;
#endif
...
static void send_accdet_status_event(u32 cable_type, u32 status)
{
	...
	case HEADSET_NO_MIC:
#ifdef CONFIG_LGE_ACCDET
		if(HEADSET_MIC == accdet_data.state && status == 1){
			input_report_switch(accdet_input_dev, SW_MICROPHONE_INSERT, 0);
			input_report_switch(accdet_input_dev, SW_HEADPHONE_INSERT, 0);
		}
#endif
	...
	case HEADSET_MIC:
		 ...
		 */
#ifdef CONFIG_LGE_ACCDET
		if(HEADSET_NO_MIC == accdet_data.state && status == 1){
			input_report_switch(accdet_input_dev, SW_HEADPHONE_INSERT, 0);
		}
		input_report_switch(accdet_input_dev, SW_HEADPHONE_INSERT, status);
		input_report_switch(accdet_input_dev, SW_MICROPHONE_INSERT, status);
#else
		if (status == 0)
			input_report_switch(accdet_input_dev,
...
#if defined(CONFIG_MOISTURE_INT_SUPPORT) || defined(CONFIG_MOISTURE_EXT_SUPPORT)
	case MOISTURE:
		input_report_switch(accdet_input_dev, MOISTURE_DEVICE,
			status);
		input_sync(accdet_input_dev);
		pr_info("%s Moisture Device %s\n", __func__,
			status ? "PlugIn" : "PlugOut");
		break;
#endif
#endif
...
#ifdef CONFIG_LGE_ACCDET
	if(auxCableInt && (accdet_dts.mic_mode == HEADSET_MODE_1)){
	    auxCableInt = false;
		pmic_write(AUDENC_ANA_CON10, pmic_read(AUDENC_ANA_CON10) ^ RG_ACCDET_MODE_ANA10_AUXINT);
	}
#endif
}
...
{
#ifndef CONFIG_MTK_SND_HPIMP
	int impedance = 0;
#endif
#ifdef CONFIG_SND_SOC_ES9218P
	bool hifi_dl_status = false;
#endif
...
#endif
			if (es9218_get_status() == E_es9218_STATUS_HIFI)
			{
				hifi_dl_status = true;
				es9218_set_status(E_es9218_STATUS_AUX);
				pr_info("%s(), Set es9218 to aux mode before detect impedance\n", __func__);
			}
#if defined(CONFIG_LGE_HIFI_HWINFO)
		}
#endif
#endif
#ifndef CONFIG_MTK_SND_HPIMP
		impedance = aux_hp_impedance();
		pr_debug("accdet %s(),impedance : %d\n", __func__, impedance);
#endif
#ifdef CONFIG_SND_SOC_ES9218P
#if defined(CONFIG_LGE_HIFI_HWINFO)
		if (lge_get_hifi_hwinfo()) {
			...
			if (hifi_dl_status)
			{
				hifi_dl_status = false;
				es9218_set_status(E_es9218_STATUS_HIFI);
				pr_info("%s(), Restore es9218 to hifi mode after detect impedance\n", __func__);
			}
...
#endif
#ifdef CONFIG_LGE_ACCDET
		if((ACCDET_STATE_AB_MASK & (pmic_read(ACCDET_STATE_RG) >> ACCDET_STATE_MEM_IN_OFFSET)) == ACCDET_STATE_AB_11 
		    && (accdet_dts.mic_mode == HEADSET_MODE_1)){
		    auxCableInt = true;
			pmic_write(AUDENC_ANA_CON10, pmic_read(AUDENC_ANA_CON10) | RG_ACCDET_MODE_ANA10_AUXINT);
		}
#endif
...
#ifdef CONFIG_LGE_ACCDET_NEW_4POLE_TH
	if((cur_AB == ACCDET_STATE_AB_00) && (accdet_status == PLUG_OUT) && (micVol > FOUR_POLE_THRESHOLD)) {
		pr_info("Forcibly change AB state to 01\n");
		cur_AB = ACCDET_STATE_AB_01;
	}
#endif
...
#ifdef CONFIG_LGE_ACCDET_NEW_4POLE_TH
	if (cur_eint_state == EINT_PIN_PLUG_IN && accdet_status == PLUG_OUT) {
		micVol = accdet_get_auxadc(0);
	}
#endif
...
#ifdef CONFIG_LGE_ACCDET
			send_accdet_status_event(MOISTURE, 0);
#endif
...
#ifdef CONFIG_LGE_ACCDET
				send_accdet_status_event(MOISTURE, 1);
#endif
				clear_accdet_eint(eintID);
				clear_accdet_eint_check(eintID);
...
#ifdef CONFIG_LGE_ACCDET
	pmic_write(AUDENC_ANA_CON10,
		pmic_read(AUDENC_ANA_CON10) | RG_ACCDET_MODE_ANA10_FASTDISCHARGER);
#endif
...
int mt_accdet_probe(struct platform_device *dev)
{
...
#ifdef CONFIG_LGE_ACCDET
	if (accdet_cdev == NULL) {
		pr_notice("%s accdet_cdev cdev_alloc fail!\n", __func__);
		goto err_chrdevregion;
	}
#endif
	...
	if (ret) {
		pr_err("[Accdet]switch_dev_register returned:%d!\n", ret);
		return 1;
	}
#endif
