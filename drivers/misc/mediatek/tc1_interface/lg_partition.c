...
static struct file *tc1_file_open(const char *path, int flags, int rights)
{
	...
	if (IS_ERR(filp)) {
		err = PTR_ERR(filp);
		pr_info("filp open fail! errno=%d\n", err);
		return NULL;
	}
...
static int tc1_file_seek(struct file *file, unsigned int offset)
{
	...
	if (offset > LGE_FAC_MAX_OFFSET) {
		pr_info("Wrong index, seek fail!\n");
		return -1;
	}

	pos = offset * EMMC_BLOCK_SIZE;
	ret = vfs_setpos(file, file->f_pos + pos,
			LGE_FAC_MAX_OFFSET * EMMC_BLOCK_SIZE);
				...
				unsigned int length)
{
	...
	ret = get_misc2_path();
	if (ret == 1) {
		pr_info("get MISC2 partition info fail!\n");
		return -1;
	}

	filp = tc1_file_open(misc2_path, O_RDWR, 0660);
	if (filp == NULL) {
		pr_info("Open MISC2 partition fail\n");
		return -1;
	}
	...
	if (ret != offset * EMMC_BLOCK_SIZE) {
		pr_info("seek fail! ret[%d]\n", ret);
		filp_close(filp, NULL);
		kfree(tmp);
		return 0;
	}
				...
				unsigned int length)
{
	...
	filp = tc1_file_open(misc2_path, O_RDONLY, 0440);
	if (filp == NULL) {
		pr_info("Open MISC2 partition fail\n");
		return -1;
	}
...
int LGE_API_test(void)
{
	...
	for (index = 0; index < LGE_FAC_WIFI_MAC_ADDR_LEN; index++)
		buff[index] = LGE_FAC_WIFI_MAC_ADDR_OFFSET + index + 3;

	snprintf(temp, LGE_FAC_WIFI_MAC_ADDR_LEN * 3 + 4,
		 "%02X %02X %02X %02X %02X %02X %02X",
		 buff[0], buff[1], buff[2], buff[3], buff[4], buff[5], buff[6]);

	pr_info("write mac : %s\n", temp);

	LGE_FacWriteWifiMacAddr(buff, 1);
