	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclients[I2C_DEV_IDX_2]->addr = 0x58;
	spin_unlock(&g_spinLock);
	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclients[I2C_DEV_IDX_3]->addr = 0x58;
	spin_unlock(&g_spinLock);
...
#endif
{
	...
	int index = 0;
	mdh5lm_otp_data_t* psl556_otp_data = NULL;
	mdh5lm_otp_data_t* psl556_3M_otp_data = NULL;
	...
	case CAM_CALIOC_G_READ:
        ...
        if(ptempbuf->sensorID == 0x556) {
            psl556_otp_data = &sl556_otp_data;
            ...
            switch(ptempbuf->u4Offset){
                ...
                case MDH5LM_AWB_5100K_FLAG:
                    *pu1Params = psl556_otp_data->awb_5100k_flag;
                    break;
                case MDH5LM_AWB_3100K_FLAG:
                    *pu1Params = psl556_otp_data->awb_3100k_flag;
                    break;
                case MDH5LM_LSC_5100K_FLAG:
                    *pu1Params = psl556_otp_data->lsc_5100k_flag;
                    break;
                case MDH5LM_LSC_4000K_FLAG:
                    *pu1Params = psl556_otp_data->lsc_4000k_flag;
                    break;

                case MDH5LM_AWB_5100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->awb_5100k.data[index];
                    }
                    break;

                case MDH5LM_AWB_3100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->awb_3100k.data[index];
                    }
                    break;

                case MDH5LM_LSC_5100K_R_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_5100k_r.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_GR_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_5100k_gr.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_GB_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_5100k_gb.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_B_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_5100k_b.data[index];
                    }
                    break;

                case MDH5LM_LSC_4000K_R_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_4000k_r.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_GR_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_4000k_gr.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_GB_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_4000k_gb.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_B_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_otp_data->lsc_4000k_b.data[index];
                    }
                    break;

                default:
                    pr_err("No menu SL556 OTP:bit error");
                    i4RetValue = 0;
                    //i4RetValue = -EPERM;
                    break;
            }
        } 
		else if(ptempbuf->sensorID == 0x558) {
            psl556_3M_otp_data = &sl556_3M_otp_data;
            ...
            switch(ptempbuf->u4Offset){
                ...
                case MDH5LM_AWB_5100K_FLAG:
                    *pu1Params = psl556_3M_otp_data->awb_5100k_flag;
                    break;
                case MDH5LM_AWB_3100K_FLAG:
                    *pu1Params = psl556_3M_otp_data->awb_3100k_flag;
                    break;
                case MDH5LM_LSC_5100K_FLAG:
                    *pu1Params = psl556_3M_otp_data->lsc_5100k_flag;
                    break;
                case MDH5LM_LSC_4000K_FLAG:
                    *pu1Params = psl556_3M_otp_data->lsc_4000k_flag;
                    break;

                case MDH5LM_AWB_5100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->awb_5100k.data[index];
                    }
                    break;

                case MDH5LM_AWB_3100K_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->awb_3100k.data[index];
                    }
                    break;

                case MDH5LM_LSC_5100K_R_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_5100k_r.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_GR_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_5100k_gr.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_GB_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_5100k_gb.data[index];
                    }
                    break;
                case MDH5LM_LSC_5100K_B_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_5100k_b.data[index];
                    }
                    break;

                case MDH5LM_LSC_4000K_R_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_4000k_r.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_GR_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_4000k_gr.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_GB_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_4000k_gb.data[index];
                    }
                    break;
                case MDH5LM_LSC_4000K_B_DATA:
                    for(index = 0 ; index < ptempbuf->u4Length ; index++){
                        *(pu1Params+index) = psl556_3M_otp_data->lsc_4000k_b.data[index];
                    }
                    break;

                default:
                    pr_err("No menu SL556_3M OTP:bit error");
                    i4RetValue = 0;
                    //i4RetValue = -EPERM;
                    break;
            }
