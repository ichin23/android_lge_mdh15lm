			      ...
			      struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	pr_err("__Shamyu__ %s end! deviceID=%d index=%u client=%p\n",
	...
	struct stCAM_CAL_CMD_INFO_STRUCT *cmdInfo)
{
	...
	if (pCamCalList != NULL) {
		pr_err("pCamCalList!=NULL && pCamCalFunc!= NULL\n");
		for (i = 0; pCamCalList[i].sensorID != 0; i++) {
			if (pCamCalList[i].sensorID == sensorID) {
				pr_err("pCamCalList[%d].sensorID==%x\n", i,
	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclients[I2C_DEV_IDX_2]->addr = 0x52;
	spin_unlock(&g_spinLock);
	...
	(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	g_pstI2Cclients[I2C_DEV_IDX_3]->addr = 0x0c;
	spin_unlock(&g_spinLock);
...
#endif
{
	...
	case CAM_CALIOC_S_WRITE:	/*Note: Write Command is Unverified! */
		pr_err("CAM_CALIOC_S_WRITE start!\n");
#ifdef CAM_CALGETDLT_DEBUG
		...
		pr_err("Write data %d bytes take %lu us\n",
			ptempbuf->u4Length, TimeIntervalUS);
#endif
		pr_err("CAM_CALIOC_S_WRITE End!\n");
		break;
	...
	case CAM_CALIOC_G_READ:
		...
		pr_err("Read data %d bytes take %lu us\n",
			ptempbuf->u4Length, TimeIntervalUS);
#endif
		break;
...
static inline int EEPROM_chrdev_register(void)
{
	...
	pr_err("EEPROM_chrdev_register Start\n");
	...
	if (register_chrdev_region(g_devNum, 1, CAM_CAL_DRV_NAME)) {
		pr_err("Register device no failed\n");
		return -EAGAIN;
	}
#endif
	...
	device = device_create(g_drvClass, NULL, g_devNum, NULL,
		CAM_CAL_DRV_NAME);
	pr_err("EEPROM_chrdev_register End\n");
...
static int __init EEPROM_drv_init(void)
{
	...
	if (platform_driver_register(&g_stEEPROM_HW_Driver)) {
		pr_err("failed to register EEPROM driver i2C main\n");
		return -ENODEV;
	}

	if (platform_device_register(&g_platDev)) {
		pr_err("failed to register EEPROM device");
		return -ENODEV;
	}
