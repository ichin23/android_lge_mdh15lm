...
/* 2017-04-04 shsh.kim@lge.com LGP_DATA_TCPIP_IPV6_MTU_MTK [START] */
/* We initialize the driver mtu value when interface is down
because there is a mismatch problem between driver MTU and net device MTU.
http://mlm.lge.com/di/browse/PPAL-2649 */
	patch_code_id("LPCP-1932@y@m@vmlinux@ccmni.c@1");
	dev->mtu = CCMNI_MTU;
/* 2017-04-04 shsh.kim@lge.com LGP_DATA_TCPIP_IPV6_MTU_MTK [END] */
	if (unlikely(ccmni_ctl == NULL)) {
	...
	enum HIF_STATE state, int is_ack)
{
	...
	case RX_IRQ:
		...
		napi_schedule(ccmni->napi);
		/* 2016-11-22, global-mtk-data@lge.com LGP_DATA_ALPS03002265 [START] */
		//ALPS04235837
		//wake_lock_timeout(&ctlb->ccmni_wakelock, HZ);
		patch_code_id("LPCP-2159@y@m@vmlinux@ccmni.c@1");
		__pm_wakeup_event(&ctlb->ccmni_wakelock, jiffies_to_msecs(HZ));
		/* 2016-11-22, global-mtk-data@lge.com LGP_DATA_ALPS03002265 [END] */
		break;
#endif
