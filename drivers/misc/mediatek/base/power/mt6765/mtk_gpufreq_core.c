...
#ifdef CONFIG_MACH_MT6765_DH40LM
...
#define GET_PMIC (0)
#define GET_GPU_IN (1)
static unsigned int __mt_gpufreq_get_cur_volt(int flag);
#else
...
#ifdef CONFIG_LGE_PM_FOUR_OPP
static struct g_opp_table_info g_opp_table_sample[] = {
GPUOP(680000, SEG3_GPU_DVFS_VOLT0, SEG3_GPU_DVFS_VSRAM0, 0),
GPUOP(600000, SEG3_GPU_DVFS_VOLT0, SEG3_GPU_DVFS_VSRAM0, 1),
GPUOP(500000, SEG3_GPU_DVFS_VOLT1, SEG3_GPU_DVFS_VSRAM1, 2),
GPUOP(400000, SEG3_GPU_DVFS_VOLT2, SEG3_GPU_DVFS_VSRAM2, 3),
};
#endif
...
unsigned int mt_gpufreq_voltage_enable_set(unsigned int enable)
{
	...
	if (enable == 1) {
		if (!g_volt_enable_state) {
			...
			gpufreq_pr_debug("@%s: VGPU is already on (ref++)\n", __func__);
		}
		g_volt_enable_state++;
	} else if (enable == 0)  {
		if (!g_volt_enable_state) {
			gpufreq_pr_debug("@%s: VGPU is already off\n", __func__);
			mutex_unlock(&mt_gpufreq_lock);
			return 0;
		}
		g_volt_enable_state--;
...
#ifdef MT_GPUFREQ_STATIC_PWR_READY2USE
#ifdef CONFIG_MACH_MT6765_DH40LM
	unsigned int cur_vcore = __mt_gpufreq_get_cur_volt(GET_GPU_IN) / 100;
#else
...
#ifdef CONFIG_MACH_MT6765_DH40LM
	seq_printf(m, "real freq = %d, real volt = %d (%d), real vsram_volt = %d\n",
			__mt_gpufreq_get_cur_freq(),
			__mt_gpufreq_get_cur_volt(GET_GPU_IN),
			__mt_gpufreq_get_cur_volt(GET_PMIC),
#else
		...
		const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf(buf, "%d %d", &fixed_freq, &fixed_volt) == 2) {
		...
		if ((fixed_freq == 0) && (fixed_volt == 0)) {
			...
			if (!g_fixed_freq_volt_state) {
				/* Enter debug state: enable vcore add ref count*/
				mt_gpufreq_voltage_enable_set(1);
			}
#else
		} else {
...
static void __mt_gpufreq_vcore_volt_switch(unsigned int volt_target)
{
	...
	tmp = get_cur_vcore_opp();
	if(tmp > g_cur_vcore_opp && is_dvfsrc_enabled())
	{
		gpufreq_pr_debug(
			"%s: VCore not ready yet request %d / got %d\n",
			__func__, g_cur_vcore_opp, tmp);
		gpufreq_pr_debug("GPU Freq/Volt: %d/%d\n",
			mt_get_abist_freq(25),
			__mt_gpufreq_get_cur_volt(GET_PMIC));
		dump_stack();
	}
...
#ifdef CONFIG_MACH_MT6765_DH40LM
		__func__, mt_get_ckgen_freq(9), __mt_gpufreq_get_cur_volt(GET_PMIC),
#else
...
#ifdef CONFIG_MACH_MT6765_DH40LM
	cur_volt = __mt_gpufreq_get_cur_volt(GET_PMIC);
#else
...
#ifndef CONFIG_MACH_MT6765_DH40LM
	mt_gpufreq_voltage_enable_set(1);
#endif
...
#ifdef CONFIG_MACH_MT6765_DH40LM
	if (g_volt_enable_state || flag == GET_PMIC) {
#else
	if (g_volt_enable_state) {
#endif
#ifdef USE_STAND_ALONE_VGPU
...
static void __mt_gpufreq_kick_pbm(int enable)
{
	...
	cur_freq = __mt_gpufreq_get_cur_freq();
#ifdef CONFIG_MACH_MT6765_DH40LM
	cur_volt = __mt_gpufreq_get_cur_volt(GET_GPU_IN);
#else
	cur_volt = __mt_gpufreq_get_cur_volt();
#endif
	if (enable) {
...
#ifdef CONFIG_LGE_PM_FOUR_OPP
	__mt_gpufreq_setup_opp_table(g_opp_table_sample,
		ARRAY_SIZE(g_opp_table_sample));
