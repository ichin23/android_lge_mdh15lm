...
/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
static int fl_set_torch_level(struct flashlight_dev *fdev, int level)
{
	struct flashlight_dev_arg fl_dev_arg;
	...
	if (fdev->ops->flashlight_ioctl(FLASH_IOC_SET_FLASH_TORCH_LEVEL,
				(unsigned long)&fl_dev_arg)) {
		pr_err("Failed to set level\n");
		return -EFAULT;
	}
...
/*LGE_CHANGE_E, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
...
// LGE_CHANGE_S, 2020-08-19, add sysfs (flashlight_torch), youmi.jun@lge.com
/* verify flashlight_torch_arg */
static int flashlight_verify_torch_arg(struct flashlight_torch_arg fl_torch_arg)
{
	if (flashlight_verify_index(fl_torch_arg.type, fl_torch_arg.ct, fl_torch_arg.part))
		return -1;
	if (fl_torch_arg.level < -1 || fl_torch_arg.level > FLASHLIGHT_TORCH_ARG_LEVEL_MAX) {
		pr_err("level (%d) is not valid\n", fl_torch_arg.level);
		return -1;
	}

	if (fl_torch_arg.flag < 0 || fl_torch_arg.flag > 1) {
		pr_err("duration (%d) is not valid\n", fl_torch_arg.flag);
		return -1;
	}
...
// LGE_CHANGE_E, 2020-08-19, add sysfs (flashlight_torch), youmi.jun@lge.com
...
/*LGE_CHANGE_S, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
	case FLASH_IOC_SET_FLASH_TORCH_LEVEL:
		pr_debug("SAT [LGE] FLASH_IOC_SET_FLASH_TORCH_LEVEL(%d,%d,%d): %d\n",
				type, ct, part, fl_arg.arg);
		mutex_lock(&fl_mutex);
		ret = fl_set_torch_level(fdev, fl_arg.arg);
		mutex_unlock(&fl_mutex);
		break;
/*LGE_CHANGE_E, 2018-09-19, Implemented Flash Torch level, soojong.jin@lge.com*/
		...
		struct device_attribute *attr, char *buf)
{
	...
	return scnprintf(buf, PAGE_SIZE,
			"[TYPE] [CT] [PART] [LEVEL] [FLAG(0:off/1:on)]\n");
}
...
{
	struct flashlight_dev *fdev;
	struct flashlight_dev_arg fl_dev_arg = {0, };
	struct flashlight_torch_arg fl_torch_arg = {0, };
	...
	while (cur) {
		...
		if (count == FLASHLIGHT_TORCH_ARG_TYPE)
			fl_torch_arg.type = (int)num;
		else if (count == FLASHLIGHT_TORCH_ARG_CT)
			fl_torch_arg.ct = (int)num;
		else if (count == FLASHLIGHT_TORCH_ARG_PART)
			fl_torch_arg.part = (int)num;
		else if (count == FLASHLIGHT_TORCH_ARG_LEVEL)
			fl_torch_arg.level = (int)num;
		else if (count == FLASHLIGHT_TORCH_ARG_FLAG)
			fl_torch_arg.flag = (int)num;
		else {
			count++;
			break;
		}
	...
	/* verify data */
	if (count != FLASHLIGHT_TORCH_ARG_NUM) {
		pr_err("Error argument number: (%d)\n", count);
		ret = -1;
		goto unlock;
	}
	if (flashlight_verify_torch_arg(fl_torch_arg)) {
		pr_err("Error arguments\n");
		ret = -1;
		goto unlock;
	}
	...
	pr_debug("(%d, %d, %d), (%d, %d)\n",
			fl_torch_arg.type, fl_torch_arg.ct, fl_torch_arg.part,
			fl_torch_arg.level, fl_torch_arg.flag);
	...
	fl_torch_arg.channel = fdev->dev_id.channel;
	fl_torch_arg.decouple = fdev->dev_id.decouple;

	pr_info("channel:%d decouple:%d\n",
			fl_torch_arg.channel, fl_torch_arg.decouple);

	if (fl_torch_arg.flag == FLASHLIGHT_TORCH_ARG_FLAG_ON) {
		if (fdev->ops) {
			fdev->ops->flashlight_open();
			fdev->ops->flashlight_set_driver(1);

			fl_dev_arg.channel = fl_torch_arg.channel;
			fl_dev_arg.arg = FLASHLIGHT_SCENARIO_FLASHLIGHT;
			if (fl_torch_arg.decouple)
				fl_dev_arg.arg |= FLASHLIGHT_SCENARIO_DECOUPLE;
			fdev->ops->flashlight_ioctl(
					FLASH_IOC_SET_SCENARIO,
					(unsigned long)&fl_dev_arg);
			...
			fl_dev_arg.channel = fl_torch_arg.channel;
			fl_dev_arg.arg = fl_torch_arg.level;
			fdev->ops->flashlight_ioctl(FLASH_IOC_SET_FLASH_TORCH_LEVEL,
					(unsigned long)&fl_dev_arg);

			fl_dev_arg.channel = fl_torch_arg.channel;
			fl_dev_arg.arg = 0;
			fdev->ops->flashlight_ioctl(FLASH_IOC_SET_TIME_OUT_TIME_MS,
					(unsigned long)&fl_dev_arg);

			fl_enable(fdev, 1);
			ret = size;
		} else {
			pr_info("Failed with no flashlight torch ops\n");
			ret = -1;
		}
	} else if (fl_torch_arg.flag == FLASHLIGHT_TORCH_ARG_FLAG_OFF) {
		if (fdev->ops) {
			fl_enable(fdev, 0);
...
err_create_pt_device_file:
	...
	device_remove_file(flashlight_device, &dev_attr_flashlight_torch); // LGE_CHANGE, 2020-08-19, add sysfs (flashlight_torch), youmi.jun@lge.com
err_create_strobe_device_file:
err_create_torch_device_file: // LGE_CHANGE, 2020-08-19, add sysfs (flashlight_torch), youmi.jun@lge.com
...
#if 0 //[LGE_UPDATE] [kyunghun.oh@lge.com] [2018-11-02] turn on the flash light even if the battery level is under 15%
