	...
	for (i = 0; i < Num_of_OPP; i++) {
		if (cl_dev_state[i] == 1) {
			if (i != previous_step) {
				...
				if (Num_of_GPU_OPP == 4) {
					if (i == 32)
						power = mt_ppm_thermal_get_idx_power(6) + 1;
					else
						power = (i * 100 + 700) -
					...
					if (i == 33)
						set_static_gpu_power_limit(
							mtk_gpu_power[Num_of_GPU_OPP-4]
									.gpufreq_power -1 );
					else
						set_static_gpu_power_limit(
							mtk_gpu_power[Num_of_GPU_OPP-3]
									.gpufreq_power -1 );
					tscpu_dprintk(
						"Num_of_GPU_OPP=%d, gpufreq_power=%d, power=%d\n",
						 Num_of_GPU_OPP,
						 mtk_gpu_power[Num_of_GPU_OPP-2]
								.gpufreq_power,
								power);

				} else if (Num_of_GPU_OPP == 3) {
#else
				if (Num_of_GPU_OPP == 3) {
#endif
					if (i == 32)
						power = mt_ppm_thermal_get_idx_power(8) + 1;
					else
						power = (i * 100 + 700) -
							mtk_gpu_power[Num_of_GPU_OPP-1]
									.gpufreq_power;
...
static void thermal_vpu_init(void)
{
	...
	if (!dir_entry)
		...
		entry = proc_create("thermal_vpu_limit", 0664,
			dir_entry, &clvpu_opp_fops);
		if (entry)
