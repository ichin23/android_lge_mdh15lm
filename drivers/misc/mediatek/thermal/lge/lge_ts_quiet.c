 ...
 */
#define pr_fmt(fmt) "[Thermal/TZ/Quiet] %s: " fmt, __func__
...
#if 0
	lgets_quiet_dprintk(
		"lgets_quiet_thermistor_conver_temp() : TAP_Value = %d\n",
								TAP_Value);

	lgets_quiet_dprintk("lgets_quiet_thermistor_conver_temp() : Res = %d\n",
									Res);

	lgets_quiet_dprintk("lgets_quiet_thermistor_conver_temp() : RES1 = %d\n",
									RES1);

	lgets_quiet_dprintk("lgets_quiet_thermistor_conver_temp() : RES2 = %d\n",
									RES2);

	lgets_quiet_dprintk("lgets_quiet_thermistor_conver_temp() : TMP1 = %d\n",
									TMP1);

	lgets_quiet_dprintk("lgets_quiet_thermistor_conver_temp() : TMP2 = %d\n",
									TMP2);
#endif
...
static int get_hw_quiet_temp(void)
{
	...
	lgets_quiet_dprintk("BTS output temperature = %d\n", output);
	return output;
}
...
int lgets_quiet_get_hw_temp(void)
{
	...
	lgets_quiet_dprintk("[lgets_quiet_get_hw_temp] T_AP, %d\n", t_ret);
	return t_ret;
}

#ifdef CONFIG_SND_SOC_TAS2563
EXPORT_SYMBOL(lgets_quiet_get_hw_temp);
#endif
...
struct thermal_zone_device *thermal, struct thermal_cooling_device *cdev)
{
	...
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		lgets_quiet_dprintk("[lgets_quiet_bind] %s\n", cdev->type);
	...
	if (mtk_thermal_zone_bind_cooling_device(thermal, table_val, cdev)) {
		lgets_quiet_dprintk(
			"[lgets_quiet_bind] error binding cooling dev\n");
		return -EINVAL;
	}

	lgets_quiet_dprintk("[lgets_quiet_bind] binding OK, %d\n", table_val);
	return 0;
}
			    ...
			    struct thermal_cooling_device *cdev)
{
	...
	if (!strcmp(cdev->type, g_bind0)) {
		table_val = 0;
		lgets_quiet_dprintk("[lgets_quiet_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind1)) {
		table_val = 1;
		lgets_quiet_dprintk("[lgets_quiet_unbind] %s\n", cdev->type);
	} else if (!strcmp(cdev->type, g_bind2)) {
	...
	if (thermal_zone_unbind_cooling_device(thermal, table_val, cdev)) {
		lgets_quiet_dprintk(
			"[lgets_quiet_unbind] error unbinding cooling dev\n");
		return -EINVAL;
	}

	lgets_quiet_dprintk("[lgets_quiet_unbind] unbinding OK\n");
	return 0;
}
...
static int lgets_quiet_read(struct seq_file *m, void *v)
{
	...
	seq_printf(m,
		"[lgets_quiet_read] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,trip_4_temp=%d\n",
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	if (sscanf
		...
		down(&sem_mutex);
		lgets_quiet_dprintk(
			"[lgets_quiet_write] lgets_quiet_unregister_thermal\n");
		...
		if (num_trip < 0 || num_trip > 10) {
			...
			aee_kernel_warning_api(__FILE__, __LINE__,
					...
					"Bad argument");
			#endif
			lgets_quiet_dprintk("[lgets_quiet_write] bad argument\n");
			kfree(ptr_lgetsquiet_data);
			up(&sem_mutex);
			return -EINVAL;
		}
		...
		lgets_quiet_dprintk(
			"[lgets_quiet_write] g_THERMAL_TRIP_0=%d,g_THERMAL_TRIP_1=%d,g_THERMAL_TRIP_2=%d,",
			g_THERMAL_TRIP[0], g_THERMAL_TRIP[1],
			g_THERMAL_TRIP[2]);
		...
		lgets_quiet_dprintk(
			"[lgets_quiet_write] cooldev0=%s,cooldev1=%s,cooldev2=%s,cooldev3=%s,cooldev4=%s,",
			g_bind0, g_bind1, g_bind2, g_bind3, g_bind4);
		...
		lgets_quiet_dprintk(
			"[lgets_quiet_write] trip_0_temp=%d,trip_1_temp=%d,trip_2_temp=%d,trip_3_temp=%d,",
		...
		lgets_quiet_dprintk(
			"[lgets_quiet_write] lgets_quiet_register_thermal\n");
...
void lgets_quiet_prepare_table(int table_num)
{
	...
	pr_notice("%s table_num=%d, table_rows=%d\n",
			__func__, table_num,
			(int)(ntc_tbl_size / sizeof(struct QUIET_TEMPERATURE)));
...
struct file *file, const char __user *buffer, size_t count, loff_t *data)
{
	...
	lgets_quiet_dprintk("[lgets_quiet_write]\n");
	...
	if (sscanf
		...
		if (!strcmp(ptr_lgetsquiet_parm_data->pull_R, "PUP_R")) {
			...
			lgets_quiet_printk(
				"[lgets_quiet_write] bad PUP_R argument\n");
			kfree(ptr_lgetsquiet_parm_data);
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_lgetsquiet_parm_data->pull_V, "PUP_VOLT")) {
			...
			lgets_quiet_printk(
				"[lgets_quiet_write] bad PUP_VOLT argument\n");
			kfree(ptr_lgetsquiet_parm_data);
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_lgetsquiet_parm_data->overcrilow,
			...
			lgets_quiet_printk(
				"[lgets_quiet_write] bad OVERCRIT_L argument\n");
			kfree(ptr_lgetsquiet_parm_data);
			return -EINVAL;
		}
		...
		if (!strcmp(ptr_lgetsquiet_parm_data->NTC_TABLE, "NTC_TABLE")) {
			...
			lgets_quiet_printk(
				"[lgets_quiet_write] bad NTC_TABLE argument\n");
			kfree(ptr_lgetsquiet_parm_data);
			return -EINVAL;
		}

		if ((ptr_lgetsquiet_parm_data->adc_channel >= 0) &&
			(ptr_lgetsquiet_parm_data->adc_channel <= 4))
	...
	lgets_quiet_printk("[lgets_quiet_write] bad argument\n");
	kfree(ptr_lgetsquiet_parm_data);
	return -EINVAL;
}
...
static int lgets_quiet_register_thermal(void)
{
	lgets_quiet_dprintk("[lgets_quiet_register_thermal]\n");
...
static void lgets_quiet_unregister_thermal(void)
{
	lgets_quiet_dprintk("[lgets_quiet_unregister_thermal]\n");
...
static int __init lgets_quiet_init(void)
{
	...
	lgets_quiet_dprintk("[lgets_quiet_init]\n");
...
{
	lgets_quiet_dprintk("[lgets_quiet_exit]\n");
	lgets_quiet_unregister_thermal();
#if 0
