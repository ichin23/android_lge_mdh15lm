...
#define PRIMARY_LCM   1
#define SECONDARY_LCM 2

static int mt_get_maker_id(void)
{
	int ret = PRIMARY_LCM;

#if defined(CONFIG_MACH_MT6762_MH4) // mh4
	if(is_lcm_name("DJN-ILI9881H") != 1)
		ret = SECONDARY_LCM;
#elif defined(CONFIG_MACH_MT6762_MH4P) // mh4p
	if(is_lcm_name("KD-ILI9881C") != 1)
		ret = SECONDARY_LCM;
#elif defined(CONFIG_MACH_MT6762_DH10LM)//dh10
	if(is_lcm_name("LCE-ILI9881C") != 1)
		ret = SECONDARY_LCM;
#else
	printk("[%s] do not need to set\n", __func__);
#endif
...
#ifdef CONFIG_LGE_USE_BRIGHTNESS_TABLE
				ret = mt_backlight_brightness_parse_dt(led_node);
		...
		if (strcmp(led_data->cust.name, "lcd-backlight") != 0) {
...
#ifndef CONFIG_LGE_USE_ESS_STRENGTH_TABLE
			results = down_interruptible(&aal_lock);
			if(level < AAL_MIN_BRIGHTNESS && disp_aal_get_ess_en()){
				disp_aal_set_ess_en(0);
				printk("[AAL] level : %d, Ess turn off!!\n", level);
			} else if(level >= AAL_MIN_BRIGHTNESS && !disp_aal_get_ess_en()){
			...
			if(led_data->cust.mode == MT65XX_LED_MODE_CUST_BLS_PWM){
				if (level != 0){
					results = down_interruptible(&aal_lock);
					if (blmap_arr != NULL){
						level10_mapping = blmap_arr[level];
						if (level10_mapping > blmap_arr[MAX_BRIGHTNESS])
							level10_mapping = blmap_arr[MAX_BRIGHTNESS];
						printk("[%s] level = %d, level10_mapping = %d\n", __func__, level, level10_mapping);
					}
					else
						level10_mapping = (((1 << MT_LED_INTERNAL_LEVEL_BIT_CNT) - 1) * level + 127) / 255 ;
					up(&aal_lock);
				}
				else{
					level10_mapping = 0;
				...
				LEDS_DEBUG("level = %d, level10_mapping = %d\n",level,level10_mapping);
				disp_pq_notify_backlight_changed(level10_mapping);
