...
#define PDAF_DATA_SIZE 8392 //[LGE_UPDATE][youmi.jun@lge.com][2020-04-28] for 4cell remosaic (4096->8392)
...
char g_lge_product_name[16] = ""; /*LGE_CHANGE, 2020-04-20, Bringup DH50 SCA, soojong.jin@lge.com*/
...
/*LGE_CHANGE_S, 2020-04-20, Bringup DH50 SCA, soojong.jin@lge.com*/
	pr_info("[LGE] g_lge_product_name : %s\n", g_lge_product_name);

	if(!strcmp(g_lge_product_name, "mdh50lm_2mfg")){

		pr_info("[LGE] imgsensor_custom_config_sca is loaded");

		imgsensor_i2c_init(&psensor_inst->i2c_cfg,
			imgsensor_custom_config_sca[psensor->inst.sensor_idx].i2c_dev);
	}
	else {
...
/*LGE_CHANGE_S, 2020-04-20, Bringup DH50 SCA, soojong.jin@lge.com*/
EXPORT_SYMBOL(g_lge_product_name);

static int __init lge_product_name(char *lge_product_name)
{
	strncpy(g_lge_product_name, lge_product_name, sizeof(g_lge_product_name)-1);
	lge_product_name[sizeof(g_lge_product_name)-1] = '\0';

	printk("g_lge_product_name : %s\n", g_lge_product_name);

	return 0;
}
early_param("androidboot.vendor.lge.product.name", lge_product_name);
/*LGE_CHANGE_E, 2020-04-20, Bringup DH50 SCA, soojong.jin@lge.com*/
