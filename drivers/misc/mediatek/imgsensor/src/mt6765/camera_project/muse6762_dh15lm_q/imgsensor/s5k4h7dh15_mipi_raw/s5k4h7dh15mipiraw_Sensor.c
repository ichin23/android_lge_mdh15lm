...
#include "s5k4h7dh15mipiraw_Sensor.h"


#define PFX "s5k4h7dh15_camera_sensor"
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = S5K4H7DH15_SENSOR_ID,
...
static kal_uint16 s5k4h7dh15_table_write_cmos_sensor(
...
#endif
#if 0
kal_uint16 addr_data_pair_init_s5k4h7dh15[] = {
	0x2000, 0x987A, 
	0x2002, 0x00FF,
	0x2004, 0x0047, 
	0x2006, 0x3FFF,
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			LOG_INF("s5k4h7dh15mipiraw get_sensor_id = %x\r\n", *sensor_id);
			if (*sensor_id == imgsensor_info.sensor_id) {
				strcpy(front_sensor_name, "s5k4h7_hlt");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
				return ERROR_NONE;
			}
			retry--;
		} while (retry > 0);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			LOG_INF("s5k4h7dh15mipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
