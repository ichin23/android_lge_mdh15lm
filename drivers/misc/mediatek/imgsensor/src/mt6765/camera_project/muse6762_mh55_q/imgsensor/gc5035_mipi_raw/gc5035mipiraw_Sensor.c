		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				printk("shy gc5035 read ok i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			#if 0
			...
			printk("shy gc5035 Read sensor id fail, write id: 0x%x, id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id);
			retry--;
		} while (retry > 0);
		i++;
		retry = 2;
...
{
	LOG_INF("E\n");
	printk("[GC5035] Start Preview mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
	printk("[GC5035] Start Capture mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
	printk("[GC5035] Start NoramlVideo mode\n");
	spin_lock(&imgsensor_drv_lock);
