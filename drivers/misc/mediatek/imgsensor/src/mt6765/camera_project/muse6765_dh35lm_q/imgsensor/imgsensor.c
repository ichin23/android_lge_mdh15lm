...
#if 0
extern char rear_cam[HARDWARE_MAX_ITEM_LONGTH];
extern char front_cam[HARDWARE_MAX_ITEM_LONGTH];
extern char wide_cam[HARDWARE_MAX_ITEM_LONGTH];
extern char depth_cam[HARDWARE_MAX_ITEM_LONGTH];
extern char marco_cam[HARDWARE_MAX_ITEM_LONGTH];
#endif
/*Murphy Zhou add & show camera sensor name*/

#define PDAF_DATA_SIZE 4096
unsigned int sensor_idx_flag=-1;
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
...
#if 0 //#ifdef CONFIG_HQ_HARDWARE_INFO
	switch(psensor_inst->sensor_idx) {
		...
		default: {
			pr_debug("other camera, not main & sub camera!");
			break;
		}
	}
	#endif
		...
		if (pSensorList[drv_idx].init) {
			...
			if (psensor->pfunc) {
				...
				psensor_inst->status.arch =
				    psensor->pfunc->arch;
#endif
				pr_err("name:%s , psensor->inst.sensor_idx:%d",psensor_inst->psensor_name,psensor->inst.sensor_idx);
				if(((psensor->inst.sensor_idx == 0) && (!strcmp(psensor_inst->psensor_name, "s5kgd1_mipi_raw")))
				||((psensor->inst.sensor_idx == 1) && (!strcmp(psensor_inst->psensor_name, "hi1336_mipi_raw")))
				||((psensor->inst.sensor_idx == 2) && (!strcmp(psensor_inst->psensor_name, "hi556_mipi_raw")))
				||((psensor->inst.sensor_idx == 3) && (!strcmp(psensor_inst->psensor_name, "gc2375hm_mipi_raw")))
				||((psensor->inst.sensor_idx == 4) && (!strcmp(psensor_inst->psensor_name, "gc2375hd_mipi_raw")))){
					if (!imgsensor_check_is_alive(psensor)) {
						...
						if(psensor->inst.sensor_idx == 0)
							strncpy(rear_cam,psensor_inst->psensor_name,strlen(psensor_inst->psensor_name)+1);
						else if(psensor->inst.sensor_idx == 1)
							strncpy(front_cam,psensor_inst->psensor_name,strlen(psensor_inst->psensor_name)+1);
						else if(psensor->inst.sensor_idx == 2)
							strncpy(wide_cam,psensor_inst->psensor_name,strlen(psensor_inst->psensor_name)+1);
						else if(psensor->inst.sensor_idx == 3)
							strncpy(depth_cam,psensor_inst->psensor_name,strlen(psensor_inst->psensor_name)+1);
						else if(psensor->inst.sensor_idx == 4)
							strncpy(marco_cam,psensor_inst->psensor_name,strlen(psensor_inst->psensor_name)+1);
#endif
						ret = drv_idx;
						break;
					}
