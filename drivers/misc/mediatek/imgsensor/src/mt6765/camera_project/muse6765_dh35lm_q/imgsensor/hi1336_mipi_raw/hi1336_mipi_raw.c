...
unsigned char hi1336_otp_data[HI1336_OTP_SIZE];
...
unsigned char hi1336_module_info_data[MODULE_INFO_DATA_SIZE] = {0};
...
unsigned char hi1336_awb_51k_data_valid = 0;
...
unsigned char hi1336_awb_30k_data_valid = 0;

#define LSC_51K_INFO_FLAG 0x2d
#define LSC_51K_INFO_START 0x2e
#define LSC_51K_INFO_END 0x779
#define LSC_51K_INFO_CHECKSUM 0x77a
#define LSC_51K_DATA_SIZE 1868
unsigned char hi1336_lsc_51k_data_valid = 0;

#define LSC_40K_INFO_FLAG 0x77b
#define LSC_40K_INFO_START 0x77c
#define LSC_40K_INFO_END 0xec7
#define LSC_40K_INFO_CHECKSUM 0xec8
#define LSC_40K_DATA_SIZE 1868
unsigned char hi1336_lsc_40k_data_valid = 0;
#endif
...
static struct imgsensor_info_struct imgsensor_info = { 
	...
	.checksum_value = 0x4f1b1d5e,       //0x6d01485c // Auto Test Mode 蓄板..
	.pre = {
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 4224, 3136,   0, 116, 4224, 2904,	  704,  484,  34, 2,  640,  480, 0, 0,  640,  480},		// hight speed video (640 x 480)
 	{ 4224, 3136,   0, 482, 4224, 2172,  1408,  724,  64, 2, 1280,  720, 0, 0, 1280,  720},       // slim video (1280 x 720)
 	{ 4224, 3136,   0, 342, 4224, 2452,  4224, 2452, 480, 2, 3264, 2448, 0, 0, 3264, 2448} // custom1
};
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x0668, 0x4302); 
...
{
#if MULTI_WRITE
	LOG_DEBUG("preview_setting multi write\n");
	hi1336_table_write_cmos_sensor(
		addr_data_pair_preview_hi1336,
		sizeof(addr_data_pair_preview_hi1336) /
		sizeof(kal_uint16));
#else
	LOG_DEBUG("preview_setting normal write\n");
	...
	write_cmos_sensor(0x1012, 0x00ba); 
	write_cmos_sensor(0x1014, 0x001b); 
	write_cmos_sensor(0x1016, 0x001b); 
	write_cmos_sensor(0x101a, 0x001b); 
	write_cmos_sensor(0x1038, 0x0000); 
	write_cmos_sensor(0x1042, 0x0008); 
	write_cmos_sensor(0x1004, 0x2b30);
...
static kal_uint16 addr_data_pair_capture_15fps_hi1336[] = {
	0x3250, 0xa060,
	0x0730, 0x7d4f,
	0x0732, 0xe0b1,
	0x1118, 0x0006,
...
{
	LOG_DEBUG("E! %s currefps:%d\n",__func__,currefps);
#if MULTI_WRITE
	LOG_DEBUG("capture_setting multi write\n");
	if (currefps > 150) {
	hi1336_table_write_cmos_sensor(
...
#else
	LOG_DEBUG("capture_setting normal write\n");
	if( currefps > 150) {
		write_cmos_sensor(0x3250, 0xa060); 
		write_cmos_sensor(0x0730, 0x770f); 
		write_cmos_sensor(0x0732, 0xe0b0); 
		write_cmos_sensor(0x1118, 0x0006); 
		write_cmos_sensor(0x1200, 0x011f); //write_cmos_sensor(0x1200, 0x0d1f);
		write_cmos_sensor(0x1204, 0x1c01); 
		write_cmos_sensor(0x1240, 0x0100); 
		...
		write_cmos_sensor(0x1010, 0x07d0); 
		write_cmos_sensor(0x1012, 0x017d); 
		write_cmos_sensor(0x1014, 0x006a); 
		write_cmos_sensor(0x1016, 0x006a); 
		write_cmos_sensor(0x101a, 0x006a); 
		write_cmos_sensor(0x1038, 0x0000);
		write_cmos_sensor(0x1042, 0x0008);
		write_cmos_sensor(0x1004, 0x2b30);
		write_cmos_sensor(0x1048, 0x0080); 
		...
		write_cmos_sensor(0x3250, 0xa060);
		write_cmos_sensor(0x0730, 0x7d4f);
		write_cmos_sensor(0x0732, 0xe0b1);
		write_cmos_sensor(0x1118, 0x0006);
		write_cmos_sensor(0x1200, 0x011f); //write_cmos_sensor(0x1200, 0x0d1f);
		write_cmos_sensor(0x1204, 0x1c01);
		write_cmos_sensor(0x1240, 0x0100);
		...
		write_cmos_sensor(0x1026, 0x0609);
		write_cmos_sensor(0x1028, 0x1207);
		write_cmos_sensor(0x102a, 0x0a0a);
		write_cmos_sensor(0x102c, 0x1600);
		write_cmos_sensor(0x1010, 0x07d0);
		write_cmos_sensor(0x1012, 0x0201);
		write_cmos_sensor(0x1014, 0x00b9);
		write_cmos_sensor(0x1016, 0x00b9);
		write_cmos_sensor(0x101a, 0x00b9);
		write_cmos_sensor(0x1038, 0x0000);
		write_cmos_sensor(0x1042, 0x0008);
		write_cmos_sensor(0x1004, 0x2b30);
		write_cmos_sensor(0x1048, 0x0080);
...
#if 0 // unused setting, soojong.jin@lge.com
#if MULTI_WRITE
...
{
#if MULTI_WRITE
	LOG_DEBUG("video_setting multi write\n");
	hi1336_table_write_cmos_sensor(
		addr_data_pair_video_hi1336,
		sizeof(addr_data_pair_video_hi1336) /
		sizeof(kal_uint16));
#else
	LOG_DEBUG("video_setting normal write\n");
	write_cmos_sensor(0x3250, 0xa060);
	...
	write_cmos_sensor(0x102a, 0x1311);
	write_cmos_sensor(0x102c, 0x2400);
	write_cmos_sensor(0x1010, 0x0a76);
	write_cmos_sensor(0x1012, 0x0024);
	write_cmos_sensor(0x1014, 0xffbe);
	write_cmos_sensor(0x1016, 0xffbe);
	write_cmos_sensor(0x101a, 0xffbe);
	write_cmos_sensor(0x0404, 0x0008);
	write_cmos_sensor(0x0406, 0x1087);
	write_cmos_sensor(0x0220, 0x0008);
	write_cmos_sensor(0x022a, 0x0017);
	write_cmos_sensor(0x0222, 0x0c80);
	write_cmos_sensor(0x022c, 0x0c89);
	write_cmos_sensor(0x0224, 0x01a6);
	write_cmos_sensor(0x022e, 0x0ae9);
	write_cmos_sensor(0x0f04, 0x0008);
	write_cmos_sensor(0x0f06, 0x0000);
	write_cmos_sensor(0x023a, 0x1111);
	write_cmos_sensor(0x0234, 0x1111);
	...
	write_cmos_sensor(0x020e, 0x0d00); //frame length lines
	write_cmos_sensor(0x0b12, 0x1070); //x output size
	write_cmos_sensor(0x0b14, 0x0940); //y output size
	write_cmos_sensor(0x0204, 0x0000);
	write_cmos_sensor(0x041c, 0x0048);
	write_cmos_sensor(0x041e, 0x1047);
...
static void hs_video_setting(void)
{
	...
	hi1336_table_write_cmos_sensor(
		...
		sizeof(kal_uint16));
#else
	LOG_DEBUG("hs_video_setting normal write\n");
...
#if MULTI_WRITE
	LOG_DEBUG("slim_video_setting multi write\n");
	hi1336_table_write_cmos_sensor(
		addr_data_pair_slim_video_hi1336,
		sizeof(addr_data_pair_slim_video_hi1336) /
		sizeof(kal_uint16));
#else
	LOG_DEBUG("slim_video_setting normal write\n");
	write_cmos_sensor(0x3250, 0xa060);
	write_cmos_sensor(0x0730, 0x770f);
...
#if MULTI_WRITE
	LOG_DEBUG("custom1_setting multi write\n");
	hi1336_table_write_cmos_sensor(
		addr_data_pair_custom1_hi1336,
		sizeof(addr_data_pair_custom1_hi1336) /
		sizeof(kal_uint16));
#else
	LOG_DEBUG("custom1_setting normal write\n");
	write_cmos_sensor(0x3250, 0xa060);
	write_cmos_sensor(0x0730, 0x600f);
	write_cmos_sensor(0x0732, 0xe0b0);
	write_cmos_sensor(0x1118, 0x0000);
...
static struct stCAM_CAL_DATAINFO_STRUCT hi1336_xl_eeprom_data ={
	.sensorID= HI1336_SENSOR_ID,
	.deviceID = 0x01,
	.dataLength = 0x0CE3,
...
static struct stCAM_CAL_CHECKSUM_STRUCT hi1336XlChecksum[8] =
{
	{MODULE_ITEM,0x0000,0x0000,0x0017,0x0018,0x55},
	{AWB_ITEM,0x0019,0x0019,0x0021,0x0022,0x55},
	{AF_ITEM,0x0023,0x0023,0x0027,0x0028,0x55},
	{LSC_ITEM,0x0029,0x0029,0x0777,0x0778,0x55},
	{PDAF_ITEM,0x0779,0x0779,0x0ce0,0x0ce1,0x55},
	{TOTAL_ITEM,0x0000,0x0000,0x0ce1,0x0ce2,0x55},
...
#if HI1336_OTP 
int read_hi1336_otp(void){
		int addr = imgsensor.i2c_write_id;
		int i, sum = 0;
		imgsensor.i2c_write_id = 0xA0;
		for(i = 0; i < HI1336_OTP_SIZE; i++){
				hi1336_otp_data[i]	= read_cmos_sensor(i);
				//pr_err("=== hi1336_otp_data[%x]: %x ===\n", i, hi1336_otp_data[i]);
		}
		imgsensor.i2c_write_id = addr;
		//check module info
		for(i = MODULE_INFO_FLAG; i < MODULE_INFO_CHECKSUM; i++){
					sum += hi1336_otp_data[i];
		}
		if((hi1336_otp_data[MODULE_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi1336_otp_data[MODULE_INFO_CHECKSUM]){
			LOG_DEBUG("Mouule   ID 0x%x ", hi1336_otp_data[1]);
			LOG_DEBUG("Position ID 0x%x ", hi1336_otp_data[2]);
			LOG_DEBUG("Lens     ID 0x%x ", hi1336_otp_data[3]);
			LOG_DEBUG("VCM      ID 0x%x ", hi1336_otp_data[4]);
			LOG_DEBUG("Year: %d Month: %d Data: %d", hi1336_otp_data[5], hi1336_otp_data[6], hi1336_otp_data[7]);
		}else{
				pr_err("=== hi1336_read module info failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi1336_otp_data[AWB_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi1336_otp_data[AWB_51K_INFO_CHECKSUM]){
			hi1336_awb_51k_data_valid = 1;
			pr_err("=== hi1336_read module info AWB 51k OK!\n");
		}else{
				pr_err("=== hi1336_read module info awb 51k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi1336_otp_data[AWB_30K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi1336_otp_data[AWB_30K_INFO_CHECKSUM]){
			hi1336_awb_30k_data_valid = 1;
			pr_err("=== hi1336_read module info AWB 30k OK!\n");
		}else{
				pr_err("=== hi1336_read module info awb 30k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi1336_otp_data[LSC_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi1336_otp_data[LSC_51K_INFO_CHECKSUM]){
			hi1336_lsc_51k_data_valid = 1;
			pr_err("=== hi1336_read module info LSC 51k OK!\n");
		}else{
				pr_err("=== hi1336_read module info lsc 51k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi1336_otp_data[LSC_40K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi1336_otp_data[LSC_40K_INFO_CHECKSUM]){
			hi1336_lsc_40k_data_valid = 1;
			pr_err("=== hi1336_read module info AWB 40k OK!\n");
		}else{
				return 0;
		}
...
static kal_uint32 open(void)
{
	...
	LOG_DEBUG("[open]: PLATFORM:MT6737,MIPI 24LANE\n");
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		spin_lock(&imgsensor_drv_lock);
		imgsensor.i2c_write_id = imgsensor_info.i2c_addr_table[i];
		spin_unlock(&imgsensor_drv_lock);
