...
#if 1//def CONFIG_HQ_HARDWARE_INFO
static kal_uint16 OTP_read_cmos_sensor(kal_uint16 otp_addr)
{
    kal_uint16 data;	
    write_cmos_sensor_8(0x010a, (otp_addr & 0xff00)>> 8); //start address H        
    write_cmos_sensor_8(0x010b, otp_addr& 0x00ff); //start address L
...
#if 0
static char sn_code[HI556_OTP_SN_LENGTH];
static int read_otp_barcode(void)
{
    kal_uint8 barcodeFlag = 0;
    kal_uint16 i = 0;
	kal_uint32 check = 0;
	kal_uint8 data[HI556_OTP_SN_LENGTH + 5];
	kal_uint16 addr = HI556_OTP_LCS_SN_FLAG_ADDR;
	//flag
	barcodeFlag = OTP_read_cmos_sensor(addr);
	LOG_INF("read_otp_barcode: Flag: 0x%x\n", barcodeFlag);
	if (0 == barcodeFlag){
		pr_err("read_otp_barcode: flag not valid.\n");
		return -1;
	}
	//SN CODE
	addr = HI556_OTP_SN_GOUP1_START_ADDR;
	if(barcodeFlag == HI556_OTP_GOUP2_FLAG)
		addr = HI556_OTP_SN_GOUP2_START_ADDR;
	else if(barcodeFlag == HI556_OTP_GOUP3_FLAG)
		addr = HI556_OTP_SN_GOUP3_START_ADDR;

	for(i = 0; i < HI556_OTP_SN_LENGTH + 4; i++, addr++){
		 data[i]= OTP_read_cmos_sensor(addr);
		 LOG_INF("read_otp_barcode: 0x%x\n", data[i]);
	}

    memset(sn_code,0,sizeof(sn_code));
    for (i = 0;i < HI556_OTP_SN_LENGTH; i++)
    {
        sn_code[i] = data[i];
        //LOG_INF("hi556 sn_code[%d]:%02x,%c\n",i,sn_code[i],sn_code[i]);
    }
	for (i = 0;i < HI556_OTP_SN_LENGTH + 3; i++)
    {
        check += data[i];
    }
    printk("hi556 sn_code:%s",sn_code);

    check = check % 255 +1;
	LOG_INF("read_otp_module_info: check:0x%x, read_check:0x%x\n", check, data[HI556_OTP_SN_LENGTH + 3]);
    if(check == data[HI556_OTP_SN_LENGTH + 3])
    {
        get_hardware_info_data(HWID_MAIN_CAM_2_SN, sn_code);
        return 0;
    }
    printk("hi556 barcode checksum error\n");
    return -1;
}
#endif

static int read_otp_module_info(void)
{
	kal_uint16 i = 0, check = 0;
	kal_uint16 module_info_Flag = 0;
	kal_uint8 data[HI556_OTP_MODULE_INFO_LENGTH + 1];
	kal_uint16 addr = HI556_OTP_START_ADDR;
	//flag
	module_info_Flag = OTP_read_cmos_sensor(addr);
	LOG_INF("read_otp_module_info: Flag: 0x%x\n", module_info_Flag);
	if (0 == module_info_Flag){
		pr_err("read_otp_module_info: flag not valid.\n");
		return -1;
	}
	//module info
	addr = HI556_OTP_MODULE_INFO_GOUP1_START_ADDR;
	if(module_info_Flag == HI556_OTP_GOUP2_FLAG)
		addr = HI556_OTP_MODULE_INFO_GOUP2_START_ADDR;
	else if(module_info_Flag == HI556_OTP_GOUP3_FLAG)
		addr = HI556_OTP_MODULE_INFO_GOUP3_START_ADDR;

	for(i = 0; i < HI556_OTP_MODULE_INFO_LENGTH + 1; i++, addr++){
		 data[i]= OTP_read_cmos_sensor(addr);
		 LOG_INF("read_otp_module_info: 0x%x\n", data[i]);
	}
	//check sum
	for(i = 0; i < HI556_OTP_MODULE_INFO_LENGTH; i++){
		check += data[i];
	}
	check = check % 255 + 1;
	LOG_INF("read_otp_module_info: check:0x%x, read_check:0x%x\n", check, data[HI556_OTP_MODULE_INFO_LENGTH]);
	if (check == data[HI556_OTP_MODULE_INFO_LENGTH]) {
		pOtp_data.vaild_flag = module_info_Flag;
		pOtp_data.vendor_id = data[0];
		pOtp_data.module_code = data[10];
		pOtp_data.module_ver = data[0];
		pOtp_data.sw_ver = data[5];
		pOtp_data.year = data[2];
		pOtp_data.month = data[3];
		pOtp_data.day = data[4];
		pOtp_data.vcm_id = data[7];
	} else {
		pr_err("read_otp_module_info: check sum err.\n");
		return -1;
	}
	return 0;
}

static int  get_otp_data(void)
{
	otp_init_setting();

	//read otp data -- start
	if (read_otp_module_info()){
		pr_err("get_otp_data: read_otp_module_info return err.\n");
		return -1;
	}
	/*
	if (read_otp_barcode()){
		pr_err("get_otp_data: read_otp_barcode return err.\n");
	}
	*/
	//end
...
0x003e, 0x0000,
//0x0074, 0x07ff,
0x0074, 0x07ff,//modified 10/01
//0x0070, 0x0411,
0x0070, 0x0411,//modified 10/01
0x0804, 0x0208,
0x0a04, 0x016a,
0x090e, 0x0010,
0x090c, 0x09c0,
...
#if MULTI_WRITE
kal_uint16 addr_data_pair_normal_video_hi556[] = {
    0x0b0a, 0x8252,
	0x0f30, 0x6e25,
	0x0f32, 0x7067,
	0x004a, 0x0100,
...
#if MULTI_WRITE
hi556_table_write_cmos_sensor(
		addr_data_pair_normal_video_hi556,
		sizeof(addr_data_pair_normal_video_hi556) /
		sizeof(kal_uint16));
#else
			...
			do {
				*sensor_id =return_sensor_id();
				if (*sensor_id == imgsensor_info.sensor_id) {
					printk("shy hi556 read ok  i2c write id  : 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
					
					#if 1 //def CONFIG_HQ_HARDWARE_INFO
						if (0 == get_otp_data()) {
						//hw_info_main2_otp.sensor_name = PFX;
						hw_info_main2_otp.otp_valid = pOtp_data.vaild_flag;
						hw_info_main2_otp.vendor_id = pOtp_data.vendor_id;
						hw_info_main2_otp.module_code = pOtp_data.module_code;
						hw_info_main2_otp.module_ver = pOtp_data.module_ver;
						hw_info_main2_otp.sw_ver = pOtp_data.sw_ver;
						hw_info_main2_otp.year = pOtp_data.year;
						hw_info_main2_otp.month = pOtp_data.month;
						hw_info_main2_otp.day = pOtp_data.day;
						hw_info_main2_otp.vcm_vendorid = pOtp_data.vcm_id;
						//hw_info_main2_otp.vcm_moduleid = pOtp_data.driver_id;
						}
					#endif
					
					return ERROR_NONE;
				}
				printk("shy hi556 get_imgsensor_id Read sensor id fail, i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
				retry--;
			} while(retry > 0);
			i++;
			retry = 2;
		printk("shy hi556 get_imgsensor_id Read sensor id , i2c write id: 0x%x,sensor id: 0x%x\n", imgsensor.i2c_write_id,*sensor_id);
}
	if (*sensor_id != imgsensor_info.sensor_id) {
...
static kal_uint32 open(void)
{
	...
	printk("hi556 [open]: PLATFORM:MT6762,MIPI 24LANE\n");
