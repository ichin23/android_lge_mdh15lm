...
static kal_uint16 read_eeprom_module(kal_uint32 addr)
{
	...
	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC8034_EEPROM_I2C_ADDR);
...
#if 0
#define DEBUG 1
#define QRCODE_VALID_FLAG 0x0780
#define QRCODE_ADDR_BASE 0x0781
#define QRCODE_CHECKSUM 0X0790
...
static int read_eeprom_barcode(void)
{
    ...
    if(barcodeFlag != 0x01)
    {
       printk("gc02m1 barcodeFlag invalid barcodeFlag =0x%x!\n",barcodeFlag);
        return -1;
    }
    ...
    for (i = 0; i < QRCODE_ADDR_LENS; i++)
    {
        ...
        pr_err("gc02m1 sn_code[%d]:%02x,%c",i,sn_code[i],sn_code[i]);
        Sum += sn_code[i];
    }
    pr_err("gc02m1 sn_code:%s",sn_code);

    checkSum=Sum%255+1;
    if((checkSum&0xff)==read_eeprom_module(QRCODE_CHECKSUM))
    {
        pr_err("gc02m1 barcode checksum sucess\n");
        get_hardware_info_data(HWID_SUB2_CAM_SN, sn_code);
        return 0;
    }
    pr_err("gc02m1 barcode checksum error\n");
    return -1;
}
...
static int get_eeprom_data(EEPROM_DATA *data)
{
    ...
    otp_data = (u8 *)data;
    for (i=0;i<=0x0E;i++)
        pr_err(" gc02m1 [simon]otpdata[0x%x]=0x%x \n", i, *(otp_data + i));
#endif
    read_eeprom_barcode();
    return 0;
}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_err("[AR][m]  sensor_id: 0x%x, return_sensor_id: 0x%x\n", *sensor_id, imgsensor_info.sensor_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
#if 0
			if ((sensor_id == imgsensor_info.sensor_id) && (gc02m1_otp_id == 0x00)) {
				pr_err("i2c write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, sensor_id, gc02m1_otp_id);
				break;
			}
			pr_err("Read sensor id fail, write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, sensor_id, gc02m1_otp_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
	...
	if ((imgsensor_info.sensor_id != sensor_id) || (gc02m1_otp_id != 0x00))
		return ERROR_SENSOR_CONNECT_FAIL;
#else
			if (sensor_id == imgsensor_info.sensor_id) {
