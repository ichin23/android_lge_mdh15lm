...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[5]={
	...
	{  1600, 1200,  0, 0, 1600, 1200, 1600, 1200,
	   0000, 0000, 1600, 1200,  0, 0, 1600, 1200}// slim video
};
...
static void set_dummy(void)
{
	...
	write_cmos_sensor(0x14, (imgsensor.dummy_line >> 8) & 0xFF);
	write_cmos_sensor(0x15, imgsensor.dummy_line & 0xFF);
	//write_cmos_sensor(0xfd, 0x03);	//page3
	write_cmos_sensor(0xfe, 0x02);	//fresh 

}    /*    set_dummy  */

#if 0//OV02B1_EEPROM_ENABLE
static kal_uint16 read_eeprom_module(kal_uint32 addr)
{
	kal_uint16 get_byte = 0;
	char pusendcmd[2] = { (char)(addr >> 8), (char)(addr & 0xFF) };

	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, OV02B1_EEPROM_I2C_ADDR);
...
static int read_eeprom_barcode(void)
{
    ...
    if(barcodeFlag != 0x01)
    {
       printk("ov02b1 barcodeFlag invalid barcodeFlag =0x%x!\n",barcodeFlag);
        return -1;
    }
    ...
    for (i = 0; i < QRCODE_ADDR_LENS; i++)
    {
        ...
        pr_err("ov02b1 sn_code[%d]:%02x,%c",i,sn_code[i],sn_code[i]);
        Sum += sn_code[i];
    }
    pr_err("ov02b1 sn_code:%s",sn_code);

    checkSum=Sum%255;
    if((checkSum&0xff)==read_eeprom_module(QRCODE_CHECKSUM))
    {
        pr_err("ov02b1 barcode checksum sucess\n");
        get_hardware_info_data(HWID_SUB2_CAM_SN, sn_code);
        return 0;
    }
    pr_err("ov02b1 barcode checksum error\n");
    return -1;
}
...
static int get_eeprom_data(EEPROM_DATA *data)
{
    ...
    otp_data = (u8 *)data;
    for (i=0;i<=0x0E;i++)
        pr_err("ov02b1 [simon]otpdata[0x%x]=0x%x \n", i, *(otp_data + i));
#endif
    read_eeprom_barcode();
    return 0;
}
