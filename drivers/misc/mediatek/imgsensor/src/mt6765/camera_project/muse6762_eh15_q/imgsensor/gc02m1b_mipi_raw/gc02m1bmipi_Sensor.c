...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{1600, 1200, 0, 0, 1600, 1200, 1600, 1200, 0000, 0000, 1600, 1200, 0, 0, 1600, 1200},  /*custom1 24fps */
};
...
#if 0
//static kal_uint8 gc02m1b_otp_read_byte(kal_uint8 addr)
static kal_uint8 gc02m1b_otp_identify(void)
{
    ...
    gc02m1b_otp_id = read_cmos_sensor(0x19) & 0x04;
	write_cmos_sensor(0xf7, 0x01);
	write_cmos_sensor(0xfe, 0x00);
	return gc02m1b_otp_id;
}
#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			pr_err("[AR][b]  sensor_id: 0x%x, return_sensor_id: 0x%x\n", *sensor_id, imgsensor_info.sensor_id);
			
			if (*sensor_id == imgsensor_info.sensor_id) {
			#if 0
		        gc02m1b_otp_id = gc02m1b_otp_identify();
		        if(gc02m1b_otp_id == 0x04){
					pr_err("i2c write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id, gc02m1b_otp_id);
					return ERROR_NONE;
		        }else{
		          pr_err("Read sensor id fail, write id: 0x%x, id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, *sensor_id, gc02m1b_otp_id);
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
#if 0
			if ((sensor_id == imgsensor_info.sensor_id) && (gc02m1b_otp_id == 0x04)) {
				pr_err("i2c write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, sensor_id, gc02m1b_otp_id);
				break;
			}
	...
	if ((imgsensor_info.sensor_id != sensor_id) || (gc02m1b_otp_id != 0x04))
		return ERROR_SENSOR_CONNECT_FAIL;
#else
			if (sensor_id == imgsensor_info.sensor_id) {
				pr_err("i2c write id: 0x%x, sensor id: 0x%x\n", imgsensor.i2c_write_id, sensor_id);
