...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.i2c_addr_table = {0x46, 0x44, 0x42, 0x40, 0xff},
	.i2c_speed = 400,
};
...
static kal_uint16 read_sensor_eeprom(kal_uint32 addr)
{
	...
	iReadRegI2C(pu_send_cmd, 2, (u8 *)&get_byte, 1, HI1336_TS_EEPROM);
...
kal_uint16 addr_data_pair_init_hi1336[] = {
	...
	0x1060, 0x340f, //0x3008, //mipi_static3  //power
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x1060, 0x340f); //0x3008); //mipi_static3
...
#if 0//def CONFIG_HQ_HARDWARE_INFO
#define QRCODE_VALID_FLAG 0X0EFB
#define QRCODE_ADDR_BASE 0X0EFC
#define QRCODE_CHECKSUM_L 0X0F0B
#define QRCODE_CHECKSUM_M 0X0F0C
...
static int  get_eeprom_data(EEPROM_DATA *data)
{
	...
	for (i =0x00;i <= 0x0E;i++,otp_data++) {
		*otp_data = read_sensor_eeprom(i);
		...
		printk("[hi1336 otp]%s :[1][0x%x]=[0x%x]",__func__, i,*otp_data);
	}

	read_checksum = read_sensor_eeprom(0x15)|(read_sensor_eeprom(0x16)<<8);
	checksum %= 65535;
	printk("[hi1336 otp]%s :[2][0x%x]=[0x%x]",__func__, checksum,read_checksum);
	read_eeprom_barcode();
