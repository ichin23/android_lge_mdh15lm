...
static void get_hi846_otpInfo(void)
{
	u8 *p = (u8 *)&pOtp_data;
	...
	kal_uint32 i;
	kal_uint16 sn_checkSum = 0, sn_read_checksum = 0;
	kal_uint32 lsc_checkSum = 0, lsc_read_checksum = 0;
	static char sn_code[16]={0};
	kal_uint16 module_otp_addr[3] = {0x0202, 0x020C, 0x0216};
	kal_uint16 sn_addr[3] = {0x1825, 0x1835, 0x1845};
	kal_uint16 lsc_addr[3] = {0x023d, 0x098a, 0x10d7};
	kal_uint8 flag[3] = {0x01,0x13,0x37};
	LOG_INF("%s Enter\n",__func__);
	otp_init_setting();

	//1.read Module info
	write_cmos_sensor_8(0x070a, 0x2); //start address H
	write_cmos_sensor_8(0x070b, 0x1); //start address L
	write_cmos_sensor_8(0x0702, 0x01); //read enable
	flag_value = read_cmos_sensor(0x0708);

	if( (flag_value != 0x01 && flag_value != 0x13 && flag_value != 0x37)){
		LOG_INF("hi846 otp error: %s module info group error\n",__func__);
		return ;
		}
	else{
		//hw_info_sub_otp.otp_valid = 0x01;
		}
	LOG_INF("hi846 module info group flag: 0x%x\n",flag_value);

	for(i = 0 ; i < 3; i++)
	{
	  if(flag_value == flag[i])
	  {
	  	write_cmos_sensor_8(0x070a, (module_otp_addr[i]&0xff00)>>8);
	    write_cmos_sensor_8(0x070b, module_otp_addr[i] & 0x00ff);
	    write_cmos_sensor_8(0x0702, 0x01); //read enable	
	    break;
	  }
	}
	for (i = 0; i < 10; i++) {
		if(i < 9){
			*p = read_cmos_sensor(0x0708);
			//LOG_INF("hi846 otp: %s module info 0x%x\n",__func__,*p);
			checksum += *p;
			p++;
			}
		else if(i==9){
			read_checksum = read_cmos_sensor(0x0708);
			}
	}
	checksum = (checksum%0xFF)+1;	
	if(checksum != read_checksum){
	    LOG_INF("hi846 otp error: %s module info checksum error\n",__func__);
	    return ;
	}
	...
	//2.read LSC data
	write_cmos_sensor_8(0x070a, 0x02); //start address H
	write_cmos_sensor_8(0x070b, 0x3c); //start address L
	write_cmos_sensor_8(0x0702, 0x01); //read enable
	flag_value = read_cmos_sensor(0x0708);
	LOG_INF("hi846 LSC group flag: 0x%x\n",flag_value);
	if( (flag_value != 0x01 && flag_value != 0x13 && flag_value != 0x37)){
		LOG_INF("hi846 otp error: %s LSC group error\n",__func__);
		return ;
	}
	...
	for(i=0 ; i< 3; i++)
	{
	  if(flag_value == flag[i])
	  {
	  	write_cmos_sensor_8(0x070a, (lsc_addr[i]&0xff00)>>8);
	    write_cmos_sensor_8(0x070b, lsc_addr[i] & 0x00ff);
	    write_cmos_sensor_8(0x0702, 0x01); //read enable
	    break;
	  }
	}
	for (i =0; i < (OTP_LSC_SIZE+1); i++) {
		if(i < OTP_LSC_SIZE){
			OTPData[i] = read_cmos_sensor(0x0708);
			lsc_checkSum += OTPData[i];
			//LOG_INF("hi846 LSC: otpdata[%d] 0x%x\n",i,OTPData[i]);
			}
		else if(i == OTP_LSC_SIZE){
			lsc_read_checksum = read_cmos_sensor(0x0708);
			}
	}
	lsc_checkSum = (lsc_checkSum%0xFF)+1;
	//LOG_INF("%s otp lsc info checksum (%d %d)\n",__func__,lsc_checkSum,lsc_read_checksum);
	if(lsc_checkSum != lsc_read_checksum)
		LOG_INF("error: %s otp lsc info checksum error\n",__func__);

	//3.read barcode.
	write_cmos_sensor_8(0x070a, 0x18); //start address H
	write_cmos_sensor_8(0x070b, 0x24); //start address L
	write_cmos_sensor_8(0x0702, 0x01); //read enable

	flag_value = read_cmos_sensor(0x0708);
	LOG_INF("hi846 barcode group flag: 0x%x\n",flag_value);

	if( (flag_value != 0x01 && flag_value != 0x13 && flag_value != 0x37)){
		LOG_INF("hi846 otp error: %s barcode group error\n",__func__);
		return ;
	}
	...
	for(i=0 ; i< 3; i++)
	{
	  if(flag_value == flag[i])
	  {
	  	write_cmos_sensor_8(0x070a, (sn_addr[i]&0xff00)>>8);
	    write_cmos_sensor_8(0x070b, sn_addr[i] & 0x00ff);
	    write_cmos_sensor_8(0x0702, 0x01); //read enable
	    break;
	  }
	}
	for (i =0; i < 16; i++) {
		if(i < 15){
			sn_code[i] = read_cmos_sensor(0x0708);
			sn_checkSum += sn_code[i];
			}
		else if(i == 15){
			sn_read_checksum = read_cmos_sensor(0x0708);
			}
	}
	sn_checkSum = (sn_checkSum%0xFF)+1;

	#if 0
	if(sn_checkSum == sn_read_checksum)
	    get_hardware_info_data(HWID_SUB_CAM_SN, sn_code);
    else
	LOG_INF("error: %s otp sn_code info checksum error\n",__func__);
	#endif
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			//hw_info_sub_otp.sensor_name = PFX;
			return ERROR_NONE;
			}
...
static kal_uint32 open(void)
{
	...
	LOG_INF("[open]: PLATFORM:MT6765,MIPI 4LANE\n");
