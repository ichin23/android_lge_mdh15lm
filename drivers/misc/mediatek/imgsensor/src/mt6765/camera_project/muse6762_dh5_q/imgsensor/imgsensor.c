...
/*MDH5LM OTP bring up*/
mdh5lm_otp_data_t sl556_otp_data;
mdh5lm_otp_data_t sl556_3M_otp_data;
...
static ssize_t show_LGCameraSensorName(struct device *dev,struct device_attribute *attr, char *buf)
{
    ...
    //if(lge_camera_hwinfo == 3) hardcoding for DH5
        return sprintf(buf, "FCam:%s^^RCam:%s^^RCam1:%s\n", front_sensor_name, rear_sensor_name, wide_sensor_name);
