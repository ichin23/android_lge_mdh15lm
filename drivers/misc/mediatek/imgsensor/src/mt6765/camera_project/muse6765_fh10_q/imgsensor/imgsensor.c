...
// [LGE_UPDATE_S] [kyunghun.oh@lge.com] [2018-12-05] Creation of node for camera module identification.
...
// [LGE_UPDATE_E] [kyunghun.oh@lge.com] [2018-12-05] Creation of node for camera module identification.
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	switch(psensor_inst->sensor_idx) {
		...
		case IMGSENSOR_SENSOR_IDX_SUB2:{
			if (ERROR_NONE == err)
				get_hardware_info_data(HWID_SUB_CAM_2, psensor_inst->psensor_name);
			else
				get_hardware_info_data(HWID_SUB_CAM_2, "Not Found!");
			break;
		}
...
{
    pr_err("show_LGCameraSensorName: rear_camera_name [%s] , front_camera_name [%s] , wide_sensor_name [%s] , aux_sensor_name[%s] \n", rear_sensor_name, front_sensor_name, wide_sensor_name, aux_sensor_name);
