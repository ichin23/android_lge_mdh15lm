...
static kal_uint16 read_eeprom_module(kal_uint32 addr)
{
	...
	iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, GC8034_HLT_EEPROM);
...
#define QRCODE_CHECKSUM_L 0X0790
...
static int read_eeprom_barcode(void)
{
    ...
    if(barcodeFlag != 0x01)
    {
       printk("gc8034 barcodeFlag invalid barcodeFlag =0x%x!\n",barcodeFlag);
        return -1;
    }
    ...
    for (i = 0; i < QRCODE_ADDR_LENS; i++)
    {
        ...
        printk("gc8034 sn_code[%d]:%02x,%c",i,sn_code[i],sn_code[i]);
        checkSum += sn_code[i];
    }
    printk("gc8034 sn_code:%s",sn_code);

    checkSum%=255;
	checkSum+=1;
    if(checkSum==read_eeprom_module(QRCODE_CHECKSUM_L))
    {
		printk("gc8034 barcode checksum very good!!!|n");
        get_hardware_info_data(HWID_SUB_CAM_SN, sn_code);
        return 0;
    }
    printk("gc8034 barcode checksum error\n");
    return -1;
}
...
static kal_uint32 return_sensor_id(void)
{
	...
	pr_err("GC8034 return_sensor_id sensor_id =0x%x \n",sensor_id);
	return sensor_id;
}
