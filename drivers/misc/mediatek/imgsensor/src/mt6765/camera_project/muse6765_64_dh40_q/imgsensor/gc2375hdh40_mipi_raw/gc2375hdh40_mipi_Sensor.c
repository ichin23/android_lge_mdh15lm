...
#include "gc2375hdh40_mipi_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc2375hdh40_camera_sensor"
#define LOG_1 cam_pr_debug("GC2375HDH40, MIPI 1LANE\n")
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC2375HDH40_SENSOR_ID,
...
#if GC2375HDH40_MIRROR_FLIP_ENABLE
...
static void sensor_init(void)
{
	...
	write_cmos_sensor(0x17, GC2375HDH40_MIRROR);
	...
	write_cmos_sensor(0x4e, GC2375HDH40_BLK_Select1_H);
	write_cmos_sensor(0x4f, GC2375HDH40_BLK_Select1_L);
	write_cmos_sensor(0x66, GC2375HDH40_BLK_Select2_H);
	write_cmos_sensor(0x67, GC2375HDH40_BLK_Select2_L);
	...
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
	...
	if (imgsensor.current_fps == imgsensor_info.cap1.max_framerate) {
		...
		if (imgsensor.current_fps != imgsensor_info.cap.max_framerate)
			LOG_DEBUG("Warning: current_fps %d fps is not support, so use cap1's setting: %d fps!\n",
	...
	MUINT32 *framerate)
{
	cam_pr_debug("scenario_id = %d\n", scenario_id);
	if (framerate == NULL) {
		LOG_DEBUG("---- framerate is NULL ---- check here\n");
		return ERROR_NONE;
	}
	spin_lock(&imgsensor_drv_lock);
	switch (scenario_id) {
...
UINT32 GC2375HDH40_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
