...
#include "gc5035unionmipiraw_Sensor.h"

/**************** Modify Following Strings for Debug ******************/
#define PFX "gc5035union_camera_sensor"
#define LOG_1 cam_pr_debug("GC5035UNION_MIPI, 2LANE\n")
/********************   Modify end    *********************************/
...
static struct imgsensor_info_struct imgsensor_info = {
	.sensor_id = GC5035UNION_SENSOR_ID,
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((check % 65535 + 1) ==
				...
				cam_pr_debug("data = %d-%d-%d\n",
					...
					module_info.month,
					module_info.day);
				cam_pr_debug("modoule_info checksum group %d pass! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(info[GC5035_OTP_MODULE_DATA_SIZE - 2] | (info[GC5035_OTP_MODULE_DATA_SIZE - 1] << 8)),
					(check % 65535 + 1));
			} else
				cam_pr_debug("modoule_info checksum group %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(info[GC5035_OTP_MODULE_DATA_SIZE - 2] | (info[GC5035_OTP_MODULE_DATA_SIZE - 1] << 8)),
					(check % 65535 + 1));
			break;
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			if ((wb_check % 65535 + 1) ==
				...
				cam_pr_debug("wb checksum group %d pass! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(wb[GC5035_OTP_WB_DATA_SIZE - 2] | (wb[GC5035_OTP_WB_DATA_SIZE - 1] << 8)),
					(wb_check % 65535 + 1));
			} else {
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
				cam_pr_debug("wb checksum group %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(wb[GC5035_OTP_WB_DATA_SIZE - 2] | (wb[GC5035_OTP_WB_DATA_SIZE - 1] << 8)),
					(wb_check % 65535 + 1));
			}
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id() + 2;
			...
			if ((*sensor_id == imgsensor_info.sensor_id) && (moduleID == 0x49)) {
//				gc5035_otp_identify();
				cam_pr_debug("i2c write id: 0x%x, sensor id: 0x%x,moduleID 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id,moduleID);
				return ERROR_NONE;
			}
			cam_pr_debug("xinj:Read sensor id fail, write id: 0x%x, id: 0x%x,moduleID 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id,moduleID);
			retry--;
		} while (retry > 0);
		i++;
	...
	cam_pr_debug("sensor_id: %d moduleID:%d\n", *sensor_id,moduleID);
...
UINT32 GC5035UNION_MIPI_RAW_SensorInit(struct SENSOR_FUNCTION_STRUCT **pfFunc)
{
	cam_pr_debug("E\n");
