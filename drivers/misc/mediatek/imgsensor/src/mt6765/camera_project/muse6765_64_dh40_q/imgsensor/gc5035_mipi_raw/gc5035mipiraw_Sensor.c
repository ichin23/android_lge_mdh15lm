...
#define cam_pr_debug(format, args...) \
	pr_debug(PFX "[%s]:[%d] " format, __func__, __LINE__, ##args)
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[6] = {
	...
	{ 2592, 1944,   0,   0, 2592, 1944, 2592, 1944, 0, 0,
		2592, 1944, 0, 0, 2592, 1944 }//custom1 24fps
};
	...
	kal_uint8 *data, kal_uint16 length)
{
	...
	if ((((addr & 0x1fff) >> 3) + length) > GC5035_OTP_DATA_LENGTH) {
		LGE_DEBUG("out of range, start addr: 0x%.4x, length = %d\n",
			addr & 0x1fff, length);
		return;
	}
...
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_EMPTY: {
			LGE_DEBUG("group %d is empty!\n", idx + 1);
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			LGE_DEBUG("group %d is valid!\n", idx + 1);
			...
			for (i = 0; i < (GC5035_OTP_MODULE_DATA_SIZE - 2); i++)
				check += info[i];

			if ((check % 65535 + 1) ==
					(info[GC5035_OTP_MODULE_DATA_SIZE - 2] | (info[GC5035_OTP_MODULE_DATA_SIZE - 1] << 8))) {
				...
				LGE_DEBUG("module_id = 0x%x\n",
					module_info.module_id);
				LGE_DEBUG("lens_id = 0x%x\n",
					module_info.lens_id);
				LGE_DEBUG("data = %d-%d-%d\n",
					module_info.year,
					module_info.month,
					module_info.day);
				LGE_DEBUG("modoule_info checksum group %d pass! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(info[GC5035_OTP_MODULE_DATA_SIZE - 2] | (info[GC5035_OTP_MODULE_DATA_SIZE - 1] << 8)),
					(check % 65535 + 1));
			} else
				LGE_DEBUG("modoule_info checksum group %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(info[GC5035_OTP_MODULE_DATA_SIZE - 2] | (info[GC5035_OTP_MODULE_DATA_SIZE - 1] << 8)),
					(check % 65535 + 1));
			break;
...
static void gc5035_otp_read_wb_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		switch (GC5035_OTP_GET_2BIT_FLAG(flag, 2 * (1 - idx))) {
		case GC5035_OTP_FLAG_EMPTY: {
			LGE_DEBUG("wb group %d is empty!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			LGE_DEBUG("wb group %d is valid!\n", idx + 1);
			...
			for (i = 0; i < (GC5035_OTP_WB_DATA_SIZE - 2); i++)
				wb_check += wb[i];

			if ((wb_check % 65535 + 1) ==
					(wb[GC5035_OTP_WB_DATA_SIZE - 2] | (wb[GC5035_OTP_WB_DATA_SIZE - 1] << 8))) {
				pWB->rg = (wb[0] | (wb[1] << 8));
				pWB->bg = (wb[2] | (wb[3] << 8));
				pWB->rg = pWB->rg == 0 ?
				...
				LGE_DEBUG("wb r/g = 0x%x\n", pWB->rg);
				LGE_DEBUG("wb b/g = 0x%x\n", pWB->bg);
				LGE_DEBUG("wb checksum group %d pass! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(wb[GC5035_OTP_WB_DATA_SIZE - 2] | (wb[GC5035_OTP_WB_DATA_SIZE - 1] << 8)),
					(wb_check % 65535 + 1));
			} else {
				pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
				LGE_DEBUG("wb checksum group %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					(wb[GC5035_OTP_WB_DATA_SIZE - 2] | (wb[GC5035_OTP_WB_DATA_SIZE - 1] << 8)),
					(wb_check % 65535 + 1));
			}
		...
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			LGE_DEBUG("wb group %d is invalid!\n", idx + 1);
			pWB->flag = pWB->flag | GC5035_OTP_FLAG_INVALID;
			break;
		}
		default:
		...
		case GC5035_OTP_FLAG_VALID: {
			LGE_DEBUG("golden group %d is valid!\n", idx + 1);
			...
			if ((golden_check % 255 + 1) ==
				...
				LGE_DEBUG("golden r/g = 0x%x\n",
						pGolden->rg);
				LGE_DEBUG("golden b/g = 0x%x\n",
						pGolden->bg);
			} else {
				pGolden->flag = pGolden->flag |
					GC5035_OTP_FLAG_INVALID;
				LGE_DEBUG("golden check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
		...
		case GC5035_OTP_FLAG_INVALID:
		case GC5035_OTP_FLAG_INVALID2: {
			LGE_DEBUG("golden group %d is invalid!\n", idx + 1);
...
static void gc5035_otp_update_wb(void)
{
	...
	LGE_DEBUG("typical rg = 0x%x, bg = 0x%x\n", rg_typical, bg_typical);
	...
	if (GC5035_OTP_CHECK_1BIT_FLAG(pWB->flag, 0)) {
		...
		LGE_DEBUG("channel gain r = 0x%x, g = 0x%x, b = 0x%x\n",
...
static void gc5035_otp_function(void)
{
	...
	if (flag == 1) {
		LGE_DEBUG("otp id mismatch, read again");
		memset(&gc5035_otp_data, 0, sizeof(gc5035_otp_data));
		for (i = 0; i < GC5035_OTP_ID_SIZE; i++)
...
static void custom1_setting(void)
{
	...
	/*Fsync slave*/ \
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x85, 0x03);
	write_cmos_sensor(0xfe, 0x00);
	write_cmos_sensor(0x00, 0x09);
	write_cmos_sensor(0x83, 0x08);
	...
	write_cmos_sensor(0x84, 0x81);
	write_cmos_sensor(0x87, 0x09);
	write_cmos_sensor(0x85, 0x53);
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			moduleID = gc5035_otp_identify();
			if ((*sensor_id == imgsensor_info.sensor_id) && (moduleID == 0x04)) {
//				gc5035_otp_identify();
				LGE_DEBUG("i2c write id: 0x%x, sensor id: 0x%x, moduleID 0x%x\n",
					imgsensor.i2c_write_id, *sensor_id, moduleID);
				return ERROR_NONE;
			}
			LGE_DEBUG("Read sensor id fail, write id: 0x%x, id: 0x%x, moduleID 0x%x\n",
				imgsensor.i2c_write_id, *sensor_id,moduleID);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LGE_DEBUG("Read sensor id fail, write id: 0x%x, id: 0x%x\n",
				imgsensor.i2c_write_id, sensor_id);
			retry--;
		} while (retry > 0);
		i++;
...
static kal_uint32 custom1(MSDK_SENSOR_EXPOSURE_WINDOW_STRUCT *image_window,
	MSDK_SENSOR_CONFIG_STRUCT *sensor_config_data)
{
    printk("[gc5035] custom1 mode start\n");
	spin_lock(&imgsensor_drv_lock);
	...
	MUINT32 *framerate)
{
	...
	if (framerate == NULL) {
		LGE_DEBUG("---- framerate is NULL ---- check here\n");
		return ERROR_NONE;
	}
	spin_lock(&imgsensor_drv_lock);
	switch (scenario_id) {
