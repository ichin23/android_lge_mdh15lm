...
#define DEVICE_VERSION_S5K3P9SX    "s5k3p9sx"
...
static kal_uint32 streaming_control(kal_bool enable)
{
	...
	if (enable) {
		...
		LOG_DEBUG("Stream Off Fail! framecnt=%d.\n", framecnt);
	}
	return ERROR_NONE;
}
...
static void sensor_init(void)
{
	...
	LOG_DEBUG("Using Burst Mode for TNP (%d)\n",(int)sizeof(uTnpArrayA));
	...
	write_cmos_sensor(0x6F12, 0x0100); //th0 en
	write_cmos_sensor(0x6F12, 0x002F); //th0 lowth 
	write_cmos_sensor(0x602A, 0x0F76); 
	write_cmos_sensor(0x6F12, 0x0030); //th0 highth
	write_cmos_sensor(0x602A, 0x0F7A); 
	write_cmos_sensor(0x6F12, 0x000B); //th0 low val
	write_cmos_sensor(0x6F12, 0x0009); //th0 high val
	write_cmos_sensor(0x6F12, 0xF46E); //th0 addr
	...
	write_cmos_sensor(0x6F12, 0xD123);
	...
	write_cmos_sensor(0x0D80, 0x1388); //case2
	...
	write_cmos_sensor(0x6F12, 0x0000);
	//xxxx add for modify T5 must be >= 0.1ms in power up sequence
	write_cmos_sensor(0x6028, 0x4000);
	write_cmos_sensor(0xB134, 0x0000);
	write_cmos_sensor(0xB136, 0x0000);
	write_cmos_sensor(0xB138, 0x0000);
	
	//xxxx add for modify SHBN
	write_cmos_sensor(0x6028, 0x2000); //
	write_cmos_sensor(0x602A, 0x16AA);
	write_cmos_sensor(0x6F12, 0x009D); //main_2_value 2018.0801 
	
	
	write_cmos_sensor(0x602A, 0x2690);
	write_cmos_sensor(0x6F12, 0x0000);
	write_cmos_sensor(0x602A, 0x2692); 
	write_cmos_sensor(0x6F12, 0x0200); //ISP Delay 2018.0801 
	
	write_cmos_sensor(0x602A, 0x0F70); //CRB Bias 2018.0801
	write_cmos_sensor(0x6F12, 0x0101); //en th0 and th1
	write_cmos_sensor(0x602A, 0x0F74); 
	write_cmos_sensor(0x6F12, 0x007F); //th1 lowth 
	write_cmos_sensor(0x602A, 0x0F78); 
	write_cmos_sensor(0x6F12, 0x0080); //th1 highth
	write_cmos_sensor(0x602A, 0x0FAA); 
	write_cmos_sensor(0x6F12, 0x000D); //th1 low value	
	write_cmos_sensor(0x602A, 0x0FAC); 
	write_cmos_sensor(0x6F12, 0x0004); //th1 high value
	write_cmos_sensor(0x602A, 0x0FAE); 
	write_cmos_sensor(0x6F12, 0xF464); //th1 addr	
	
	write_cmos_sensor(0x602A, 0x0E86); //MIPI Threshold 2018.0801
...
{
	LOG_DEBUG("capture_setting() E! currefps:%d\n", currefps);
...
{
	LOG_DEBUG("normal_video_setting() E! currefps:%d\n", currefps);
...
static kal_uint32 open(void)
{
	...
	LOG_DEBUG("PLATFORM:MT6595,MIPI 2LANE\n");
	LOG_DEBUG("preview 1280*960@30fps,864Mbps/lane; video 1280*960@30fps,864Mbps/lane; capture 5M@30fps,864Mbps/lane\n");
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			LOG_DEBUG("Read sensor id fail, id: 0x%x\n", imgsensor.i2c_write_id);
			retry--;
		} while(retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id) {
							 ...
							 UINT8 *feature_para,UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
         ...
         *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 3000000; //This value needs to be filled in based on the actual test value!
