...
static void slim_video_setting(void)
{
	kal_uint8 framecnt;
	int i;
	LOG_DEBUG("slim_video_setting() E\n");
...
static kal_uint32 open(void)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			sensor_id = return_sensor_id();
			LOG_DEBUG("s5k4h7yxmipiraw open sensor_id = %x\r\n", sensor_id);
			if (sensor_id == imgsensor_info.sensor_id) {
							 ...
							 UINT8 *feature_para, UINT32 *feature_para_len)
{
	...
	switch (feature_id) {
         ...
         *(MUINT32 *)(uintptr_t)(*(feature_data + 1)) = 2500000; //This value needs to be filled in based on the actual test value!
    break; //for ITS sensor fusion
		...
		case SENSOR_FEATURE_SET_HDR:
			LOG_DEBUG("Warning! Not Support IHDR Feature");
			spin_lock(&imgsensor_drv_lock);
