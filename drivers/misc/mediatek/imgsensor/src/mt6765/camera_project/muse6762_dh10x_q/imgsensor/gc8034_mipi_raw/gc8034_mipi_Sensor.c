...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
				gc8034_otp_identify();
				printk("[gc8034_camera_sensor]get_imgsensor_id:i2c write id: 0x%x, sensor id: 0x%x\n",
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		...
		*sensor_id = 0xFFFFFFFF;
		printk("[gc8034_camera_sensor] read fail!!");
...
{
	LOG_INF("E\n");
	printk("[GC8034] Start Preview mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
	printk("[GC8034] Start Capture mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	LOG_INF("E\n");
	printk("[GC8034] Start NoramlVideo mode\n");
	spin_lock(&imgsensor_drv_lock);
