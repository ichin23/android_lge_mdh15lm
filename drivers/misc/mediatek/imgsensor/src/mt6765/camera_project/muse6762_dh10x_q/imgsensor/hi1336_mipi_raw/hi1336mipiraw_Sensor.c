...
static struct imgsensor_info_struct imgsensor_info = {
    ...
    .custom1 = {
		...
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate = 240,
		.mipi_pixel_rate = 403200000, //(1008M * 4 / 10 )
	},
    .custom2 = {
		.pclk = 600000000,
		.linelength = 7504,
...
#ifdef CONFIG_HQ_HARDWARE_INFO
#define QRCODE_VALID_FLAG 0X073b
#define QRCODE_ADDR_BASE 0X073c
#define QRCODE_CHECKSUM_L 0X0756
#define QRCODE_CHECKSUM_M 0X0757
...
static int read_eeprom_barcode(void)
{
    ...
    if(barcodeFlag != 0x01)
    {
        printk("hi1336 barcodeFlag invalid !\n");
        return -1;
    }
    printk ("hi1336  barcodeFlag valid !barcodeFlag =0x%x!\n",barcodeFlag);
    ...
    printk("hi1336 sn_code:%s",sn_code);
    checkSum%=65535;
...
#endif
/*  kal_uint16 index;
    for(index=0; index<=0X1973; index++)
    {
		LOG_INF("OTP DATA:ADDR:%04x,DATA:%02x \n",index,read_sensor_eeprom(index));
    }*/
    printk("hi1336  barcode checksum error\n");
    return -1;
}
static int  get_eeprom_data(EEPROM_DATA *data)
{
	...
	for (i =0x00;i <=0x0E;i++,otp_data++) {
		*otp_data = read_sensor_eeprom(i);
		checksum += *otp_data;
		printk("[hi1336 barcode]%s :[1][0x%x]=[0x%x]",__func__, i,*otp_data);
	}

	read_checksum = read_sensor_eeprom(0x19)|(read_sensor_eeprom(0x1A)<<8);
	checksum %= 65535;
	printk("[hi1336 barcode]%s :[2][0x%x]=[0x%x]",__func__, checksum,read_checksum);
	read_eeprom_barcode();
...
static kal_uint32 get_imgsensor_id(UINT32 *sensor_id)
{
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			*sensor_id = return_sensor_id();
			...
			printk("hi1336 i2c write id : 0x%x, sensor id: 0x%x\n",
	...
	if (*sensor_id != imgsensor_info.sensor_id) {
		LOG_INF("hi1336 Read id fail,sensor id: 0x%x\n", *sensor_id);
...
static kal_uint32 open(void)
{
    ...
    printk("[hi1336] Open sensor\n");
	pr_err("[HI1336][open]: PLATFORM:MT6737,MIPI 24LANE\n");
...
{
	LOG_INF("E");
    printk("[hi1336] Start preview mode\n");
	spin_lock(&imgsensor_drv_lock);
...
{
	spin_lock(&imgsensor_drv_lock);
    printk("[hi1336] Start capture mode\n");
...
{
	spin_lock(&imgsensor_drv_lock);
    printk("[hi1336] Start NormalVideo mode\n");
