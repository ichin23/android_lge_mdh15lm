...
static kal_uint8 read_sensor_eeprom(kal_uint32 addr)
{
...
{
	return read_sensor_eeprom(addr);
}

static void gc5035_otp_read_group(kal_uint16 addr,
	kal_uint8 *otp_data, kal_uint16 length)
{
	kal_uint16 i = 0;
	kal_uint16 j = 0;

	if ((addr + length) > GC5035_OTP_DATA_LENGTH) {
		cam_pr_debug("out of range, start addr: 0x%.4x, length = %d\n",
			addr, length);
		return;
	}
	for (i = 0, j = addr; i < length; i++, otp_data++){
		*otp_data = read_sensor_eeprom(j);
...
/*
static char sn_code[GC5035_OTP_BARCODE_DATA_SIZE + 1];
static int gc5035_otp_read_barcode(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	...
	flag = gc5035_otp_read_byte(GC5035_OTP_BARCODE_FLAG_OFFSET);
	cam_pr_debug("flag = 0x%x\n", flag);

	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		switch (GC5035_OTP_GET_2BIT_FLAG(flag, 0)) {
		...
		case GC5035_OTP_FLAG_VALID: {
			...
			gc5035_otp_read_group(GC5035_OTP_BARCODE_DATA_OFFSET,
				&sn_code[0], GC5035_OTP_BARCODE_DATA_SIZE + 1);
			for (i = 0; i < GC5035_OTP_BARCODE_DATA_SIZE; i++){
				check += sn_code[i];
				cam_pr_debug("sn:0x%x, check:0x%x\n", sn_code[i], check);
			}

    		if ((check % 255 + 1) ==
					sn_code[GC5035_OTP_BARCODE_DATA_SIZE]) {
				get_hardware_info_data(HWID_MAIN_CAM_2, sn_code);
				return 0;
			} else
...
static kal_uint8 gc5035_otp_read_module_info(void)
{
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_VALID: {
    		...
    		if ((check % 255 + 1) ==
					...
					hw_info_main2_otp.otp_valid = flag;
					hw_info_main2_otp.vendor_id = info[0];//supplier_code
					hw_info_main2_otp.module_code = info[1];//Module code
					hw_info_main2_otp.module_ver = info[2];//module_ver
					hw_info_main2_otp.sw_ver = info[3];//sw_ver
					hw_info_main2_otp.year = info[4];
					hw_info_main2_otp.month = info[5];
					hw_info_main2_otp.day = info[6];
					hw_info_main2_otp.vcm_vendorid = info[10];

				cam_pr_debug("vendor_id = 0x%x\n",
					hw_info_main2_otp.vendor_id);
				cam_pr_debug("module_code = 0x%x\n",
					hw_info_main2_otp.module_code);
				cam_pr_debug("module_ver = 0x%x\n",
					hw_info_main2_otp.module_ver);
				cam_pr_debug("data = %d-%d-%d\n",
					hw_info_main2_otp.year,
					hw_info_main2_otp.month,
					hw_info_main2_otp.day);
			} else
				cam_pr_debug("check sum %d error! check sum = 0x%x, calculate result = 0x%x\n",
					idx + 1,
					info[GC5035_OTP_MODULE_DATA_SIZE - 1],
	...
	return hw_info_main2_otp.module_code;
}

#if 0
static void gc5035_otp_read_awb_info(void)
{
	kal_uint8 i = 0;
	kal_uint8 idx = 0;
	kal_uint8 flag = 0;
	kal_uint16 awb_check = 0;
	kal_uint16 awb_start_offset = GC5035_OTP_WB_DATA_OFFSET;
	kal_uint8 awb[GC5035_OTP_WB_DATA_SIZE];
	struct gc5035_awb_t *pAWB = &gc5035_otp_data.awb;

	memset(&awb, 0, GC5035_OTP_WB_DATA_SIZE);
	...
	for (idx = 0; idx < GC5035_OTP_GROUP_CNT; idx++) {
		...
		case GC5035_OTP_FLAG_EMPTY: {
			...
			pAWB->awb_flag = pAWB->awb_flag | GC5035_OTP_FLAG_EMPTY;
			break;
		}
		case GC5035_OTP_FLAG_VALID: {
			cam_pr_debug("wb group %d is valid!\n", idx + 1);
			awb_start_offset = GC5035_OTP_WB_DATA_OFFSET
				+ GC5035_OTP_GET_OFFSET(idx *
				GC5035_OTP_WB_DATA_SIZE);
			gc5035_otp_read_group(awb_start_offset, &awb[0],
				GC5035_OTP_WB_DATA_SIZE);
			...
			if ((awb_check % 255 + 1) ==
					awb[GC5035_OTP_WB_DATA_SIZE - 1]) {
				//pAWB->rg = (awb[0] | ((awb[1] & 0xf0) << 4));
				//pAWB->bg = (((awb[1] & 0x0f) << 8) | awb[2]);
				//pAWB->rg = pAWB->rg == 0 ?
				//	GC5035_OTP_WB_RG_TYPICAL : pAWB->rg;
				//pAWB->bg = pAWB->bg == 0 ?
				//	GC5035_OTP_WB_BG_TYPICAL : pAWB->bg;
				pAWB->awb_flag = 1;
				pAWB->awb_r = (awb[0] | ((awb[1] & 0xf0) << 4));
				pAWB->awb_gr = (awb[2] | ((awb[3] & 0xf0) << 4));
				cam_pr_debug("wb r/g = 0x%x\n", pAWB->awb_r);
				cam_pr_debug("wb b/g = 0x%x\n", pAWB->awb_gr);
			} else {
				pAWB->awb_flag = pAWB->awb_flag | GC5035_OTP_FLAG_INVALID;
...
{
	kal_uint8 module_code = 0;
	//kal_uint8 ret_barcode = 0;
#if GC5035_OTP_DEBUG
	kal_uint16 i = 0;
...
#if GC5035_OTP_FOR_CUSTOMER
	module_code = gc5035_otp_read_module_info();
	//gc5035_otp_read_awb_info();
	/*
	ret_barcode = gc5035_otp_read_barcode();
	if (ret_barcode == 0){
		cam_pr_debug("read ret_barcode success\n");
	}else
		cam_pr_debug("read ret_barcode failed\n");
*/
...
static kal_uint8 gc5035_otp_identify(void)
{
	...
	module_code = gc5035_otp_read_sensor_info();
	cam_pr_debug("module_code:0x%x\n", module_code);
