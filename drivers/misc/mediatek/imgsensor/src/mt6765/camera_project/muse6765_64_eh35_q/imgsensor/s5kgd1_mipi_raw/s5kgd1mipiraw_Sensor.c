...
static struct imgsensor_info_struct imgsensor_info = {
	...
	.custom2 = {
		...
		.startx= 0,
		.starty = 0,
		.grabwindow_width  = 6112,
		.grabwindow_height = 4576,
		.mipi_data_lp2hs_settle_dc = 85,
		.max_framerate =150,
...
static struct SENSOR_WINSIZE_INFO_STRUCT imgsensor_winsize_info[7] = {
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
 ...
 { 6560, 4928,  228,  184, 6112, 4576, 3056, 2288,   0, 0, 3056, 2288,   0, 0, 3056, 2288},
#endif
 { 6560, 4928,  228,  184, 6112, 4576, 6112, 4576,   0, 0, 6112, 4576,   0, 0, 6112, 4576}
};
...
static void set_dummy(void)
{
	...
	static kal_uint16 read_eeprom_module(kal_uint32 addr)
	{
		...
		iReadRegI2C(pusendcmd, 2, (u8 *) &get_byte, 1, S5KGD1_LY_EEPROM);
...
#if 0//def CONFIG_HQ_HARDWARE_INFO
  #define DEBUG 0
  #define QRCODE_VALID_FLAG 0X0ED0
  #define QRCODE_ADDR_BASE 0X0ED1
  #define QRCODE_CHECKSUM_L 0X0EE0
  #define QRCODE_RESERVED 0X00
  #define QRCODE_ADDR_LENS (QRCODE_CHECKSUM_L - QRCODE_ADDR_BASE - QRCODE_RESERVED)
  ...
  static int read_eeprom_barcode(void)
  {
	  ...
	  if(barcodeFlag != 0x01)
	  {
		 printk("S5KGD1 barcodeFlag invalid barcodeFlag =0x%x!\n",barcodeFlag);
		  return -1;
	  }
	  ...
	  for (i = 0; i < QRCODE_ADDR_LENS; i++)
	  {
		  ...
		  printk("S5KGD1 sn_code[%d]:%02x,%c",i,sn_code[i],sn_code[i]);
		  checkSum += sn_code[i];
	  }
	  printk("S5KGD1 sn_code:%s",sn_code);

	  checkSum%=255;
	  checkSum+=1;
	  if(checkSum==read_eeprom_module(QRCODE_CHECKSUM_L))
	  {
		  printk("S5KGD1 barcode checksum very good!!!|n");
		  get_hardware_info_data(HWID_SUB_CAM_SN, sn_code);
		  return 0;
	  }
	  printk("S5KGD1 barcode checksum error\n");
	  return -1;
  }
...
static int get_eeprom_data(EEPROM_DATA *data)
{
	...
	otp_data = (u8 *)data;
	for (i=0;i<=0x0E;i++)
		LOG_INF_NEW(" [simon]otpdata[0x%x]=0x%x    ", i, *(otp_data + i));
#endif
	read_eeprom_barcode();
...
static kal_uint16 set_gain(kal_uint16 gain)
{
	...
	spin_unlock(&imgsensor_drv_lock);
	LOG_INF("set_gain = %d , reg_gain = 0x%x\n ", gain, reg_gain);
...
{
#if STEREO_CUSTOM1_30FPS
	LOG_INF("custom1_setting() 6 M*30 fps E!\n");
#else
	LOG_INF("custom1_setting() 6 M*24 fps E!\n");
#endif
