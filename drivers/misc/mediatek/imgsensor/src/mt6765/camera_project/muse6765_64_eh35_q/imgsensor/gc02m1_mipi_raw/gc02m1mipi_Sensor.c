...
static int read_eeprom_barcode(void)
{
    ...
    if((checkSum&0xff)==read_eeprom_module(QRCODE_CHECKSUM))
    {
        ...
        get_hardware_info_data(HWID_SUB2_CAM, sn_code);
        return 0;
    }
	...
	while (imgsensor_info.i2c_addr_table[i] != 0xff) {
		...
		do {
			...
			if ((sensor_id == imgsensor_info.sensor_id) && (gc02m1_otp_id == 0x00)) {
				LOG_INF("i2c write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, sensor_id, gc02m1_otp_id);
				break;
			}
			LOG_INF("Read sensor id fail, write id: 0x%x, sensor id: 0x%x, otp id: 0x%x\n", imgsensor.i2c_write_id, sensor_id, gc02m1_otp_id);
			retry--;
		} while (retry > 0);
		i++;
		if (sensor_id == imgsensor_info.sensor_id)
