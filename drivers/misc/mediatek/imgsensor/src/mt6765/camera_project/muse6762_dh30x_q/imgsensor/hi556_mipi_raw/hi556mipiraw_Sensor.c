...
#if HI556_OTP
#define HI556_OTP_SIZE 0x0ec8+1
unsigned char hi556_otp_data[HI556_OTP_SIZE];
...
unsigned char hi556_module_info_data[MODULE_INFO_DATA_SIZE] = {0};
...
unsigned char hi556_awb_51k_data_valid = 0;
...
unsigned char hi556_awb_30k_data_valid = 0;
...
unsigned char hi556_lsc_51k_data_valid = 0;
...
unsigned char hi556_lsc_40k_data_valid = 0;
#endif
...
int read_hi556_otp(void){
		...
		for(i = 0; i < HI556_OTP_SIZE; i++){
				hi556_otp_data[i]	= read_cmos_sensor(i);
				//pr_err("=== hi556_otp_data[%x]: %x ===\n", i, hi556_otp_data[i]);
		}
		imgsensor.i2c_write_id = addr;
		//check module info
		for(i = MODULE_INFO_FLAG+1; i < MODULE_INFO_CHECKSUM; i++){
					sum += hi556_otp_data[i];
		}
		
		if((hi556_otp_data[MODULE_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi556_otp_data[MODULE_INFO_CHECKSUM]){
			LOG_DEBUG("Mouule   ID 0x%x ", hi556_otp_data[1]);
			LOG_DEBUG("Position ID 0x%x ", hi556_otp_data[2]);
			LOG_DEBUG("Lens     ID 0x%x ", hi556_otp_data[3]);
			LOG_DEBUG("VCM      ID 0x%x ", hi556_otp_data[4]);
			LOG_DEBUG("Year: %d Month: %d Data: %d", hi556_otp_data[5], hi556_otp_data[6], hi556_otp_data[7]);
		}else{
				pr_err("=== hi556_read module info failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi556_otp_data[AWB_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi556_otp_data[AWB_51K_INFO_CHECKSUM]){
			hi556_awb_51k_data_valid = 1;
			pr_err("=== hi556_read module info awb 51k OK!\n");
		}else{
				pr_err("=== hi556_read module info AWB 51k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi556_otp_data[AWB_30K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi556_otp_data[AWB_30K_INFO_CHECKSUM]){
			hi556_awb_30k_data_valid = 1;
			pr_err("=== hi556_read module info awb 30k OK!\n");
		}else{
				pr_err("=== hi556_read module info AWB 30k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi556_otp_data[LSC_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi556_otp_data[LSC_51K_INFO_CHECKSUM]){
			hi556_lsc_51k_data_valid = 1;
			pr_err("=== hi556_read module info LSC 51k OK!\n");
		}else{
				pr_err("=== hi556_read module info lsc 51k failed ===\n");
				return 0;
		}
		sum = 0;
		...
		if((hi556_otp_data[LSC_40K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == hi556_otp_data[LSC_40K_INFO_CHECKSUM]){
			hi556_lsc_40k_data_valid = 1;
			pr_err("=== hi556_read module info LSC 40k OK!\n");
		}else{
				return 0;
		}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
              strcpy(wide_sensor_name, "hi556_jsl");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
#if HI556_OTP 
			ret = read_hi556_otp();
			if(!ret){
					LOG_DEBUG("get eeprom data failed\n");
