...
#if GC5035_OTP
#define GC5035_OTP_SIZE 0x0ec8+1
unsigned char gc5035_otp_data[GC5035_OTP_SIZE];
...
unsigned char gc5035_module_info_data[MODULE_INFO_DATA_SIZE] = {0};
...
unsigned char gc5035_awb_51k_data_valid = 0;
...
unsigned char gc5035_awb_30k_data_valid = 0;
...
unsigned char gc5035_lsc_51k_data_valid = 0;
...
unsigned char gc5035_lsc_40k_data_valid = 0;
#endif
...
static kal_uint16 read_cmos_eeprom(kal_uint32 addr)
{
...
#if GC5035_OTP
int read_gc5035_otp(void){
		int addr = imgsensor.i2c_write_id;
		int i, sum = 0;
		imgsensor.i2c_write_id = 0xA2;
		for(i = 0; i < GC5035_OTP_SIZE; i++){
				gc5035_otp_data[i] = read_cmos_eeprom(i);
				//pr_err("=== gc5035_otp_data[%x]: %x ===\n", i, gc5035_otp_data[i]);
		}
		imgsensor.i2c_write_id = addr;
		//check module info
		for(i = MODULE_INFO_FLAG+1; i < MODULE_INFO_CHECKSUM; i++){
					sum += gc5035_otp_data[i];
		}
		if((gc5035_otp_data[MODULE_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == gc5035_otp_data[MODULE_INFO_CHECKSUM]){
			LOG_INF("Mouule   ID 0x%x ", gc5035_otp_data[1]);
			LOG_INF("Position ID 0x%x ", gc5035_otp_data[2]);
			LOG_INF("Lens     ID 0x%x ", gc5035_otp_data[3]);
			LOG_INF("VCM      ID 0x%x ", gc5035_otp_data[4]);
			LOG_INF("Year: %d Month: %d Data: %d", gc5035_otp_data[5], gc5035_otp_data[6], gc5035_otp_data[7]);
		}else{
				pr_err("=== gc5035_read module info failed ===\n");
				return 0;
		}
		sum = 0;
		//check module awb 51k info
		for(i = AWB_51K_INFO_FLAG+1; i < AWB_51K_INFO_CHECKSUM; i++){
					sum += gc5035_otp_data[i];
		}
		if((gc5035_otp_data[AWB_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == gc5035_otp_data[AWB_51K_INFO_CHECKSUM]){
			gc5035_awb_51k_data_valid = 1;
			pr_err("=== gc5035_read module info AWB 51k OK!\n");
		}else{
				pr_err("=== gc5035_read module info awb 51k failed ===\n");
				return 0;
		}
		sum = 0;
		//check module awb 30k info
		for(i =AWB_30K_INFO_FLAG+1; i < AWB_30K_INFO_CHECKSUM; i++){
					sum += gc5035_otp_data[i];
		}
		if((gc5035_otp_data[AWB_30K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == gc5035_otp_data[AWB_30K_INFO_CHECKSUM]){
			gc5035_awb_30k_data_valid = 1;
			pr_err("=== gc5035_read module info AWB 30k OK!\n");
		}else{
				pr_err("=== gc5035_read module info awb 30k failed ===\n");
				return 0;
		}
		sum = 0;
		//check module lsc 51k info
		for(i = LSC_51K_INFO_FLAG+1; i < LSC_51K_INFO_CHECKSUM; i++){
					sum += gc5035_otp_data[i];
		}
		if((gc5035_otp_data[LSC_51K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == gc5035_otp_data[LSC_51K_INFO_CHECKSUM]){
			gc5035_lsc_51k_data_valid = 1;
			pr_err("=== gc5035_read module info LSC 51k OK!\n");
		}else{
				pr_err("=== gc5035_read module info lsc 51k failed ===\n");
				return 0;
		}
		sum = 0;
		//check module lsc 40k info
		for(i = LSC_40K_INFO_FLAG+1; i < LSC_40K_INFO_CHECKSUM; i++){
					sum += gc5035_otp_data[i];
		}
		if((gc5035_otp_data[LSC_40K_INFO_FLAG] = 0x55) && (sum % 0xff + 1) == gc5035_otp_data[LSC_40K_INFO_CHECKSUM]){
			gc5035_lsc_40k_data_valid = 1;
			pr_err("=== gc5035_read module info LSC 40k OK!\n");
		}else{
				return 0;
		}
		...
		do {
			*sensor_id = return_sensor_id();
			if (*sensor_id == imgsensor_info.sensor_id) {
                strcpy(wide_sensor_name, "gc5035_txd");/*LGE_CHANGE, 2019-07-04, add the camera identifying logic , kyunghun.oh@lge.com*/
