...
static void check_for_regulator_get(struct REGULATOR *preg, struct device
*pdevice, int index);
...
//pr_err("regulator_dbg regulator_set sensor_idx %d, regulator %s,
//status %d\n", sensor_idx, regulator_control[(reg_type_offset + pin -IMGSENSOR_HW_PIN_AVDD)].pregulator_type,
