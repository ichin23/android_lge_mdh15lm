...
/*Murphy Zhou add & show camera sensor name zhoucong@huaqin.com*/
#ifdef CONFIG_HQ_HARDWARE_INFO
#include <linux/hardware_info.h>
#endif
/*Murphy Zhou add & show camera sensor name*/
...
/*LGE_CHANGE_S, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
char *sensor_configs_mh4p_triple = " ov16885_mipi_raw s5k3p9_mipi_raw s5k3l6_mipi_raw ov13855_mipi_raw hi556_mipi_raw gc5035_mipi_raw sp2509v_mipi_raw gc2375h_mipi_raw ";
int sensor_type_mh4p_triple[9] = {0, 0, 1, 1, 2, 2, 4, 4, -1};

char *sensor_configs_mh4p_dual = " hi1336_mipi_raw s5k3l6txd_mipi_raw s5k3l6_mipi_raw ov13855_mipi_raw sp2509v_dual_mipi_raw gc2375htxd_mipi_raw ";
int sensor_type_mh4p_dual[7] = {0, 0, 1, 1, 2, 2, -1};

static int lge_camera_hwinfo = 0;
/*LGE_CHANGE_E, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/

// LGE_CHANGE_S, Fix for Black Screen Issue (ISP timeout Error) - ALPS05199962, ranjith.k@lge.com, 29th June 2020
/*prevent imgsensor race condition in vulunerbility test*/
struct mutex imgsensor_mutex;
// LGE_CHANGE_E, Fix for Black Screen Issue (ISP timeout Error) - ALPS05199962, ranjith.k@lge.com, 29th June 2020
 ...
 ************************************************************************/
#define IMGSENSOR_FUNCTION_ENTRY()    /*pr_err("[%s]:E\n",__FUNCTION__)*/
#define IMGSENSOR_FUNCTION_EXIT()     /*pr_err("[%s]:X\n",__FUNCTION__)*/
...
static inline int imgsensor_check_is_alive(struct IMGSENSOR_SENSOR *psensor)
{
	...
	if (err != ERROR_NONE)
		pr_err("ERROR: No imgsensor alive\n");
	...
	switch(psensor_inst->sensor_idx) {
		case IMGSENSOR_SENSOR_IDX_MAIN: {
			if (ERROR_NONE == err)
				get_hardware_info_data(HWID_MAIN_CAM, psensor_inst->psensor_name);
			else
				get_hardware_info_data(HWID_MAIN_CAM, "Not Found!");
			break;
		}
		case IMGSENSOR_SENSOR_IDX_SUB: {
			if (ERROR_NONE == err)
				get_hardware_info_data(HWID_SUB_CAM, psensor_inst->psensor_name);
			else
				get_hardware_info_data(HWID_SUB_CAM, "Not Found!");
			break;
		}
		case IMGSENSOR_SENSOR_IDX_MAIN2:{
			if (ERROR_NONE == err)
				get_hardware_info_data(HWID_MAIN_CAM_2, psensor_inst->psensor_name);
			else
				get_hardware_info_data(HWID_MAIN_CAM_2, "Not Found!");
			break;
		}
		case IMGSENSOR_SENSOR_IDX_MAIN3:{
			if (ERROR_NONE == err)
				get_hardware_info_data(HWID_MAIN_CAM_3, psensor_inst->psensor_name);
			else
				get_hardware_info_data(HWID_MAIN_CAM_3, "Not Found!");
			break;
		}
		default: {
			pr_err("other camera, not main & sub camera!");
			break;
		}
	}
	#endif
...
/*LGE_CHANGE_S, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
static int __init lge_camera_hwinfo_setup(char *hydra)
{
	pr_err("__Shamyu__ %s\n", hydra);
	if (!strcmp(hydra, "Plus")) {
		lge_camera_hwinfo = 3; //triple
		pr_err("[LGE] Triple camera\n");
	} else if(!strcmp(hydra, "Prime")) {
		lge_camera_hwinfo = 2; //dual
		pr_err("[LGE] Triple camera\n");
	} else {
		lge_camera_hwinfo = -1; //error
		pr_err("[LGE]Error!!!! reading hydra\n");
	}
	return 1;
}
__setup("androidboot.vendor.lge.hydra=", lge_camera_hwinfo_setup);

int lge_get_camera_hwinfo(void)
{
	return lge_camera_hwinfo;
}
/*LGE_CHANGE_E, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
...
/*LGE_CHANGE_S, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
      char *sensor_configs = NULL;
      //lge_get_camera_hwinfo();

    pr_err("__Shamyu__ lge_camera_hwinfo - %d\n", lge_camera_hwinfo);
	if (lge_camera_hwinfo == 3) {
	  	sensor_configs=  sensor_configs_mh4p_triple;
	}
	else if (lge_camera_hwinfo == 2) {
	  	sensor_configs=  sensor_configs_mh4p_dual;
	}
	else {
		pr_err("[LGE]Error!!!! Failed to set sensor list\n");
		sensor_configs = STRINGIZE(CONFIG_CUSTOM_KERNEL_IMGSENSOR);
	}
/*LGE_CHANGE_E, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
...
/*LGE_CHANGE_S, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
	if (lge_camera_hwinfo == 3) {
		if(sensor_type_mh4p_triple[i] != psensor->inst.sensor_idx) {
			pr_err("[LGE] skip sensor : sensor_type_mh4p_triple : %d, sensor_idx : %d", sensor_type_mh4p_triple[i], psensor->inst.sensor_idx);
			continue;
		}
	} else if (lge_camera_hwinfo == 2) {
		if(sensor_type_mh4p_dual[i] != psensor->inst.sensor_idx) {
			pr_err("[LGE] skip sensor : sensor_type_mh4p_dual : %d, sensor_idx : %d", sensor_type_mh4p_dual[i], psensor->inst.sensor_idx);
			continue;
		}
	}
/*LGE_CHANGE_E, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/

		pr_err("[__Shamyu__] skip sensor : sensor_type_mh4p_triple : %d, sensor_idx : %d", sensor_type_mh4p_triple[i], psensor->inst.sensor_idx);
...
static inline int adopt_CAMERA_HW_GetInfo(void *pBuf)
{
	...
	if (copy_to_user((void __user *) (pSensorGetInfo->pConfig),
		...
		pr_err("[CAMERA_HW][config] ioctl copy to user failed\n");
	...
	MUINT32 *temp)
{
	...
	*temp  = 0;
	...
	if (psensor_inst->state != IMGSENSOR_STATE_CLOSE) {
		...
		pr_err("senDevId(%d), temperature(%d)\n", senDevId, *temp);
	...
	struct work_struct *data)
{
	...
	pr_err("Temperature Meter Report.\n");
	...
	pr_err("senDevId(%d), valid(%d), temperature(%d)\n",
...
static inline int adopt_CAMERA_HW_GetInfo2(void *pBuf)
{
	...
	if (pSensorGetInfo == NULL ||
		...
		pr_err("[adopt_CAMERA_HW_GetInfo2] NULL arg.\n");
		return -EFAULT;
	}

	psensor = imgsensor_sensor_get_inst(pSensorGetInfo->SensorId);
	if (psensor == NULL) {
		pr_err("[adopt_CAMERA_HW_GetInfo2] NULL psensor.\n");
		return -EFAULT;
	}

	pr_err("[adopt_CAMERA_HW_GetInfo2]Entry%d\n",
	...
	if (copy_to_user(
		...
		pr_err("[CAMERA_HW][Resolution] ioctl copy to user failed\n");
		ret = -EFAULT;
		goto IMGSENSOR_GET_INFO_RETURN;
	}
...
static inline int adopt_CAMERA_HW_FeatureControl(void *pBuf)
{
	...
	case SENSOR_FEATURE_SET_ESHUTTER_GAIN:
		pr_err("[kd_sensorlist]enter kdSetExpGain\n");
		if (copy_from_user(
		    (void *)pFeaturePara,
		    (void *) pFeatureCtrl->pFeaturePara,
		    FeatureParaLen)) {
	...
	case SENSOR_FEATURE_SET_PDAF_REG_SETTING:
	{
		...
		if (u4RegLen > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			pr_err("check: u4RegLen > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
	...
	case SENSOR_FEATURE_GET_4CELL_DATA:
	{
		...
		if (buf_size > PDAF_DATA_SIZE) {
			kfree(pFeaturePara);
			pr_err("check: buf_size > PDAF_DATA_SIZE\n");
			return -EINVAL;
		}
		pPdaf_data = kmalloc(
...
imgsensor_compat_ioctl(struct file *filp, unsigned int cmd, unsigned long arg)
{
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO:
	{
		...
		struct IMGSENSOR_GET_CONFIG_INFO_STRUCT __user *data;
		int err;
		/*pr_err("CAOMPAT_KDIMGSENSORIOC_X_GETINFO E\n"); */
	...
	case COMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL:
	{
		...
		/* pr_err("CAOMPAT_KDIMGSENSORIOC_X_FEATURECONCTROL\n"); */
	...
	case COMPAT_KDIMGSENSORIOC_X_CONTROL:
	{
		...
		pr_err("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_CONTROL\n");
	...
	case COMPAT_KDIMGSENSORIOC_X_GETINFO2:
	{
		...
		pr_err("[CAMERA SENSOR] CAOMPAT_KDIMGSENSORIOC_X_GETINFO2\n");
	...
	unsigned long a_u4Param)
{
	...
	if (_IOC_DIR(a_u4Command) != _IOC_NONE) {
		...
		if (pBuff == NULL) {
			pr_err("[CAMERA SENSOR] ioctl allocate mem failed\n");
			i4RetValue = -ENOMEM;
...
#define ISP_CLK_HIGH   490
	...
	if ((_IOC_READ & _IOC_DIR(a_u4Command)) &&
		    copy_to_user((void __user *) a_u4Param,
						...
						_IOC_SIZE(a_u4Command))) {
		kfree(pBuff);
		pr_err("[CAMERA SENSOR] ioctl copy to user failed\n");
		i4RetValue =  -EFAULT;
		goto CAMERA_HW_Ioctl_EXIT;
	}
...
static inline int imgsensor_driver_register(void)
{
	...
	if (alloc_chrdev_region(&dev_no, 0, 1, IMGSENSOR_DEV_NAME)) {
		pr_err("[CAMERA SENSOR] Allocate device no failed\n");
	...
	if (gpimgsensor_cdev ==  NULL) {
		...
		pr_err("[CAMERA SENSOR] Allocate mem for kobject failed\n");
		return -ENOMEM;
	}
...
{
	/*pr_err("mmsys_clk_change_cb ori:%d, new:%d, current_mmsys_clk %d\n",
	 *		ori_clk_mode,
	 *		new_clk_mode,
	 *		current_mmsys_clk);
	 */
...
{
	pr_err("[camerahw_probe] start\n");

      lge_get_camera_hwinfo();  /*LGE_CHANGE, 2019-03-07, skiped unnecessary sensor probing , soojong.jin@lge.com*/
