...
static unsigned int polling_interval = 2000; // 2 sec
...
int get_esd_polling_interval(void)
{
  return polling_interval;
}

void set_esd_polling_interval(void)
{
  ...
  DISPERR("[ESD] polling_interval = %d sec\n", (polling_interval/1000));
}
...
static unsigned int check_use_of_ext_te(void)
{
  int ret = DISABLE;
...
static int primary_display_check_recovery_worker_kthread(void *data)
{
	...
	loop_interval = get_esd_polling_interval();

	while (1) {
		msleep(loop_interval);
...
#ifndef CONFIG_LGE_USE_MTK_ESD_RECOVERY
	_primary_path_switch_dst_lock();
#endif
