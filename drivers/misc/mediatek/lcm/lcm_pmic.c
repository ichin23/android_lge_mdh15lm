...
static int dsv_voltage = -1; // default voltage

void set_display_bias_dsv_voltage(int volt)
{
	dsv_voltage = volt;
	dsv_voltage *= 1000; // uV
}

int get_display_bias_dsv_voltage(void)
{
	return dsv_voltage; // uV
}
...
int display_bias_enable(void)
{
	int ret = 0;
	int retval = 0;
	int disp_dsv_voltage = 0;

	display_bias_regulator_init();
	disp_dsv_voltage = get_display_bias_dsv_voltage();

	/* set voltage with min & max*/
	ret = regulator_set_voltage(disp_bias_pos, disp_dsv_voltage, disp_dsv_voltage);
	if (ret < 0)
		pr_info("set voltage disp_bias_pos fail, ret = %d\n", ret);
	retval |= ret;

	ret = regulator_set_voltage(disp_bias_neg, disp_dsv_voltage, disp_dsv_voltage);
	if (ret < 0)
