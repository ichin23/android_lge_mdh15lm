...
int read_hw_ocv(struct gauge_device *gauge_dev, int *data)
{
...
#ifdef CONFIG_LGE_PM
	if (now_temp > gauge_dev->fg_cust_data->hwocv_swocv_diff_lt_temp)
		now_thr = gauge_dev->fg_cust_data->hwocv_swocv_diff;
	else
		now_thr = gauge_dev->fg_cust_data->hwocv_swocv_diff_lt;
#else /* Mediatek */
	...
	*data = _hw_ocv;
	...
	bm_err("[read_hw_ocv] _hw_ocv %d _hw_ocv_src %d _prev_hw_ocv %d _prev_hw_ocv_src %d _flag_unreliable %d\n",
		...
		_flag_unreliable);
	bm_err("[read_hw_ocv] _hw_ocv_35_pon_rdy %d _hw_ocv_35_pon %d _hw_ocv_35_plugin %d _hw_ocv_chgin %d _sw_ocv %d now_temp %d now_thr %d\n",
