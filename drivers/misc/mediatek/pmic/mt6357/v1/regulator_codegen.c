...
#ifdef CONFIG_LGE_PM_VOLTAGE_EXTENDED_REGULATOR
...
#ifdef CONFIG_LGE_PM
static const int vibr_idx[] = { /* sync idx with datasheet */
	0, 1, 2, 4, 5, 9, 11, 13,
};
#else /* MediaTek */
...
#ifdef CONFIG_LGE_PM_VOLTAGE_EXTENDED_REGULATOR
	if (mreg->vol_cb != NULL && mreg->cal_cb != NULL) {
		ret = (mreg->vol_cb)(idx >> 8);
		ret = (mreg->cal_cb)(idx & 0xF);
	} else {
...
#ifdef CONFIG_LGE_PM_VOLTAGE_EXTENDED_REGULATOR
	if (mreg->da_vol_cb == NULL || mreg->da_cal_cb == NULL) {
...
#ifdef CONFIG_LGE_PM_VOLTAGE_EXTENDED_REGULATOR
	NON_REGULAR_VOLTAGE_EXTENDED_REGULATOR_GEN(vcamd, ldo,
