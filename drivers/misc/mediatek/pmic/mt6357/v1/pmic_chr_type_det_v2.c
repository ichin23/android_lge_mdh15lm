...
#ifdef CONFIG_LGE_PM_APPLE_CHARGER
static unsigned int hw_bc11_stepA1(void)
{
	...
	msleep(80);
	wChargerAvailDM = bc11_get_register_value(PMIC_RGS_BC11_CMP_OUT);
	...
	bc11_set_register_value(PMIC_RG_BC11_CMP_EN, 0x2);
	msleep(80);
	wChargerAvailDP = bc11_get_register_value(PMIC_RGS_BC11_CMP_OUT);
	...
	pr_info("wChargerAvailDM : %d, wChargerAvailDP :  %d\n", wChargerAvailDM, wChargerAvailDP);
...
#endif
#endif /* CONFIG_LGE_PM_APPLE_CHARGER */
...
int hw_charging_get_charger_type(void)
{
	...
	if (lge_is_factory_cable_boot()) {
		if (CHR_Type_num == NONSTANDARD_CHARGER) {
			CHR_Type_num = STANDARD_HOST;
			pr_info("charger type: Force to STANDARD_HOST\n");
		}
	}
#endif
