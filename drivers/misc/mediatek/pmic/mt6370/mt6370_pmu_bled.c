...
int chargepump_set_backlight_level(bool enable)
{
	...
	if (enable == 0){
		if(is_suspend == false){
			...
			mt6370_pmu_bled_bright_set(&mt6370_pmu_bled_dev, 0);
	...
	struct mt6370_pmu_bled_data *bled_data)
{
	...
	uint32_t bright = pdata->max_bled_brightness;
	bled_dim = bright;
#else
	uint32_t bright = (pdata->max_bled_brightness << 8) / 255;
	bled_dim = bright;
...
static inline int mt_parse_dt(struct device *dev)
{
	...
	CPD_LOG("use_pwm = %d, ext_en_pin = %d, chan_en = 0x%x\n", pdata->use_pwm, pdata->ext_en_pin, pdata->chan_en);
	return 0;
}
