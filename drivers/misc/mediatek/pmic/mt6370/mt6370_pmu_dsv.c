...
#include "inc/mt6370_pmu_dsv.h"

static struct mt6370_pmu_dsv_data *g_dsv_data;
static struct mt6370_pmu_dsv_platform_data pdata, mask;
...
/* LGE_CHANGED */
static int mt6370_dsv_external_pin_ctrl(struct mt6370_pmu_chip *chip, u8 val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL1, DB_EXT_EN_MASK << DB_EXT_EN_SHIFT , val << DB_EXT_EN_SHIFT);

	if(ret < 0)
		pr_err("%s: reg update failed\n", __func__);

	return ret;
}

static int mt6370_dsv_periodic_mode(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL1, DB_PERIODIC_MODE_MASK << DB_PERIODIC_MODE_SHIFT, val << DB_PERIODIC_MODE_SHIFT);
...
static int mt6370_dsv_frequency_pm(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL1, DB_FREQ_PM_MASK << DB_FREQ_PM_SHIFT, val << DB_FREQ_PM_SHIFT);
...
static int mt6370_dsv_single_pin_ctrl(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL1, DB_SINGLE_PIN_MASK << DB_SINGLE_PIN_SHIFT, val << DB_SINGLE_PIN_SHIFT);
...
static int mt6370_dsv_periodic_fix(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL1, DB_PERIODIC_FIX_MASK << DB_PERIODIC_FIX_SHIFT, val << DB_PERIODIC_FIX_SHIFT);
...
static int mt6370_dsv_discharge_ctrl(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL2, DB_VPOS_DISC_MASK << DB_VPOS_DISC_SHIFT, val << DB_VPOS_DISC_SHIFT);
	ret |= mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL2, DB_VNEG_DISC_MASK << DB_VNEG_DISC_SHIFT, val << DB_VNEG_DISC_SHIFT);
...
static int mt6370_dsv_startup(struct mt6370_pmu_chip *chip, unsigned char val)
{
	int ret = 0;

	ret = mt6370_pmu_reg_update_bits(chip, MT6370_PMU_REG_DBCTRL2, DB_STARTUP_MASK << DB_STARTUP_SHIFT, val << DB_STARTUP_SHIFT);
...
int mt6370_dsv_set_property(enum DSV_CTRL_PROPERTY prop, u8 val)
{
	int ret = 0;
	struct mt6370_pmu_dsv_data *pdata = g_dsv_data;

	printk("[%s] prop = %d, val = %d\n", __func__, prop, val);

	switch(prop){
	case DB_EXT_EN:
		mt6370_dsv_external_pin_ctrl(pdata->chip, val);
		break;
	case DB_PERIODIC_MODE:
		mt6370_dsv_periodic_mode(pdata->chip, val);
		break;
	case DB_FREQ_PM:
		mt6370_dsv_frequency_pm(pdata->chip, val);
		break;
	case DB_SINGLE_PIN:
		mt6370_dsv_single_pin_ctrl(pdata->chip, val);
		break;
	case DB_PERIODIC_FIX:
		mt6370_dsv_periodic_fix(pdata->chip, val);
		break;
	case DB_VPOS_EN:
		if(val == ENABLE)
			mt6370_dsv_enable(pdata->dsvp->regulator);
		else
			mt6370_dsv_disable(pdata->dsvp->regulator);
		break;
	case DB_VPOS_VNEG_DISCHARGE:
		mt6370_dsv_discharge_ctrl(pdata->chip, val);
		break;
	case DB_VNEG_EN:
		if(val == ENABLE)
			mt6370_dsv_enable(pdata->dsvn->regulator);
		else
			mt6370_dsv_disable(pdata->dsvn->regulator);
		break;
	case DB_STARTUP:
		mt6370_dsv_startup(pdata->chip, val);
		break;
	case DB_VPOS_VOLTAGE:
		if (val >= mt6370_dsv_min_uV && val <= mt6370_dsv_max_uV)
			mt6370_dsv_set_voltage_sel(pdata->dsvp->regulator, (val - mt6370_dsv_min_uV) / mt6370_dsv_step_uV );
		else
			pr_err("%s: VPOS voltage is invalid range!!\n", __func__);
		break;
	case DB_VNEG_VOLTAGE:
		if(val >= mt6370_dsv_min_uV && val <= mt6370_dsv_max_uV)
			mt6370_dsv_set_voltage_sel(pdata->dsvn->regulator, (val - mt6370_dsv_min_uV) / mt6370_dsv_step_uV );
		else
			pr_err("%s: VNEG voltage is invalid range!!\n", __func__);
		break;
	case DB_VPOS_GET_VOLTAGE:
		ret = mt6370_dsv_get_voltage_sel(pdata->dsvp->regulator);
		break;
	case DB_VNEG_GET_VOLTAGE:
		ret = mt6370_dsv_get_voltage_sel(pdata->dsvn->regulator);
		break;
	default:
		return -EINVAL;
	}
...
static ssize_t mt6370_dsv_dump_reg_show(struct device *dev,
				   struct device_attribute *attr,
				   char *buf)
{
	struct mt6370_pmu_dsv_data *dsv_data = dev_get_drvdata(dev);
	unsigned char mt6370_dsv_reg[MT6370_DSV_REG_NUM] = { 0 };
	int i = 0;

	for( i = 0 ; i < MT6370_DSV_REG_NUM ; i++ ){
		mt6370_dsv_reg[i] = mt6370_pmu_reg_read(dsv_data->chip, DB_CTRL1+i);
	}

	return sprintf(buf, "Addr[0xB0] = 0x%x\nAddr[0xB1] = 0x%x\nAddr[0xB2] = 0x%x\nAddr[0xB3] = 0x%x\nAddr[0xB4] = 0x%x\n", \
		mt6370_dsv_reg[0], mt6370_dsv_reg[1], mt6370_dsv_reg[2], mt6370_dsv_reg[3], mt6370_dsv_reg[4]);
}

static const DEVICE_ATTR(mt6370_dsv_dump_reg, S_IRUGO, mt6370_dsv_dump_reg_show, NULL);
...
static int mt6370_pmu_dsv_probe(struct platform_device *pdev)
{
	...
	ret = device_create_file(dsv_data->dev, &dev_attr_mt6370_dsv_dump_reg);
	if (ret < 0) {
		dev_notice(&pdev->dev, "create mt6370 dsv dump reg attr fail\n");
		goto reg_create_file_fail;
	}
