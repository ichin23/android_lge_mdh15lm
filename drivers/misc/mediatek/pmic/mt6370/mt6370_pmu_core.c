...
static int mt6370_pmu_core_reset(struct mt6370_pmu_core_data *core_data)
{
	...
	/* reset fled/ldo/rgb/bl/dsv logic and all pmu register without chg when no battery */
	ret = mt6370_pmu_reg_write(core_data->chip,
		MT6370_PMU_REG_CORECTRL2,
		mt6370_pmu_is_te_enabled() ? 0x7F : 0x3F);
#else /* Mediatek */
	/* reset chg/fled/ldo/rgb/bl/dsv logic and all pmu register */
	ret = mt6370_pmu_reg_write(core_data->chip,
...
#ifdef CONFIG_LGE_PM
	if (mt6370_pmu_ship_mode_enabled())
		return;
#endif
