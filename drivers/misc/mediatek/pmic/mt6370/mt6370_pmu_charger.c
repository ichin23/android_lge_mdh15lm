...
#ifdef CONFIG_LGE_PM
#define OTG_VOBST   0x8F    //OTG Requlation Voltage 5300mV
#endif
static int mt6370_enable_otg(struct charger_device *chg_dev, bool en)
{
	...
	if (usb_compliance_mode) {
		/* OTG Requlation Voltage 5300mV */
        ret = mt6370_pmu_reg_write(chg_data->chip, MT6370_PMU_REG_CHGCTRL5,
                OTG_VOBST);
        if (ret < 0) {
            dev_err(chg_data->dev,
                    "%s: set otg voltage failed %d)\n",
                    __func__, ret);
            goto out;
		}
        }
	...
	u32 uV)
{
	...
	if (unlikely(system_state > SYSTEM_RUNNING)) {
		dev_info(chg_data->dev, "%s: system not running\n", __func__);
		return -ENOTSUPP;
	}
#endif
