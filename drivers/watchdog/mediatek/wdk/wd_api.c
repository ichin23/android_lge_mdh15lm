...
void arch_reset(char mode, const char *cmd)
{
    ...
    } else if (cmd && !strcmp(cmd, "FOTA LCD off")) {
        lge_set_reboot_reason(LGE_REBOOT_REASON_FOTA_LCD_OFF);
        reboot = 1;
    } else if (cmd && !strcmp(cmd, "FOTA OUT LCD off")) {
        lge_set_reboot_reason(LGE_REBOOT_REASON_FOTA_OUT_LCD_OFF);
        reboot = 1;
...
#endif
	} else if (cmd && !strcmp(cmd, "opid mismatched")) {
		lge_set_reboot_reason(LGE_REBOOT_REASON_OPID_MISMATCHED);
		reboot = 1;
	} else {
		reboot = WD_SW_RESET_BYPASS_PWR_KEY;
	}
