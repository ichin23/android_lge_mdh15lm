...
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
extern unsigned char aw87349_audio_kspk(void);
extern unsigned char aw87349_audio_drcv(void);
extern unsigned char aw87349_audio_abrcv(void);
extern unsigned char aw87349_audio_off(void);
extern unsigned char aw87529_audio_receiver(void);
extern unsigned char aw87529_audio_speaker(void);
extern unsigned char aw87529_audio_speaker_headset(void);
extern unsigned char aw87529_audio_headset(void);
...
int (*set_R_channel_mute)(bool enable) = NULL;
void (*set_adda_dl_src_gain)(bool mute) = NULL;
/* Jogi: Need? @{ */
...
int set_codec_ops(struct mtk_codec_ops *ops)
{
	...
	set_R_channel_mute = ops->set_R_channel_mute;
	set_adda_dl_src_gain = ops->set_adda_dl_src_gain;
	return 0;
}
static int audio_get_auxadc_value(void)
{
	...
	res = pmic_get_auxadc_value(AUXADC_LIST_HPOFS_CAL);
	/* pr_debug("%s(), auxadc value %d\n", __func__, res); */
	return res;
#else
	return 0;
#endif
...
static void enable_hs_buffer(bool enable)
{
	if (enable) {
		/* Set HS STB enhance circuits */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x90, 0xffff);
		/* Enable HS driver bias circuits */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x92, 0xffff);
		/* Enable HS driver core circuits */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x93, 0xffff);
	} else
...
#endif //ONFIG_SND_SMARTPA_AW8898
#endif
...
static bool OpenHeadPhoneImpedanceSetting(bool bEnable)
{
	...
	if (bEnable == true) {
		...
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x2000, 0x2000);
		/* Disable handset short-circuit protection */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x0010, 0xffff);
		/* Disable linout short-circuit protection */
		/*enable_lo_buffer(false);*/
		/* Reduce ESD resistance of AU_REFN */
		Ana_Set_Reg(AUDDEC_ANA_CON2, 0x200, 0xfff0);
		/* Turn on DA_600K_NCP_VA18 */
		Ana_Set_Reg(AUDNCP_CLKDIV_CON1, 0x0001, 0xffff);
		/* Set NCP clock as 604kHz // 26MHz/43 = 604KHz */
		Ana_Set_Reg(AUDNCP_CLKDIV_CON2, 0x002c, 0xffff);
		...
		Ana_Set_Reg(AUDDEC_ANA_CON2, 0x200, 0xfff0);
		/* Enable HP main CMFB Switch */
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0200, 0x0200);
		/* Enable AUD_CLK */
		Ana_Set_Reg(AUDDEC_ANA_CON11, 0x1, 0x1);
		/* Enable Audio L channel DAC */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x2006, 0xaca6);
		/* Enable HPDET circuit,
		 * select DACLP as HPDET input and HPR as HPDET output
		 */
		Ana_Set_Reg(AUDDEC_ANA_CON5, 0x1B00, 0xff00);
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0283, 0x0fff);
		Ana_Set_Reg(AUDDEC_ANA_CON7, 0x02, 0x0002);
	} else {
		/* disable HPDET circuit */
		Ana_Set_Reg(AUDDEC_ANA_CON5, 0x0000, 0xff00);
		/* Disable Audio DAC */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x0000, 0x0006);
		if (GetDLStatus() == false) {
			/* Disable AUD_CLK */
			Ana_Set_Reg(AUDDEC_ANA_CON11, 0x0, 0x1);
		}
		...
		Ana_Set_Reg(AUDDEC_ANA_CON1, 0x0, 0x3);
		/* Disable HP driver core circuits */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x0, 0x1 << 5);
		/* Disable HP driver bias circuits */
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x0, 0x1 << 7);
		/* Disable HP aux CMFB loop */
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0, 0xff << 8);
		/* Enable HP main CMFB Switch */
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x2 << 8, 0xff << 8);
		/* Pull-down HPL/R, HS, LO to AVSS28_AUD */
		Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0, 0x0002);
		if (GetDLStatus() == false) {
...
static void detect_impedance(void)
{
...
#endif //CONFIG_SND_SMARTPA_AW8898
#endif
...
static void TurnOnDacPower(int device)
{
	...
	case AUDIO_ANALOG_DEVICE_OUT_SPEAKERR:
		break;
	case AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L:
		...
		Ana_Set_Reg(AUDDEC_ANA_CON7, 0x0080, 0xffff);
		/* clock inverse cci_lch_inv=1'b1*/
		Ana_Set_Reg(AFUNC_AUD_CON0, 0x1 << 10, 0x1 << 10);
		break;
	default:
		/* Dsiable HP main CMFB Switch */
		Ana_Set_Reg(AUDDEC_ANA_CON6, 0x0000, 0x0fff);
	...
	Ana_Set_Reg(AFUNC_AUD_CON0, 0xCBA1, 0xfbff);
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
static bool mAudio_HAC_Mode = false;
#define HAC_AMP_MODE_MIN 1
#define HAC_AMP_MODE_MAX 4
static int mHAC_Amp_Mode = 1; //Default Mode 1
bool mAudio_HAC_Switch = false;
EXPORT_SYMBOL(mAudio_HAC_Switch);
#endif
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
		if(mAudio_HAC_Mode) {
			AudDrv_GPIO_RCV_EXTAMP_Select(true, mHAC_Amp_Mode);
			udelay(500);
		}
#endif
		if (GetDLStatus() == false) {
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
		if(mAudio_HAC_Mode) {
			AudDrv_GPIO_RCV_EXTAMP_Select(false, mHAC_Amp_Mode);
			udelay(500);
		}
#endif
	}
...
static void Headset_Receiver_Amp_Change(bool enable)
{
	pr_debug("%s(), enable %d\n", __func__, enable);
	...
	if (enable) {
		...
		if (GetDLStatus() == false)
			TurnOnDacPower(
				AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L);
		pr_debug("%s(), enable\n", __func__);
		...
		/* HP IVBUF (Vin path) de-gain enable: -12dB */
		if (apply_n12db_gain)
			Ana_Set_Reg(AUDDEC_ANA_CON7, 0x4, 0xf);
		/* Disable handset short-circuit protection */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x0010, 0xffff);
		/* Disable linout short-circuit protection */
		enable_lo_buffer(false);
		...
		/* Switch HS MUX to audio DAC */
		Ana_Set_Reg(AUDDEC_ANA_CON3, 0x009b, 0xffff);
		Ana_Set_Reg(AUDDEC_ANA_CON0, 0x30ff, 0xf0ff);
		/* Switch HPR MUX to DAC */
		...
		if (GetDLStatus() == false) {
			...
			/* decrease HS gain to minimum gain */
			Ana_Set_Reg(ZCD_CON3, DL_GAIN_N_40DB_REG, 0xffff);
...
static int Headset_Receiver_Amp_Get(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	pr_debug("%s()\n", __func__);
	ucontrol->value.integer.value[0] =
...
static int Headset_Receiver_Amp_Set(struct snd_kcontrol *kcontrol,
				   struct snd_ctl_elem_value *ucontrol)
{
	/* struct snd_soc_codec *codec = snd_kcontrol_chip(kcontrol); */
	pr_debug("%s() gain = %lu\n ", __func__,
	...
	if ((ucontrol->value.integer.value[0] == true) &&
	    (mCodec_data->mAudio_Ana_DevicePower
		[AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L] == false)) {
		Headset_Receiver_Amp_Change(true);
		mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L] =
			ucontrol->value.integer.value[0];
	} else if ((ucontrol->value.integer.value[0] == false) &&
		...
		mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_RECEIVER_HEADSET_L] =
			ucontrol->value.integer.value[0];
		Headset_Receiver_Amp_Change(false);
	}
	return 0;
}
...
{
#ifdef BYPASS_HPIMP
	pr_debug("%s(), no need calculate hp_impedance\n", __func__);
	ucontrol->value.integer.value[0] = 32;
	return 0;
#elif defined(CONFIG_MTK_SND_HPIMP)
...
int aux_hp_impedance(void)
{
	...
	if ((mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_SPEAKERL] ||
		mCodec_data->mAudio_Ana_DevicePower
			[AUDIO_ANALOG_DEVICE_OUT_SPEAKERR]) == false){
		set_adda_dl_src_gain(true);
		mute_check = true;
		pr_debug("%s(), set SDM mute\n", __func__);
	}
	mutex_lock(&Ana_Ctrl_Mutex);
	if (OpenHeadPhoneImpedanceSetting(true)) {
		mic_vinp_mv = get_accdet_auxadc();
...
EXPORT_SYMBOL(aux_hp_impedance);
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
static const char *const HAC_Amp_Mode[] = { "MODE_1", "MODE_2", "MODE_3", "MODE_4" };
static int Audio_HAC_Switch_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mAudio_HAC_Switch) {
...
static int Audio_HAC_Switch_set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		AudDrv_GPIO_RCV_EXTAMP_Select(true, mHAC_Amp_Mode);
...
static int Audio_HAC_State_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mAudio_HAC_Mode) {
		ucontrol->value.integer.value[0] = 1;
...
static int Audio_HAC_State_set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mAudio_HAC_Mode = true;
	} else if (ucontrol->value.integer.value[0] == false) {
		mAudio_HAC_Mode = false;
	}
	return 0;
}

static int Audio_HAC_Amp_Mode_get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] = mHAC_Amp_Mode - 1;
	return 0;
}
static int Audio_HAC_Amp_Mode_set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	int amp_mode = ucontrol->value.integer.value[0] + 1;
	if (HAC_AMP_MODE_MIN <= amp_mode && amp_mode <= HAC_AMP_MODE_MAX) {
		mHAC_Amp_Mode = amp_mode;
	}
	return 0;
}
#endif
...
static bool mExtAmpMonoLSPK = false;
static bool mExtAmpMonoRSPK = false;
static bool mExtAmpRCV = false;
static bool mExtAmpSPKHPH = false;
static bool mExtAmpHPH = false;
static bool mExtAmpLRSwap = false;

static int Ext_Amp_Mono_L_SPK_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpMonoLSPK) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_Mono_L_SPK_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpMonoLSPK = true;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		aw87529_audio_off();
		usleep_range(1 * 1000, 2 * 1000);
		aw87529_audio_speaker();
...
static int Ext_Amp_Mono_R_SPK_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpMonoRSPK) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_Mono_R_SPK_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpMonoRSPK = true;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		aw87349_audio_off();
		usleep_range(1 * 1000, 2 * 1000);
		aw87349_audio_kspk();
...
static int Ext_Amp_RCV_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpRCV) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_RCV_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpRCV = true;
...
static int Ext_Amp_SPK_HPH_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpSPKHPH) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_SPK_HPH_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpSPKHPH = true;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		aw87529_audio_speaker_headset();
#endif
	} else if (ucontrol->value.integer.value[0] == false) {
		mExtAmpSPKHPH = false;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
...
static int Ext_Amp_HPH_Switch_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpHPH) {
		ucontrol->value.integer.value[0] = 1;
...
static int Ext_Amp_HPH_Switch_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpHPH = true;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		aw87529_audio_headset();
#endif
	} else if (ucontrol->value.integer.value[0] == false) {
		mExtAmpHPH = false;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
...
static int Ext_Amp_SPK_LR_Swap_Get(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if(mExtAmpLRSwap) {
		ucontrol->value.integer.value[0] = 1;
...
extern void AFE_DL_LR_Swap(bool swap);
static int Ext_Amp_SPK_LR_Swap_Set(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == true) {
		mExtAmpLRSwap = true;
		...
		mExtAmpLRSwap = false;
#if defined(CONFIG_SND_SOC_AW87349) && defined(CONFIG_SND_SOC_AW87529)
		AFE_DL_LR_Swap(false);
#endif
	}
	return 0;
}

static const char *const swap_function[] = { "Normal", "Swap" };
static const struct soc_enum Stereo_Speaker_Enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(swap_function), swap_function),
};
#endif
...
static const struct soc_enum Audio_DL_Enum[] = {
	...
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function),
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(HAC_Amp_Mode), HAC_Amp_Mode),
#endif
};
static const struct snd_kcontrol_new mt6357_snd_controls[] = {
	...
	SOC_ENUM_EXT("Headset_Receiver_Amp_Switch", Audio_DL_Enum[4],
		     Headset_Receiver_Amp_Get,
		     Headset_Receiver_Amp_Set),
...
#ifdef CONFIG_RECEIVER_EXTAMP_USE
	SOC_ENUM_EXT("Audio_HAC_Switch", Audio_DL_Enum[15],
		     Audio_HAC_Switch_get, Audio_HAC_Switch_set),
	SOC_ENUM_EXT("Audio_HAC_State", Audio_DL_Enum[16],
		     Audio_HAC_State_get, Audio_HAC_State_set),
	SOC_ENUM_EXT("Audio_HAC_Amp_Mode", Audio_DL_Enum[17],
		     Audio_HAC_Amp_Mode_get, Audio_HAC_Amp_Mode_set),
#endif
#ifdef CONFIG_LGE_STEREO_SPEAKER
	SOC_ENUM_EXT("Ext_Amp_Mono_L_SPK_Switch", Stereo_Speaker_Enum[0],
		     Ext_Amp_Mono_L_SPK_Switch_Get, Ext_Amp_Mono_L_SPK_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_Mono_R_SPK_Switch", Stereo_Speaker_Enum[1],
		     Ext_Amp_Mono_R_SPK_Switch_Get, Ext_Amp_Mono_R_SPK_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_RCV_Switch", Stereo_Speaker_Enum[2],
		     Ext_Amp_RCV_Switch_Get,  Ext_Amp_RCV_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_SPK_HPH_Switch", Stereo_Speaker_Enum[3],
		     Ext_Amp_SPK_HPH_Switch_Get, Ext_Amp_SPK_HPH_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_HPH_Switch", Stereo_Speaker_Enum[4],
		     Ext_Amp_HPH_Switch_Get, Ext_Amp_HPH_Switch_Set),
	SOC_ENUM_EXT("Ext_Amp_SPK_LR_Swap", Stereo_Speaker_Enum[5],
		     Ext_Amp_SPK_LR_Swap_Get, Ext_Amp_SPK_LR_Swap_Set),
#endif
};
...
#ifdef CONFIG_LGE_CODEC_INPUT_MUTE_CONTROL
static const struct soc_enum Input_Mute_Enum = SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(amp_function), amp_function);
static int INPUT_MUTE_FLAG = 0;
static int Input_Mute_Switch_get(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	ucontrol->value.integer.value[0] =INPUT_MUTE_FLAG;
	return 0;
}
static int Input_Mute_Switch_set(struct snd_kcontrol *kcontrol,
			    struct snd_ctl_elem_value *ucontrol)
{
	if (ucontrol->value.integer.value[0] == 0 || ucontrol->value.integer.value[0] == 1) {
		INPUT_MUTE_FLAG = ucontrol->value.integer.value[0];
	} else {
		pr_info("%s(), return -EINVAL\n", __func__);
		return -EINVAL;
	}
	return 0;
}
...
#ifdef CONFIG_LGE_CODEC_INPUT_MUTE_CONTROL
	if(unlikely(INPUT_MUTE_FLAG == 1)) {
		pr_info("%s(), skip preamp selcetion.\n", __func__);
		return true;
	}
#endif
...
#ifdef CONFIG_LGE_CODEC_INPUT_MUTE_CONTROL
	SOC_ENUM_EXT("Input_Mute_Switch", Input_Mute_Enum,
		     Input_Mute_Switch_get, Input_Mute_Switch_set),
#endif
};
