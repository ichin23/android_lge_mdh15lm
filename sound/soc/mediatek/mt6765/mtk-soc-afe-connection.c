			...
			unsigned int Output)
{
	...
	if (Output == Soc_Aud_InterConnectionOutput_O04)
		mute_state = get_R_channel_mute(ConnectionState, Input);
	...
	case Soc_Aud_InterCon_Connection: {
		...
		if (CheckBitsandReg(connectReg, set_bit)) {
			if (!mute_state)
				Afe_Set_Reg(connectReg, 1 << set_bit,
					    1 << set_bit);
			mConnectionState[Input][Output] |=
				Soc_Aud_InterCon_Connection;
		}
