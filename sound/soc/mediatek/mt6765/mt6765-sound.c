...
unsigned int R_ch_mute_status;
static int set_R_channel_mute(bool enable)
{
	unsigned int status = Afe_Get_Reg(AFE_CONN4);

	R_ch_mute_status = enable;

	/* open adda dl path for dc compensation */
	if (enable) {
		restore_value = Afe_Get_Reg(AFE_CONN4);
		Afe_Set_Reg(AFE_CONN4, 0x0, 0xffffffff);
	} else
		Afe_Set_Reg(AFE_CONN4, restore_value|status, 0xffffffff);

	pr_debug("%s(), enable %d AFE_CONN4= 0x%x\n",
		 __func__, enable, Afe_Get_Reg(AFE_CONN4));

	return 0;
}
bool get_R_channel_mute(unsigned int ConnectionState,
			unsigned int Input)
{
	int set_bit = Input < Soc_Aud_InterConnectionInput_I32
			? Input : Input - Soc_Aud_InterConnectionInput_I32;

	if (R_ch_mute_status) {
		if (ConnectionState == Soc_Aud_InterCon_DisConnect)
			restore_value &= ~(0x1 << set_bit);
		else /* ConnectionState == Soc_Aud_InterCon_DisConnect */
			restore_value |= (0x1 << set_bit);
		return true;
	}
	return false;
...
static struct mtk_codec_ops mtk_codec_platform_ops = {
	...
	.set_R_channel_mute = set_R_channel_mute,
	.set_adda_dl_src_gain = set_adda_dl_src_gain,
};
