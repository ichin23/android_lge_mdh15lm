...
kal_uint32 Mask_LR_Swap = 0x1 << 20;
void AFE_DL_LR_Swap(bool swap)
{
	if(swap) {
		Afe_Set_Reg(AFE_ADDA_DL_SDM_DCCOMP_CON, Mask_LR_Swap, Mask_LR_Swap);
	} else {
		Afe_Set_Reg(AFE_ADDA_DL_SDM_DCCOMP_CON, 0x0, Mask_LR_Swap);
	}
	pr_info("%s AFE LR Swap %d\n", __func__, swap);
}
#endif
