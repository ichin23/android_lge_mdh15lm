...
#include <../mediatek/common_int/mtk-auddrv-gpio.h>
#endif
struct reg_config {
	uint8_t reg_addr;
	uint32_t reg_data;
...
static int rt5509_init_impedance_ctrl_fix(struct snd_soc_codec *codec)
{
    ...
    ret = snd_soc_write(codec, RT5509_REG_DELAYRES, result);
    inited = true;
    return ret;
}
...
#if defined(CONFIG_LGE_EXTAMPTYPE_HWINFO)
	hwinfo = lge_get_extamptype_hwinfo();
	dev_err(&client->dev, "get extamptype hwinfo\n");

   if ( hwinfo != 1) { // 1 : richtek, 0 : awnic.
        pr_err("%s: RT5509 is not support for this pcb(default is AW87318) \n", __func__);
