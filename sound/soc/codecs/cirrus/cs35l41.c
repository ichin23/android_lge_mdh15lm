...
#ifdef CONFIG_SND_SOC_CS35L41_TEST
static const char *const Amp_Function[] = { "Off", "On" };
static int Amp_Status = 0;
static const struct soc_enum Test_Amp_Enum = SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(Amp_Function), Amp_Function);
static int TestAmpSwitchGet(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	pr_info("%s() = %d\n", __func__, Amp_Status);
	ucontrol->value.integer.value[0] = Amp_Status;
	return 0;
}

static int TestAmpSwitchSet(struct snd_kcontrol *kcontrol,
			  struct snd_ctl_elem_value *ucontrol)
{
	struct snd_soc_codec *codec = snd_soc_kcontrol_codec(kcontrol);
	struct cs35l41_private *cs35l41 = snd_soc_codec_get_drvdata(codec);
...
#ifdef CONFIG_SND_SOC_CS35L41_TEST
	SOC_ENUM_EXT("Cirrus Main Amp Test Switch", Test_Amp_Enum, TestAmpSwitchGet, TestAmpSwitchSet),
#endif
...
static irqreturn_t cs35l41_irq(int irq, void *data)
{
	...
	if (status[3] & CS35L41_OTP_BOOT_DONE) {
		dev_crit(cs35l41->dev, "cs35l41_irq CS35L41_OTP_BOOT_DONE....\n");
		regmap_update_bits(cs35l41->regmap, CS35L41_IRQ1_MASK4,
				CS35L41_OTP_BOOT_DONE, CS35L41_OTP_BOOT_DONE);
	}

	dev_crit(cs35l41->dev, "cs35l41_irq end....\n");
...
static const unsigned int cs35l41_src_rates[] = {
	8000, 12000, 11025, 16000, 22050, 24000, 32000, 44100, 48000
};
#else
static const unsigned int cs35l41_src_rates[] = {
...
static int cs35l41_codec_probe(struct snd_soc_codec *codec)
{
	...
	pr_info("cs35l41_codec_probe start\n");
	...
	pr_info("cs35l41_codec_probe end\n");
exit:
	return ret;
}
...
#ifdef CONFIG_SND_SOC_CS35L41_LGE
struct pinctrl *pinctrl_cs35l41 = NULL;
struct pinctrl_state *gpio2_int_pin_ctrl = NULL;
#endif
				...
				struct cs35l41_platform_data *pdata)
{
...
#ifdef CONFIG_SND_SOC_CS35L41_LGE
	pinctrl_cs35l41= devm_pinctrl_get(cs35l41->dev);
	if (IS_ERR(pinctrl_cs35l41)) {
		pr_err("%s Cannot find pinctrl_cs35l41!\n",__func__);
	} else {
		gpio2_int_pin_ctrl = pinctrl_lookup_state(pinctrl_cs35l41, "default");
		if (IS_ERR(gpio2_int_pin_ctrl)) {
			ret = PTR_ERR(gpio2_int_pin_ctrl);
			pr_err("%s pinctrl_lookup_state default fail %d\n", __func__, ret);
		} else {
			ret = pinctrl_select_state(pinctrl_cs35l41, gpio2_int_pin_ctrl);
			if (ret) pr_err("%s could not set default pin\n",__func__);
		}
	}
#endif
...
#ifdef CONFIG_SND_SOC_CS35L41_LGE
	cs35l41->irq = of_get_named_gpio(cs35l41->dev->of_node, "irq-gpio", 0);
	ret = devm_request_threaded_irq(cs35l41->dev, gpio_to_irq(cs35l41->irq), NULL,
...
#ifndef CONFIG_SND_SOC_CS35L41_LGE
		goto err;
#endif
	}
	...
	dev_err(cs35l41->dev, "Cirrus Logic CS35L41 (%x), Revision: %02X\n",
			regid, reg_revid);
	pr_info("cs35l41_probe end\n");
