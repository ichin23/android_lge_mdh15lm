/*
* aw87529_audio.c   aw87529 pa module
*
* Version: v1.0.5
*
...
#include "aw87529_audio.h"
/*******************************************************************************
* aw87529 marco
******************************************************************************/
#define AW87529_I2C_NAME	"aw87529_pa"
#define AW87529_DRIVER_VERSION	"v1.0.5"

/*******************************************************************************
* aw87529 variable
******************************************************************************/
struct aw87529 *aw87529;
struct aw87529_container *aw87529_spk_cnt;
struct aw87529_container *aw87529_rcv_cnt;

static char *aw87529_spk_name = "aw87529_spk.bin";
static char *aw87529_rcv_name = "aw87529_rcv.bin";
...
unsigned int aw87529_hw_on(struct aw87529 *aw87529)
{
	...
	if (aw87529 && gpio_is_valid(aw87529->reset_gpio)) {
		...
		dev_err(&aw87529->i2c_client->dev, "%s:  failed\n", __func__);
	}
...
******************************************************************************/
unsigned char aw87529_audio_receiver(void)
{
	unsigned int i;
	unsigned int length;
	...
	length = sizeof(aw87529_spk_cfg_default)/sizeof(char);
	if (aw87529->rcv_cfg_update_flag == 0) {   /*update array data*/
		for (i = 0; i < length; i = i+2)
			aw87529_i2c_write(aw87529,
					aw87529_rcv_cfg_default[i],
					aw87529_rcv_cfg_default[i+1]);
	}
	if (aw87529->rcv_cfg_update_flag == 1) {  /*update bin data*/
		for (i = 0; i < aw87529_rcv_cnt->len; i = i+2)
			aw87529_i2c_write(aw87529,
					aw87529_rcv_cnt->data[i],
					aw87529_rcv_cnt->data[i+1]);
	}

	return 0;
}

unsigned char aw87529_audio_speaker(void)
{
	unsigned int i;
	unsigned int length;
	...
	length = sizeof(aw87529_spk_cfg_default)/sizeof(char);
	if (aw87529->spk_cfg_update_flag == 0) { /*send array data*/
		for (i = 0; i < length; i = i+2)
			aw87529_i2c_write(aw87529,
					aw87529_spk_cfg_default[i],
					aw87529_spk_cfg_default[i+1]);
	}
	if (aw87529->spk_cfg_update_flag == 1) {  /*send bin data*/
		for (i = 0; i < aw87529_spk_cnt->len; i = i+2)
			aw87529_i2c_write(aw87529,
					aw87529_spk_cnt->data[i],
					aw87529_spk_cnt->data[i+1]);
	}

	return 0;
}

unsigned char aw87529_audio_speaker_headset(void)
{
	unsigned int i;
	unsigned int length;
	...
	length = sizeof(aw87529_spk_hph_cfg_default)/sizeof(char);
	if (aw87529->spk_cfg_update_flag == 0) { /*send array data*/
		for (i = 0; i < length; i = i+2)
			aw87529_i2c_write(aw87529,
					aw87529_spk_hph_cfg_default[i],
					aw87529_spk_hph_cfg_default[i+1]);
	}
	if (aw87529->spk_cfg_update_flag == 1) {  /*send bin data*/
		for (i = 0; i < aw87529_spk_cnt->len; i = i+2)
			aw87529_i2c_write(aw87529,
	...
	aw87529_i2c_write(aw87529, 0x01, 0xf4);  /* Enable Combo Path */

	return 0;
}

unsigned char aw87529_audio_headset(void)
{
	unsigned int i;
	unsigned int length;
...
******************************************************************************/
static void aw87529_receiver_cfg_loaded(const struct firmware *cont,
				void *context)
{
	int i = 0;
	...
	/* aw87529 ram update */
	aw87529_rcv_cnt = kzalloc(cont->size+sizeof(int), GFP_KERNEL);
	if (!aw87529_rcv_cnt) {
		release_firmware(cont);
		pr_err("%s: Error allocating memory\n", __func__);
		return;
	}
	aw87529_rcv_cnt->len = cont->size;
	memcpy(aw87529_rcv_cnt->data, cont->data, cont->size);
	release_firmware(cont);

	aw87529->rcv_cfg_update_flag = 1;

	pr_info("%s: fw update complete\n", __func__);
}

static int aw87529_rcv_update(struct aw87529 *aw87529)
{
	pr_info("%s enter\n", __func__);
	return request_firmware_nowait(THIS_MODULE,
					...
					GFP_KERNEL,
					aw87529,
					aw87529_receiver_cfg_loaded);
}

static void aw87529_speaker_cfg_loaded(const struct firmware *cont,
				void *context)
{
	int i = 0;
	...
	/* aw87529 ram update */
	aw87529_spk_cnt = kzalloc(cont->size+sizeof(int), GFP_KERNEL);
		if (!aw87529_spk_cnt) {
		release_firmware(cont);
		pr_err("%s: Error allocating memory\n", __func__);
		return;
	}
	aw87529_spk_cnt->len = cont->size;
	memcpy(aw87529_spk_cnt->data, cont->data, cont->size);
	release_firmware(cont);

	aw87529->spk_cfg_update_flag = 1;
	pr_info("%s: fw update complete\n", __func__);

	aw87529_rcv_update(aw87529);
}


#ifdef AWINIC_CFG_UPDATE_DELAY
static int aw87529_cfg_update(struct aw87529 *aw87529)
{
	pr_info("%s enter\n", __func__);
...
out:
	dev_err(&aw87529->i2c_client->dev, "%s: i2c access fail to register\n",
		__func__);
out_strtoint:
	dev_err(&aw87529->i2c_client->dev, "%s: fail to change str to int\n",
		__func__);
	return ret;
}
...
int aw87529_read_chipid(struct aw87529 *aw87529)
{
	...
	while (cnt < AW_READ_CHIPID_RETRIES) {
		aw87529_i2c_write(aw87529, 0x64, 0x2C);
		ret = aw87529_i2c_read(aw87529, REG_CHIPID, &reg_val);
		if (reg_val == AW87529_CHIPID) {
			pr_info("%s This Chip is  AW87529 chipid=0x%x\n",
				__func__, reg_val);
			return 0;
		}
		pr_info("%s: aw87529 chipid=0x%x error\n", __func__, reg_val);
		cnt++;
		usleep_range(2000, 2500);
	}
			...
			const struct i2c_device_id *id)
{
	...
	aw87529 = devm_kzalloc(&client->dev, sizeof(struct aw87529), GFP_KERNEL);
	if (aw87529 == NULL) {
		ret = -ENOMEM;
	...
	if (ret < 0) {
		dev_err(&client->dev, "%s: aw87529_read_chipid failed ret=%d\n",
			__func__, ret);
	...
	/* aw87529 cfg update */
	aw87529->spk_cfg_update_flag = 0;
	aw87529->rcv_cfg_update_flag = 0;
